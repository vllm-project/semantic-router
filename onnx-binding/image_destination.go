...
var publicImageDestinationIPv6Prefix = netip.MustParsePrefix("2000::/3")

var blockedImageDestinationPrefixes = [...]netip.Prefix{
...
func isPublicPref64Address(address netip.Addr) bool {
	return isPublicNativeIPv6Destination(address)
}

func pref64OverlapsBlockedImageDestination(prefix netip.Prefix) bool {
	for _, blocked := range blockedImageDestinationPrefixes {
		if !blocked.Addr().Is6() {
			continue
		}
...
func isPublicImageDestination(address netip.Addr) bool {
	if !address.IsValid() || address.Zone() != "" {
		return false
	}
	address = address.Unmap()
	if embedded, ok := embeddedTranslationIPv4(address); ok {
		return isPublicImageDestination(embedded)
	}
	if address.Is6() {
		return isPublicNativeIPv6Destination(address)
	}
	...
	for _, prefix := range blockedImageDestinationPrefixes {
...
func isPublicNativeIPv6Destination(address netip.Addr) bool {
	...
	if !publicImageDestinationIPv6Prefix.Contains(address) ||
...
func isPublicImageDestinationWithPrefixes(address netip.Addr, prefixes []netip.Prefix) bool {
	if !isPublicImageDestination(address) {
		return false
	}
	address = address.Unmap()
...
func imageDestinationsArePublic(
	ctx context.Context,
	...
	for _, address := range addresses {
		if !isPublicImageDestinationWithPrefixes(address, prefixes) {
			return false
		}
	}
	return true
