...
class Finding:
    ...
    def to_dict(self):
        return {
            ...
            "snippet": self.snippet[:200], "ast_context": self.ast_context,
        }
...
SKIP_DIRS = {
    ...
    "build", "egg-info", ".eggs", ".venv-onnx",
    "security", "supply-chain-security-scan",
}
SKIP_DIR_PATTERNS = {
    "model", "models", "weights", "checkpoint", "onnx", "safetensors",
    "target", "site-packages", "lora_", "mmbert", "pii_",
}
...
def _find_nested_call(node, target_names: set) -> Optional[str]:
...
KNOWN_TEST_DIRS = {"test", "tests", "spec", "specs", "mock", "mocks",
                   "fixture", "fixtures", "example", "examples", "testdata",
                   "testing", "e2e"}
...
def _is_test_file(filepath: str) -> bool:
    ...
    return (any(p.lower() in KNOWN_TEST_DIRS for p in parts)
            or fname.endswith("_test") or fname.startswith("test_"))
...
CREDENTIAL_STRINGS = {
    ...
    "~/.config/gcloud", "~/.azure", "~/.npmrc", "~/.vault-token",
    "~/.netrc", "~/.gitconfig", "~/.git-credentials",
    "/etc/ssl/private", "/etc/kubernetes",
    ".bash_history", ".zsh_history",
    ".bitcoin", ".ethereum", ".solana",
    "169.254.169.254", "metadata.google.internal",
    "/home/user/.ssh", "/root/.ssh", "/root/.aws",
}
...
JS_PROCESS_SPAWN = {"exec", "execSync", "spawn", "spawnSync", "execFile",
                    "execFileSync"}
...
RS_PROCESS_SPAWN = {"Command::new", "process::Command::new",
                    "std::process::Command::new"}
RS_FILE_READ = {"fs::read_to_string", "fs::read", "std::fs::read_to_string",
                "std::fs::read"}
...
ALLOWED_URL_DOMAINS = {
    ...
    "crates.io", "pkg.go.dev", "rubygems.org", "repo1.maven.org",
    "central.sonatype.com",
    # Code hosting
    "github.com", "githubusercontent.com", "api.github.com",
    "github.io", "githubassets.com",
    "gitlab.com", "bitbucket.org",
    # Cloud providers
    "amazonaws.com", "s3.amazonaws.com", "awsstatic.com",
    "googleapis.com", "storage.googleapis.com", "cloud.google.com",
    "azure.com", "blob.core.windows.net", "azureedge.net",
    "azurewebsites.net", "microsoft.com", "windows.net",
    "cloudflare.com", "cloudflare-dns.com",
    "cloudfront.net", "akamaized.net",
    "digitalocean.com", "spaces.digitaloceanspaces.com",
    # CDNs
    "cdn.jsdelivr.net", "unpkg.com", "cdnjs.cloudflare.com", "esm.sh",
    "esm.run", "cdn.skypack.dev", "fonts.googleapis.com",
    "fonts.gstatic.com",
    # Documentation / specs / academic
    "python.org", "docs.python.org",
    "nodejs.org", "docs.rs", "go.dev", "golang.org",
    "readthedocs.io", "readthedocs.org",
    "wikipedia.org", "wikimedia.org",
    "stackoverflow.com", "stackexchange.com",
    "w3.org", "w3schools.com", "ietf.org",
    "schema.org", "json-schema.org", "json.org",
    "yaml.org", "toml.io", "semver.org",
    "developer.mozilla.org", "mozilla.org",
    "swagger.io", "openapis.org",
    "arxiv.org", "doi.org", "acm.org", "ieee.org",
    "creativecommons.org", "contributor-covenant.org",
    # Container registries
    "docker.io", "docker.com", "registry.hub.docker.com",
    "ghcr.io", "quay.io", "gcr.io", "k8s.io",
    # CI/CD & code quality
    ...
    # Auth providers
    "auth0.com", "okta.com", "accounts.google.com",
    # Monitoring
    ...
    # Container / infra
    "podman.io", "kubernetes.io", "helm.sh",
    "terraform.io", "hashicorp.com",
    "jenkins.io", "gradle.org",
    # Example / test
...
def _load_url_allowlist(root: Path):
    ...
    _url_allowlist = set(d.lower() for d in ALLOWED_URL_DOMAINS)
...
PY_DNS_FUNCS = {
    ...
    "dns.resolver.query", "dns.resolver.resolve",
}
    ...
    for filepath, added_lines in file_additions.items():
        ...
        if parser is None:
            for ln, content in added_lines:
                changes.append(StructuralChange(
                    filepath, "raw_line", "", "added", ln, ln, content))
            continue
        try:
        ...
        for child in tree.root_node.children:
            if child.type not in ENTITY_TYPES and not child.is_named:
                continue
            child_lines = set(range(child.start_point.row + 1,
                                    child.end_point.row + 2))
            ...
            changes.append(StructuralChange(
                filepath, kind, name, ct,
                child.start_point.row + 1, child.end_point.row + 1,
...
def _check_network_tool_exfil(fn_name: str, node, filepath: str,
                               result: ScanResult):
    ...
    for tool in NETWORK_TOOLS:
        if tool in text and any(kw in text for kw in ("post", "--data", "-d ")):
            result.add(Severity.HIGH, "AST_SUBPROCESS_EXFIL", filepath,
...
def _check_credential_strings(node, filepath: str, is_test: bool,
                               result: ScanResult):
    """Shared: flag string literals referencing sensitive paths."""
    STRING_TYPES = {
        "string", "string_literal", "interpreted_string_literal",
        "raw_string_literal", "string_content", "template_string",
    }
    ...
    for cred in CREDENTIAL_STRINGS:
        if cred in text:
            sev = Severity.HIGH if ("169.254.169.254" in text
                                     or "metadata.google" in text) else Severity.MEDIUM
            result.add(sev, "AST_CREDENTIAL_REF", filepath,
                       node.start_point.row + 1,
                       f"String references sensitive path: {cred}",
                       text[:150], _ast_path(node))
            return
...
def _check_long_base64(node, filepath: str, result: ScanResult):
    ...
    STRING_TYPES = {
        "string", "string_literal", "interpreted_string_literal",
        "raw_string_literal", "concatenated_string", "template_string",
    }
    ...
    for m in LONG_B64_RE.finditer(text):
        if len(m.group()) > 500:
            result.add(Severity.HIGH, "AST_LONG_BASE64", filepath,
                       node.start_point.row + 1,
                       f"Long base64 string ({len(m.group())} chars) in literal",
                       m.group()[:120] + "...", _ast_path(node))
            return
...
def _check_url(node, filepath: str, is_test: bool, result: ScanResult):
    ...
    for pat in EXFIL_SERVICE_PATTERNS:
        if pat.search(text):
            result.add(Severity.CRITICAL, "URL_KNOWN_EXFIL_SERVICE", filepath,
                       node.start_point.row + 1,
    ...
    for tld in SUSPICIOUS_TLDS:
        if domain.endswith(tld):
            result.add(Severity.HIGH, "URL_SUSPICIOUS_TLD", filepath,
                       node.start_point.row + 1,
                       f"URL not in allowlist, suspicious TLD ({tld}): "
                       f"{url_short}",
                       text[:150], _ast_path(node))
            return

    # Not in allowlist → MEDIUM
    result.add(Severity.MEDIUM, "URL_NOT_ALLOWLISTED", filepath,
               node.start_point.row + 1,
...
def _scan_python_call(node, fn_name: str, filepath: str, is_test: bool,
                      result: ScanResult):
    bare = fn_name.split(".")[-1] if "." in fn_name else fn_name
    ...
    if bare in PY_DYNAMIC_EXEC:
        ...
        if args:
            ...
            if nested:
                result.add(Severity.CRITICAL, "AST_OBFUSCATED_EXEC", filepath,
                           node.start_point.row + 1,
                ...
                for s in _collect_string_values(args):
                    if any(len(m) > 100 for m in LONG_B64_RE.findall(s)):
                        result.add(Severity.CRITICAL, "AST_EXEC_LONG_ENCODED",
                                   filepath, node.start_point.row + 1,
    ...
    if bare == "__import__":
        ...
        if gp and gp.type in ("attribute", "subscript", "argument_list"):
            ...
            result.add(Severity.HIGH, "AST_DYNAMIC_IMPORT", filepath,
                       node.start_point.row + 1,
                       "Dynamic __import__ with attribute/call access — evasion",
                       ctx.text.decode(errors="replace")[:150], _ast_path(node))

    if fn_name in ("types.CodeType",):
        result.add(Severity.HIGH, "AST_BYTECODE_INJECT", filepath,
                   node.start_point.row + 1,
                   "types.CodeType() — dynamic bytecode creation",
                   node.text.decode(errors="replace")[:150], _ast_path(node))

    if fn_name == "marshal.loads":
        result.add(Severity.HIGH, "AST_MARSHAL_LOADS", filepath,
                   node.start_point.row + 1,
        ...
        if args:
            for s in _collect_string_values(args):
                if _SCHEME_RE.match(s) and not _is_url_allowed(s):
                    result.add(Severity.HIGH, "AST_HTTP_EXFIL", filepath,
                               node.start_point.row + 1,
        ...
        if args:
            ...
            for s in _collect_string_values(args):
                if len(s) > 60 or LONG_B64_RE.search(s):
                    result.add(Severity.HIGH, "AST_DNS_EXFIL", filepath,
                               node.start_point.row + 1,
                ...
                if has_concat:
                    result.add(Severity.MEDIUM, "AST_DNS_EXFIL", filepath,
                               node.start_point.row + 1,
    ...
    if fn_name in ("json.dumps", "json.dump", "str", "repr") and not is_test:
        ...
        if args and "os.environ" in args.text.decode(errors="replace"):
            result.add(Severity.HIGH, "AST_ENV_SERIALIZE", filepath,
                       node.start_point.row + 1,
    ...
    if fn_name in PY_HTTP_EXFIL and not is_test:
        args = node.child_by_field_name("arguments")
        if args and "os.environ" in args.text.decode(errors="replace"):
            result.add(Severity.CRITICAL, "AST_ENV_HTTP_EXFIL", filepath,
                       node.start_point.row + 1,
...
def _check_setup_py_class(node, filepath: str, result: ScanResult):
    ...
    for hook_base in PY_SETUP_HOOK_BASES:
        ...
        suspicious = [s for s in (
            "os.system", "subprocess", "exec(", "eval(",
            "Popen", "urlopen", "urlretrieve", "requests.",
            "curl", "wget", "base64",
        ) if s in body]
        if suspicious:
            result.add(
                Severity.CRITICAL, "SETUP_PY_CMD_OVERWRITE", filepath,
                node.start_point.row + 1,
                f"setup.py overrides '{hook_base}' command with "
                f"suspicious calls ({', '.join(suspicious[:3])}) — "
                "auto-executes on pip install",
                body[:200], _ast_path(node))
        else:
            result.add(
                Severity.MEDIUM, "SETUP_PY_CMD_OVERRIDE", filepath,
                node.start_point.row + 1,
...
def _check_setup_py_module_exec(node, filepath: str, result: ScanResult):
    ...
    text = node.text.decode(errors="replace")
    for danger in ("exec(", "eval(", "os.system(", "subprocess.",
                   "Popen(", "__import__("):
        if danger in text:
            result.add(
                Severity.HIGH, "SETUP_PY_MODULE_EXEC", filepath,
                node.start_point.row + 1,
                f"Module-level {danger.rstrip('(')} in setup.py — "
                "executes on pip install",
                text[:200], _ast_path(node))
            return
...
def _check_download_and_execute(node, filepath: str, result: ScanResult):
    ...
    if _is_test_file(filepath):
        return
    body = node.text.decode(errors="replace")
    has_download = any(f in body for f in (
        "urlretrieve(", "urlopen(", "requests.get(", "httpx.get(",
        ".download(", 'wget "', 'wget \'', 'curl "', 'curl \'', "curl -",
    ))
    has_execute = any(f in body for f in (
        "os.system(", "os.popen(", "os.chmod(",
        "os.execv(", "Popen(", "check_output(", "check_call(",
    ))
    if has_download and has_execute:
        name_node = node.child_by_field_name("name")
        name = name_node.text.decode(errors="replace") if name_node else "<anon>"
        result.add(Severity.HIGH, "AST_DOWNLOAD_EXECUTE", filepath,
                   node.start_point.row + 1,
...
def _scan_conftest_py(filepath: str, source: bytes, result: ScanResult):
    ...
    for cred in ("~/.ssh", "~/.aws", "~/.kube", "/root/.ssh"):
        if cred in body:
            ...
            break
    if suspicious:
        result.add(Severity.MEDIUM, "CONFTEST_SUSPICIOUS", filepath, 1,
...
def _scan_go_call(node, fn_name: str, filepath: str, is_test: bool,
                  result: ScanResult):
        ...
        if args:
                ...
                if sl in ("sh", "bash", "cmd", "/bin/sh", "/bin/bash"):
                    for s2 in strings:
                        if any(t in s2.lower() for t in NETWORK_TOOLS):
                            result.add(Severity.HIGH, "AST_GO_SHELL_EXEC",
                                       filepath, node.start_point.row + 1,
            ...
            for tool in NETWORK_TOOLS:
                if tool in text_lower and any(
                    ...
                    result.add(Severity.HIGH, "AST_GO_SUBPROCESS_EXFIL",
                               filepath, node.start_point.row + 1,
                               f"{fn_name}() spawning {tool} with POST",
                               node.text.decode(errors="replace")[:150],
                               _ast_path(node))
                    return
            if any(s.lower() in ("sh", "bash", "/bin/sh", "/bin/bash")
                   for s in strings):
                for s in strings:
                    if "|" in s and any(t in s.lower() for t in NETWORK_TOOLS):
                        result.add(Severity.HIGH, "AST_GO_PIPE_EXFIL",
                                   filepath, node.start_point.row + 1,
    ...
    if fn_name in GO_FILE_READ and not is_test:
        ...
        if args:
            for s in _collect_string_values(args):
                for cred in CREDENTIAL_STRINGS:
                    if cred in s:
                        result.add(Severity.HIGH, "AST_GO_CRED_READ", filepath,
                                   node.start_point.row + 1,
    ...
    if fn_name in GO_HTTP_EXFIL and not is_test:
        ...
        if args:
            for s in _collect_string_values(args):
                if _SCHEME_RE.match(s) and not _is_url_allowed(s):
                    result.add(Severity.HIGH, "AST_GO_HTTP_EXFIL",
                               filepath, node.start_point.row + 1,
    ...
    if fn_name.startswith("reflect.ValueOf") and not is_test:
        ...
        if "MethodByName" in full:
            result.add(Severity.HIGH, "AST_GO_REFLECT_EVASION", filepath,
                       node.start_point.row + 1,
                       "reflect.ValueOf().MethodByName() — dynamic dispatch evasion",
                       full[:150], _ast_path(node))
...
def _scan_go_init_func(node, filepath: str, result: ScanResult):
    ...
    for cred in CREDENTIAL_STRINGS:
        if cred in body:
            ...
            break
    if suspicious:
        result.add(Severity.HIGH, "AST_GO_INIT_SUSPICIOUS", filepath,
                   node.start_point.row + 1,
...
def _scan_js_call(node, fn_name: str, filepath: str, is_test: bool,
                  result: ScanResult):
    bare = fn_name.split(".")[-1] if "." in fn_name else fn_name
    ...
    if bare in JS_DYNAMIC_EXEC:
        ...
        if args:
            ...
            if nested:
                result.add(Severity.CRITICAL, "AST_JS_EVAL_DECODE", filepath,
                           node.start_point.row + 1,
                ...
                for s in _collect_string_values(args):
                    if any(len(m) > 100 for m in LONG_B64_RE.findall(s)):
                        result.add(Severity.CRITICAL, "AST_JS_EVAL_ENCODED",
                                   filepath, node.start_point.row + 1,
                                   f"{bare}() with long encoded string",
                                   node.text.decode(errors="replace")[:150],
                                   _ast_path(node))
        ...
        if args:
            for s in _collect_string_values(args):
                if s in JS_DANGEROUS_REQUIRE or "child_process" in s:
                    result.add(Severity.MEDIUM, "AST_JS_CHILD_PROCESS", filepath,
                               node.start_point.row + 1,
            ...
            if "+" in text and "process" in text.lower():
                result.add(Severity.HIGH, "AST_JS_DYNAMIC_REQUIRE", filepath,
                           node.start_point.row + 1,
    ...
    if bare in JS_PROCESS_SPAWN:
        ...
        if args:
            text = args.text.decode(errors="replace").lower()
            for tool in NETWORK_TOOLS:
                if tool in text and any(kw in text for kw in
                                        ("post", "--data", "-d ")):
                    result.add(Severity.HIGH, "AST_JS_SUBPROCESS_EXFIL",
                               filepath, node.start_point.row + 1,
    ...
    if fn_name in JS_HTTP_EXFIL and not is_test:
        ...
        if args:
            for s in _collect_string_values(args):
                if _SCHEME_RE.match(s) and not _is_url_allowed(s):
                    result.add(Severity.HIGH, "AST_JS_HTTP_EXFIL",
                               filepath, node.start_point.row + 1,
                               f"{fn_name}() to non-allowlisted URL",
                               node.text.decode(errors="replace")[:150],
                               _ast_path(node))
    ...
    if fn_name == "process.env" and not is_test:
        ...
        if parent and parent.type not in ("member_expression", "subscript_expression"):
            result.add(Severity.MEDIUM, "AST_JS_BULK_ENV", filepath,
                       node.start_point.row + 1,
...
def _scan_js_new(node, filepath: str, is_test: bool, result: ScanResult):
    ...
    if name == "Function":
        ...
        if args:
            nested = _find_nested_call(args, JS_DECODE_FUNCS)
            if nested:
                result.add(Severity.CRITICAL, "AST_JS_FUNCTION_DECODE",
                           filepath, node.start_point.row + 1,
                ...
                for s in _collect_string_values(args):
                    if any(len(m) > 100 for m in LONG_B64_RE.findall(s)):
                        result.add(Severity.CRITICAL, "AST_JS_FUNCTION_ENCODED",
                                   filepath, node.start_point.row + 1,
...
def _scan_rust_call(node, fn_name: str, filepath: str, is_test: bool,
                    result: ScanResult):
    if fn_name in RS_PROCESS_SPAWN:
        ...
        if args:
            ...
            for s in strings:
                sl = s.lower()
                if sl in ("sh", "bash", "/bin/sh", "/bin/bash"):
                    if any(t in full_text.lower() for t in NETWORK_TOOLS):
                        result.add(Severity.HIGH, "AST_RS_SHELL_EXEC",
                                   filepath, node.start_point.row + 1,
                                   f"{fn_name}() shell exec with network tool",
                                   full_text[:150], _ast_path(node))
                        return
                for tool in NETWORK_TOOLS:
                    if tool == sl:
                        if any(kw in full_text.lower()
                               for kw in ("post", "--data", "-d ")):
                            result.add(Severity.HIGH, "AST_RS_SUBPROCESS_EXFIL",
                                       filepath, node.start_point.row + 1,
                                       f"{fn_name}() spawning {tool} with POST",
                                       full_text[:150], _ast_path(node))
                            return
    ...
    if fn_name in RS_FILE_READ and not is_test:
        ...
        if args:
            for s in _collect_string_values(args):
                for cred in CREDENTIAL_STRINGS:
                    if cred in s:
                        result.add(Severity.HIGH, "AST_RS_CRED_READ", filepath,
                                   node.start_point.row + 1,
                                   f"{fn_name}() reading credential path: {s}",
                                   node.text.decode(errors="replace")[:150],
                                   _ast_path(node))
    ...
    if not is_test and any(m in full_text for m in RS_HTTP_EXFIL_METHODS):
        for s in _collect_string_values(node):
            if _SCHEME_RE.match(s) and not _is_url_allowed(s):
                result.add(Severity.HIGH, "AST_RS_HTTP_EXFIL", filepath,
                           node.start_point.row + 1,
...
def _scan_rust_build_rs(filepath: str, source: bytes, result: ScanResult):
    ...
    if "env::var" in body:
        for sensitive in ("AWS_", "GITHUB_TOKEN", "SECRET", "PASSWORD",
                          "API_KEY", "PRIVATE_KEY", "TOKEN", "CREDENTIALS"):
    ...
    for url_m in re.finditer(r"https?://([^/:\s\"'`]+)", body):
        if not _is_url_allowed(url_m.group(1)):
            ...
            break
    if suspicious:
        result.add(Severity.HIGH, "RS_BUILD_RS_SUSPICIOUS", filepath, 1,
...
def _scan_single_package_json(filepath: str, result: ScanResult):
    ...
    for hook_name, command in scripts.items():
        ...
        if matches:
            result.add(Severity.HIGH, "PKG_JSON_LIFECYCLE_SCRIPT",
                       filepath, 0,
            ...
            result.add(Severity.LOW, "PKG_JSON_LIFECYCLE_HOOK",
                       filepath, 0,
...
def scan_file_ast(filepath: str, source: bytes, result: ScanResult):
    ...
    def walk(node):
        ...
        if (is_setup_py and node.type == "expression_statement"
...
def score_function_anomaly(node, filepath: str, result: ScanResult):
    ...
    if len(name) <= 2 and len(lines) > 20:
        result.add(Severity.MEDIUM, "AST_SUSPICIOUS_FUNC", filepath,
                   node.start_point.row + 1,
                   f"Function '{name}' has very short name but {len(lines)} lines",
                   body[:150], _ast_path(node))

    if lines:
        avg_len = sum(len(l) for l in lines) / len(lines)
        if avg_len > 200 and len(lines) > 3:
            result.add(Severity.MEDIUM, "AST_DENSE_FUNCTION", filepath,
                       node.start_point.row + 1,
    ...
    if len(body_bytes) > 500:
        ...
        if ent > 5.5:
            result.add(Severity.LOW, "AST_HIGH_ENTROPY_FUNC", filepath,
                       node.start_point.row + 1,
                       f"Function '{name}' has high source entropy "
                       f"({ent:.2f} bits/byte)",
                       body[:150], _ast_path(node))
...
def scan_diff(base_ref: str, repo_root: str, result: ScanResult):
    ...
    print(f"  Layer 1: Extracted {len(changes)} structural changes",
          file=sys.stderr)
    for ch in changes:
    ...
    for filepath in file_additions:
        ...
        for finding in fr.findings:
            if finding.line in added or any(
                abs(finding.line - a) <= 2 for a in added
            ):
    ...
    for fp in file_additions:
        ...
        if fp.endswith("setup.py"):
            result.add(Severity.MEDIUM, "SETUP_PY_IN_DIFF", fp, 0,
...
def main():
    ...
    dp.add_argument("--fail-on", default="HIGH",
                    choices=["CRITICAL", "HIGH", "MEDIUM", "LOW"])
