...
SKIP_DIRS = {
    ...
    "supply-chain-security-scan", "security",
}
...
CREDENTIAL_PATHS = [
    ...
    r"~/\.gnupg/", r"~/\.config/gcloud/", r"~/\.azure/",
    r"~/\.npmrc", r"~/\.vault-token", r"~/\.netrc",
    r"~/\.gitconfig", r"~/\.git-credentials",
    r"/etc/ssl/private", r"/etc/kubernetes/",
    r"\.bitcoin/", r"\.ethereum/", r"\.solana/",
    r"\.bash_history", r"\.zsh_history",
    r"\.pgpass", r"\.my\.cnf", r"\.mongorc",
]
...
OBFUSCATION_PATTERNS = [
    (re.compile(r"exec\s*\(\s*base64\.b64decode\s*\("), "CRITICAL",
     "exec(base64.b64decode(...)) — classic obfuscated payload execution"),
    (re.compile(r"eval\s*\(\s*base64\.b64decode\s*\("), "CRITICAL",
     "eval(base64.b64decode(...)) — obfuscated payload execution"),
    (re.compile(r"exec\s*\(\s*compile\s*\(\s*base64"), "CRITICAL",
     "exec(compile(base64...)) — compiled obfuscated payload"),
    (re.compile(r"base64\.b64decode\s*\(\s*base64\.b64decode"), "CRITICAL",
     "Double base64 decoding — multi-layer obfuscation"),
    (re.compile(r"exec\s*\(\s*zlib\.decompress\s*\("), "HIGH",
     "exec(zlib.decompress(...)) — compressed payload execution"),
    (re.compile(r"exec\s*\(\s*gzip\.decompress\s*\("), "HIGH",
     "exec(gzip.decompress(...)) — compressed payload execution"),
    (re.compile(r"marshal\.loads\s*\("), "HIGH",
     "marshal.loads() — bytecode deserialization, potential code injection"),
    (re.compile(r"types\.CodeType\s*\("), "HIGH",
     "types.CodeType() construction — dynamic bytecode creation"),
    (re.compile(r"exec\s*\(\s*['\"]\\x[0-9a-fA-F]"), "HIGH",
     "exec() with hex-encoded string"),
    (re.compile(r"exec\s*\(\s*bytes\.fromhex\s*\("), "HIGH",
     "exec(bytes.fromhex(...)) — hex-encoded payload"),
    (re.compile(r"exec\s*\(\s*codecs\.decode\s*\("), "HIGH",
     "exec(codecs.decode(...)) — encoded payload execution"),
    (re.compile(r"__import__\s*\(\s*['\"]os['\"]\s*\)\s*\.\s*system"), "HIGH",
     "Dynamic import + os.system — evasion pattern"),
    (re.compile(r"getattr\s*\(\s*__import__"), "MEDIUM",
     "getattr(__import__(...)) — dynamic import evasion"),
]

EXFILTRATION_PATTERNS = [
    (re.compile(r"requests\.(post|put)\s*\(\s*['\"]https?://(?!localhost|127\.0\.0\.1)"), "HIGH",
     "HTTP POST/PUT to external URL — potential exfiltration"),
    (re.compile(r"urllib\.request\.urlopen\s*\(\s*urllib\.request\.Request\s*\([^)]*method\s*=\s*['\"]POST"), "HIGH",
     "urllib POST to external URL"),
    (re.compile(r"subprocess\.\w+\s*\(\s*\[?\s*['\"]curl['\"].*-X\s*POST"), "HIGH",
     "subprocess curl POST — potential exfiltration"),
    (re.compile(r"subprocess\.\w+\s*\(\s*\[?\s*['\"]wget['\"].*--post"), "HIGH",
     "subprocess wget POST"),
    (re.compile(r"socket\.socket\s*\(.*\)\s*.*\.connect\s*\(\s*\(\s*['\"](?!127\.|localhost)"), "MEDIUM",
...
SUSPECT_SETUP_PATTERNS = [
    (re.compile(r"class\s+\w*(Install|PostInstall|Develop)\w*\s*\("), "MEDIUM",
     "Custom install command class — may run arbitrary code on pip install"),
    (re.compile(r"cmdclass\s*=\s*\{"), "LOW",
...
def has_elf_pe_macho_header(path: Path) -> Optional[str]:
        ...
        for fname in filenames:
            ...
            for lineno, line in enumerate(lines, 1):
                for pattern, severity, desc in OBFUSCATION_PATTERNS:
                    if pattern.search(line):
                        result.add(severity, "OBFUSCATION", str(fpath),
                                   lineno, desc, line.strip())

                for pattern, severity, desc in EXFILTRATION_PATTERNS:
                    if pattern.search(line):
                        result.add(severity, "EXFILTRATION", str(fpath),
                                   lineno, desc, line.strip())

                for pattern, severity, desc in CREDENTIAL_PATTERNS:
                    if pattern.search(line):
                        is_test = any(t in str(fpath).lower()
                                      for t in ("test", "spec", "mock", "fixture", "example"))
                        actual_sev = "LOW" if is_test else severity
                        result.add(actual_sev, "CREDENTIAL_ACCESS", str(fpath),
                                   lineno, desc, line.strip())
            ...
            if suffix in (".py", ".pyw"):
                for pattern, severity, desc in SUSPECT_SETUP_PATTERNS:
                    if "setup" in fname.lower():
                        for lineno, line in enumerate(lines, 1):
                            if pattern.search(line):
                                result.add(severity, "INSTALL_HOOK", str(fpath),
                                           lineno, desc, line.strip())
            ...
            for match in b64_matches:
                if len(match) > 500:
                    for lineno, line in enumerate(lines, 1):
                        if match[:80] in line:
                            result.add("HIGH", "LONG_BASE64", str(fpath), lineno,
                                       f"Very long base64 string ({len(match)} chars) — "
                                       "possible encoded payload", match[:120] + "...")
                            break
...
def scan_binary_anomalies(root: Path, result: ScanResult):
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        in_build = any(p in str(rel_dir).lower()
                       for p in ("build", "dist", "target", "release", "debug",
        ...
        for fname in filenames:
            ...
            if suffix in BINARY_EXTENSIONS and not in_build:
                ...
                if exe_type:
                    result.add("HIGH", "BINARY_IN_SOURCE", str(fpath), 0,
                               f"Compiled {exe_type} binary in source tree — "
                               "verify this is intentional")
                else:
                    result.add("MEDIUM", "BINARY_IN_SOURCE", str(fpath), 0,
            ...
            if suffix == ".pyc":
                ...
                if not py_source.exists() and not parent_py.with_suffix(".py").exists():
                    result.add("HIGH", "ORPHAN_PYC", str(fpath), 0,
            ...
            if (not in_build
                ...
                try:
                    ...
                    if size > 1024:
                        ...
                        ent = entropy(data)
                        if ent > 7.5:
                            result.add("MEDIUM", "HIGH_ENTROPY", str(fpath), 0,
