...
class ImageCalibrationEvidenceTests(unittest.TestCase):
    ...
    def test_calibration_runs_profile_contract_and_full_calibration_only(self):
        normalized = {
        ...
        self.assertIn("./profiles/multimodal-routing", run.call_args_list[1].args[0])
        self.assertTrue(
            any("calibration" in argument for argument in run.call_args_list[2].args[0])
        )
        self.assertTrue(
            all(
                "REQUIRE_OMNI_TESTS" not in call.kwargs.get("env", {})
