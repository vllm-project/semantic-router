...
class PythonPublisherContractTests(unittest.TestCase):
    ...
    def test_python_publication_waits_for_the_published_decision_image(self) -> None:
        publisher = self.workflows["release.yml"].jobs["decision_pypi"]
        ...
        self.assertEqual(
            rocm_step["if"],
            "github.event_name == 'push' && steps.decision_mode.outputs.enabled == 'true'",
        )
        ...
        self.assertEqual(
            release.jobs["decision_pypi"]["with"]["decision_rocm_qualification"],
            "${{ needs.validate.outputs.decision_rocm_qualification }}",
        )

    def test_stable_release_defaults_to_the_existing_package_and_image_path(
        self,
    ) -> None:
        release = self.workflows["release.yml"]
        normal = release.jobs["pypi"]
        decision = release.jobs["decision_pypi"]
        self.assertEqual(needs(normal), {"validate", "gate"})
        self.assertFalse(normal["with"].get("decision_runtime", False))
        mode = "needs.validate.outputs.decision_runtime_release"
        self.assertEqual(
            release.jobs["validate"]["outputs"]["decision_runtime_release"],
            "${{ steps.decision_mode.outputs.enabled }}",
        )
        self.assertIn(
            "DECISION_RUNTIME_RELEASE", str(release.jobs["validate"]["steps"])
        )
        self.assertIn(f"{mode} != 'true'", normal["if"])
        self.assertIn(f"{mode} == 'true'", decision["if"])
        self.assertEqual(needs(decision), {"validate", "gate", "docker"})
        self.assertTrue(decision["with"]["decision_runtime"])
        self.assertEqual(
            release.jobs["ci"]["with"]["decision-runtime-images"],
            "${{ needs.validate.outputs.decision_runtime_release == 'true' }}",
        )
        notes = release.jobs["release-notes"]
        self.assertIn("needs.pypi.result == 'success'", notes["if"])
        self.assertIn("needs.decision_pypi.result == 'success'", notes["if"])
        python_downloads = [
            step
        ...
        self.assertEqual(
            [(step["if"], step["with"]["name"]) for step in python_downloads],
            [
                (f"{mode} != 'true'", "vllm-sr-dist"),
                (f"{mode} == 'true'", "vllm-sr-bound-dist"),
            ],
        )
        normal_plan = make_plan([], source_sha="a" * 40, profile="release")
        decision_plan = make_plan(
            [], source_sha="a" * 40, profile="release", decision_runtime_images=True
        )
        self.assertNotIn("decision-runtime-cpu", normal_plan["publish_images"])
        self.assertEqual(
            sorted([*normal_plan["publish_images"], "decision-runtime-cpu"]),
            sorted(decision_plan["publish_images"]),
        )
