...
# Configuration
KIND_CLUSTER_NAME ?= semantic-router-$(shell python3 -c 'import hashlib, os; print(hashlib.sha256(os.getcwd().encode()).hexdigest()[:10])')
KIND_RUN_DIR = $(CURDIR)/.agent-harness/kube/$(KIND_CLUSTER_NAME)
KIND_CONFIG_FILE ?= $(KIND_RUN_DIR)/kind-config.yaml
KIND_KUBECONFIG ?= $(KIND_RUN_DIR)/kubeconfig.yaml
KIND_STORAGE_DIR ?= $(or $(TMPDIR),/tmp)/kind-semantic-router-$(KIND_CLUSTER_NAME)
KIND_HTTP_PORT ?= 0
...
create-cluster: ## Create a kind cluster (defaults to this worktree; override KIND_CLUSTER_NAME for another)
	@echo "$(BLUE)[INFO]$(NC) Creating kind cluster: $(KIND_CLUSTER_NAME)"
	@if kind get clusters | grep -qx "$(KIND_CLUSTER_NAME)"; then \
		echo "Cluster already exists; choose another KIND_CLUSTER_NAME or use its explicit context" >&2; \
		exit 1; \
	fi
	@mkdir -p "$(KIND_RUN_DIR)" "$(KIND_STORAGE_DIR)"
	@if [ ! -f "$(KIND_CONFIG_FILE)" ]; then \
		OUTPUT_FILE="$(KIND_CONFIG_FILE)" KIND_STORAGE_DIR="$(KIND_STORAGE_DIR)" KIND_HTTP_PORT="$(KIND_HTTP_PORT)" \
		bash tools/kind/generate-kind-config.sh; \
	fi
	@kind create cluster --name "$(KIND_CLUSTER_NAME)" --config "$(KIND_CONFIG_FILE)" --kubeconfig "$(KIND_KUBECONFIG)"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) wait --for=condition=Ready nodes --all --timeout=300s
	@echo "$(GREEN)[SUCCESS]$(NC) Cluster $(KIND_CLUSTER_NAME) is ready"

# Delete kind cluster
delete-cluster: ## Delete the kind cluster
	@echo "$(BLUE)[INFO]$(NC) Deleting kind cluster: $(KIND_CLUSTER_NAME)"
	@if kind get clusters | grep -q "^$(KIND_CLUSTER_NAME)$$"; then \
		kind delete cluster --name "$(KIND_CLUSTER_NAME)" --kubeconfig "$(KIND_KUBECONFIG)"; \
...
cluster-info: ## Show cluster information and resource usage
	...
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) cluster-info || echo "$(RED)[ERROR]$(NC) Cluster not accessible"
	@echo "$(BLUE)[INFO]$(NC) Node information:"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get nodes -o wide || echo "$(RED)[ERROR]$(NC) Cannot get nodes"
	@echo "$(BLUE)[INFO]$(NC) Resource usage:"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) describe nodes | grep -A 10 "Allocated resources:" || echo "$(YELLOW)[WARNING]$(NC) Cannot get resource info"
...
deploy: ## Deploy semantic-router to the cluster
	...
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) apply -k deploy/kubernetes/
	@echo "$(BLUE)[INFO]$(NC) Waiting for namespace to be ready..."
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) wait --for=condition=Ready namespace/$(KUBE_NAMESPACE) --timeout=60s || true
	@echo "$(BLUE)[INFO]$(NC) Waiting for deployment to be ready..."
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) wait --for=condition=Available deployment/semantic-router -n $(KUBE_NAMESPACE) --timeout=600s
	@echo "$(GREEN)[SUCCESS]$(NC) Deployment completed successfully!"
	@echo "$(BLUE)[INFO]$(NC) Deployment status:"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get pods -n $(KUBE_NAMESPACE) -o wide
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get services -n $(KUBE_NAMESPACE)
...
undeploy: ## Remove semantic-router from the cluster
	...
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) delete -k deploy/kubernetes/ --ignore-not-found=true
...
test-deployment: ## Test the deployment
	...
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get pvc -n $(KUBE_NAMESPACE)
	@echo "$(BLUE)[INFO]$(NC) Checking pod readiness..."
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) wait --for=condition=Ready pod -l app=semantic-router -n $(KUBE_NAMESPACE) --timeout=60s
...
port-forward-api: ## Port forward Classification API (8080)
	...
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) port-forward -n $(KUBE_NAMESPACE) svc/semantic-router 8080:8080
...
port-forward-grpc: ## Port forward gRPC API (50051)
	...
	@echo "$(YELLOW)[INFO]$(NC) Press Ctrl+C to stop port forwarding"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) port-forward -n $(KUBE_NAMESPACE) svc/semantic-router 50051:50051
...
port-forward-metrics: ## Port forward Prometheus metrics (9190)
	...
	@echo "$(YELLOW)[INFO]$(NC) Press Ctrl+C to stop port forwarding"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) port-forward -n $(KUBE_NAMESPACE) svc/semantic-router-metrics 9190:9190

# Show logs
logs: ## Show semantic-router logs
	@echo "$(BLUE)[INFO]$(NC) Showing semantic-router logs"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) logs -n $(KUBE_NAMESPACE) -l app=semantic-router -f

# Show deployment status
status: ## Show deployment status
	@echo "$(BLUE)[INFO]$(NC) Semantic Router deployment status"
	@echo "$(BLUE)[INFO]$(NC) Pods:"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get pods -n $(KUBE_NAMESPACE) -o wide || echo "$(RED)[ERROR]$(NC) Cannot get pods"
	@echo "$(BLUE)[INFO]$(NC) Services:"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get services -n $(KUBE_NAMESPACE) || echo "$(RED)[ERROR]$(NC) Cannot get services"
	@echo "$(BLUE)[INFO]$(NC) PVC:"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get pvc -n $(KUBE_NAMESPACE) || echo "$(RED)[ERROR]$(NC) Cannot get PVC"
...
kube-deploy-llm-katan: ## Deploy llm-katan to cluster (OVERLAY=gpt35|claude, default: gpt35)
	...
	@if ! kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) cluster-info &>/dev/null; then \
		echo "$(RED)[ERROR]$(NC) Kubernetes cluster is not accessible"; \
		echo "$(BLUE)[INFO]$(NC) Run 'make create-cluster' first"; \
		exit 1; \
	fi
	@echo "$(BLUE)[INFO]$(NC) Applying Kubernetes manifests..."
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) apply -k $(LLM_KATAN_BASE_PATH)/overlays/$(LLM_KATAN_OVERLAY)
	@echo "$(BLUE)[INFO]$(NC) Waiting for namespace to be ready..."
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) wait --for=condition=Ready namespace/$(LLM_KATAN_NAMESPACE) --timeout=60s || true
	@echo "$(BLUE)[INFO]$(NC) Waiting for deployment to be ready..."
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) wait --for=condition=Available deployment/llm-katan-$(LLM_KATAN_OVERLAY) \
	...
	@echo "$(BLUE)[INFO]$(NC) Deployment status:"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get pods -n $(LLM_KATAN_NAMESPACE) -l app=llm-katan-$(LLM_KATAN_OVERLAY) -o wide
...
kube-deploy-llm-katan-multi: ## Deploy both gpt35 and claude overlays
	...
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get pods -n $(LLM_KATAN_NAMESPACE) -o wide
...
kube-undeploy-llm-katan: ## Remove llm-katan from cluster (OVERLAY=gpt35|claude|all, default: gpt35)
	...
	@if [ "$(LLM_KATAN_OVERLAY)" = "all" ]; then \
		...
		kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) delete -k $(LLM_KATAN_BASE_PATH)/overlays/gpt35 --ignore-not-found=true; \
		kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) delete -k $(LLM_KATAN_BASE_PATH)/overlays/claude --ignore-not-found=true; \
	else \
		kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) delete -k $(LLM_KATAN_BASE_PATH)/overlays/$(LLM_KATAN_OVERLAY) --ignore-not-found=true; \
	fi
...
kube-status-llm-katan: ## Show llm-katan deployment status
	...
	@echo ""
	@echo "$(BLUE)[INFO]$(NC) Pods:"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get pods -n $(LLM_KATAN_NAMESPACE) -o wide || echo "$(RED)[ERROR]$(NC) Cannot get pods"
	@echo ""
	@echo "$(BLUE)[INFO]$(NC) Services:"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get services -n $(LLM_KATAN_NAMESPACE) || echo "$(RED)[ERROR]$(NC) Cannot get services"
	@echo ""
	@echo "$(BLUE)[INFO]$(NC) PVCs:"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get pvc -n $(LLM_KATAN_NAMESPACE) || echo "$(RED)[ERROR]$(NC) Cannot get PVCs"
	@echo ""
	@echo "$(BLUE)[INFO]$(NC) Deployments:"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get deployments -n $(LLM_KATAN_NAMESPACE) || echo "$(RED)[ERROR]$(NC) Cannot get deployments"
...
kube-logs-llm-katan: ## Show llm-katan logs (OVERLAY=gpt35|claude, default: gpt35)
	...
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) logs -n $(LLM_KATAN_NAMESPACE) -l app=llm-katan-$(LLM_KATAN_OVERLAY) -f
...
kube-port-forward-llm-katan: ## Port forward llm-katan API (OVERLAY=gpt35|claude, PORT=8000)
	...
	@echo "$(YELLOW)[INFO]$(NC) Press Ctrl+C to stop port forwarding"
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) port-forward -n $(LLM_KATAN_NAMESPACE) svc/llm-katan-$(LLM_KATAN_OVERLAY) $(PORT):8000
...
kube-test-llm-katan: ## Test llm-katan deployment (OVERLAY=gpt35|claude, default: gpt35)
	...
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) get svc -n $(LLM_KATAN_NAMESPACE) llm-katan-$(LLM_KATAN_OVERLAY)
	@echo ""
	@echo "$(BLUE)[INFO]$(NC) Checking pod readiness..."
	@kubectl --kubeconfig="$(KIND_KUBECONFIG)" --context=kind-$(KIND_CLUSTER_NAME) wait --for=condition=Ready pod -l app=llm-katan-$(LLM_KATAN_OVERLAY) \
...
.PHONY: require-kind-cluster-name
require-kind-cluster-name:
	@test -n "$(KIND_CLUSTER_NAME)" || { echo "Set KIND_CLUSTER_NAME explicitly for this operation" >&2; exit 2; }
	@printf '%s' "$(KIND_CLUSTER_NAME)" | grep -Eq '^[a-z0-9][a-z0-9-]*$$' || { echo "Invalid KIND_CLUSTER_NAME" >&2; exit 2; }

create-cluster delete-cluster cluster-info deploy undeploy load-image test-deployment test-api port-forward-api port-forward-grpc port-forward-metrics logs status setup cleanup kube-deploy-llm-katan kube-deploy-llm-katan-gpt35 kube-deploy-llm-katan-claude kube-deploy-llm-katan-multi kube-undeploy-llm-katan kube-status-llm-katan kube-logs-llm-katan kube-port-forward-llm-katan kube-test-llm-katan kube-load-llm-katan-image: | require-kind-cluster-name
