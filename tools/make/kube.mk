...
##@ Provider mocker Kubernetes

PROVIDER_MOCKER_NAMESPACE ?= default
PROVIDER_MOCKER_BASE_PATH ?= deploy/kubernetes/provider-mocker

kube-load-provider-mocker-image: docker-build-provider-mocker ## Load the unified mocker into kind
	@kind load docker-image "$(PROVIDER_MOCKER_IMAGE)" --name "$(KIND_CLUSTER_NAME)"

kube-deploy-provider-mocker: kube-load-provider-mocker-image ## Deploy the unified mocker in kind
	@kubectl apply -n "$(PROVIDER_MOCKER_NAMESPACE)" -k "$(PROVIDER_MOCKER_BASE_PATH)"
	@kubectl set image -n "$(PROVIDER_MOCKER_NAMESPACE)" deployment/provider-mocker provider-mocker="$(PROVIDER_MOCKER_IMAGE)"
	@kubectl rollout status -n "$(PROVIDER_MOCKER_NAMESPACE)" deployment/provider-mocker --timeout=120s

kube-undeploy-provider-mocker: ## Remove the unified mocker from the cluster
	@kubectl delete -n "$(PROVIDER_MOCKER_NAMESPACE)" -k "$(PROVIDER_MOCKER_BASE_PATH)" --ignore-not-found=true

kube-status-provider-mocker: ## Show provider mocker pods and service
	@kubectl get deployment,pods,service -n "$(PROVIDER_MOCKER_NAMESPACE)" -l app=provider-mocker -o wide

kube-logs-provider-mocker: ## Follow provider mocker logs
	@kubectl logs -n "$(PROVIDER_MOCKER_NAMESPACE)" deployment/provider-mocker -f

kube-port-forward-provider-mocker: ## Expose the mocker on localhost (PROVIDER_MOCKER_PORT=8000)
	@kubectl port-forward -n "$(PROVIDER_MOCKER_NAMESPACE)" service/provider-mocker $(PROVIDER_MOCKER_PORT):8000

.PHONY: kube-load-provider-mocker-image kube-deploy-provider-mocker kube-undeploy-provider-mocker \
	kube-status-provider-mocker kube-logs-provider-mocker kube-port-forward-provider-mocker
