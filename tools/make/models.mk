...
download-models: ## Download all models (full set for local dev, minimal for CI if CI_MINIMAL_MODELS=true)
	@$(LOG_TARGET)
	@mkdir -p models
...
download-models-minimal: ## Download minimal set of models for CI/unit tests
	@mkdir -p models
...
download-models-full: ## Download full set of models for local development
	@mkdir -p models
	# Pre-download tiny LLM for llm-katan (optional but speeds up first start)
	@if [ ! -f "models/Qwen/Qwen3-0.6B/.downloaded" ] || [ ! -d "models/Qwen/Qwen3-0.6B" ]; then \
		hf download Qwen/Qwen3-0.6B --local-dir models/Qwen/Qwen3-0.6B && printf '%s\n' "$$\(date -u +%Y-%m-%dT%H:%M:%SZ)" > models/Qwen/Qwen3-0.6B/.downloaded; \
