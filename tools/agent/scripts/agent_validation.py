...
def validate_routing_fixtures(errors: list[str]) -> None:
    ...
    if not fixtures_path.exists():
        errors.append("Missing routing fixtures file: tools/agent/routing-fixtures.yaml")
        return
...
def validate_codex_bridge(errors: list[str]) -> None:
    bridge_path = REPO_ROOT / ".agents" / "skills" / "vllm-semantic-router-harness" / "SKILL.md"
