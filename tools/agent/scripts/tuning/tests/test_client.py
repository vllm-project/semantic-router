...
def test_run_probes_reports_unresolved_before_calibration_adapter():
    from tuning.scenarios.calibration import CalibrationScenario

    payload = {"decision_error": "decision unresolved"}
    client = RouterClient()
    with mock.patch.object(client, "eval_probe", return_value=payload):
        results = client.run_probes(
            [{"id": "p1", "query": "hello", "expected_decision": "keep_7b"}],
            result_adapter=CalibrationScenario().adapt_result,
        )
    assert results[0]["actual"] == "UNRESOLVED"
    assert results[0]["correct"] is False
