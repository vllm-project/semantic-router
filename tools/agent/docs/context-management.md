...
This document defines how the harness exposes the minimum useful context for a task instead of forcing agents to read the entire `tools/agent/docs/` tree.

## Why This Exists

- `AGENTS.md` should stay short and navigational.
- `tools/agent/docs/*` should stay canonical, but the set is now large enough that agents need a task-first read path.
...
- Human-readable policy: this document and the linked harness docs under `tools/agent/docs/`
