...
- Real AMD deployment playbook: [website/docs/installation/amd-rocm.md](../../../website/docs/installation/amd-rocm.md)
- Real AMD routing profile: [config/recipes/balance/config.yaml](../../../config/recipes/balance/config.yaml)
- Expected behavior:
...
- Use [config/recipes/balance/config.yaml](../../../config/recipes/balance/config.yaml) when you need a single real AMD backend with a multi-alias routing profile.
- Use [website/docs/installation/amd-rocm.md](../../../website/docs/installation/amd-rocm.md) when you need the full backend deployment flow, Docker network setup, model container examples, and dashboard-first vs YAML-first setup guidance.
