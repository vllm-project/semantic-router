...
- [x] EP-010 backend-independent Eval and CLI selection by request-facing model,
      including selected recipe, algorithm, and plugin metadata
- [x] EP-011 maintained multi-recipe calibration manifest covering objective and
      orchestration branches without real backend completions
- [x] EP-012 Router image runtime-sync packaging so management API deploy and
      rollback regenerate the effective config under local runtime overrides
- [x] EP-013 transactional management deploys that preserve the submitted
      canonical document and restore both source and runtime config on sync failure
- [x] EP-014 recipe-aware CLI validation and inventory summaries for signals,
      projections, decisions, algorithms, and plugins across every profile
- [x] EP-015 shared knowledge-base registration with recipe-local dependency
      selection, so each classifier graph initializes only the KBs it references
- [x] EP-016 focused recipe lifecycle API with validate-only operations, strong
      ETags, reference protection, immutable backups, and runtime-sync rollback
- [x] EP-017 runtime document identity and activation-aware config management,
      closing the persisted-versus-active read-after-write gap
- [x] EP-018 100-probe multilingual and adversarial Eval matrix, including
      conversation/tool shapes, collision negatives, and bounded long inputs

## Next Action

Land PR #2741 after the full local stack, backend-independent recipe calibration,
repository gates, and CI pass. Track multi-profile DSL syntax separately if that
language extension is prioritized.
