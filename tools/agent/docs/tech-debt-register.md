...
- `tools/agent/docs/tech-debt/td-*.md`
  - current open debt entries only

## Policy

- When current code materially diverges from the desired architecture or harness rules and the gap is not fully closed in the same change, add or update the matching detailed entry under `tools/agent/docs/tech-debt/`.
...
- When a debt item is retired:
  ...
  - remove the entry from `tools/agent/docs/tech-debt/README.md`
