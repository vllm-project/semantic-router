...
def test_extract_mock_header_delay():
    # ChatRequest parsing
    req1 = ChatRequest(model="test", messages=[{"role": "user", "content": "hello __mock_header_delay_2.5s__ world"}])
    assert extract_mock_header_delay(req1) == 2.5

    req2 = ChatRequest(model="test", messages=[{"role": "user", "content": "regular prompt"}])
    assert extract_mock_header_delay(req2) == 0.0

    # Response API dict parsing
    assert response_extract_mock_header_delay({"input": "__mock_header_delay_1s__ test"}) == 1.0
...
def test_extract_mock_frame_stall():
    ...
    req1 = ChatRequest(model="test", messages=[{"role": "user", "content": "hello __mock_frame_stall_5s__ world"}])
    assert extract_mock_frame_stall(req1) == 5.0

    # Default stall duration for bare markers
    req2 = ChatRequest(model="test", messages=[{"role": "user", "content": "hello __mock_frame_stall__ world"}])
    assert extract_mock_frame_stall(req2) == 15.0

    # Incomplete stream does NOT trigger frame stall (decoupled)
    req3 = ChatRequest(model="test", messages=[{"role": "user", "content": "hello __mock_incomplete_stream__ world"}])
    assert extract_mock_frame_stall(req3) == 0.0

    req4 = ChatRequest(model="test", messages=[{"role": "user", "content": "normal stream"}])
    assert extract_mock_frame_stall(req4) == 0.0

    # Response API dict parsing
    assert response_extract_mock_frame_stall({"input": "__mock_frame_stall_3s__ test"}) == 3.0
    assert response_extract_mock_frame_stall({"input": "__mock_frame_stall__ test"}) == 15.0
    assert response_extract_mock_frame_stall({"input": "__mock_incomplete_stream__ test"}) == 0.0
...
async def test_chat_completions_controllable_header_delay():
    body = {
        "model": "test-model",
        "messages": [{"role": "user", "content": "__mock_header_delay_0.2s__ test prompt"}],
    }
    start = time.time()
...
async def test_chat_completions_streaming_frame_stall():
    body = {
        "model": "test-model",
        "stream": True,
        "messages": [{"role": "user", "content": "__mock_frame_stall_0.1s__ test prompt"}],
    }
    start = time.time()
    async with httpx.AsyncClient(
        transport=httpx.ASGITransport(app=app), base_url="http://simulator"
...
async def test_chat_completions_streaming_frame_stall_incomplete():
    body = {
        "model": "test-model",
        "stream": True,
        "messages": [{"role": "user", "content": "__mock_incomplete_stream__ __mock_frame_stall_0.1s__ test prompt"}],
    }
    start = time.time()
    async with httpx.AsyncClient(
        transport=httpx.ASGITransport(app=app), base_url="http://simulator"
