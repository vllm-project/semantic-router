...
import httpx
import pytest
from app import app, chat_completions
from fastapi import FastAPI
from provider_boundary import RequestStore
from shadow_control import ShadowControl, router
...
def controlled_app() -> FastAPI:
    ...
    instance.state.shadow_control = ShadowControl()
    instance.include_router(router)
    instance.add_api_route("/v1/chat/completions", chat_completions, methods=["POST"])
    return instance
...
async def test_default_simulator_does_not_enable_shadow_control() -> None:
    async with httpx.AsyncClient(
        ...
        assert primary.status_code == HTTPStatus.OK
        assert (
            '"mock":"mock-vllm"' in primary.json()["choices"][0]["message"]["content"]
        )
