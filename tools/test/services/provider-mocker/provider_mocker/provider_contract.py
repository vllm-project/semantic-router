...
def validate_provider_request(protocol: str, body: Any) -> dict[str, Any]:
    ...
    if protocol in {"openai_chat_completions", "anthropic_messages"}:
    ...
    if protocol == "anthropic_messages":
        ...
        if not isinstance(model, str) or not model.strip():
            raise ContractViolationError("model must be a non-empty string", "model")
        ...
        if stops is not None and (
            ...
            raise ContractViolationError(
                "stop_sequences must be an array of strings", "stop_sequences"
            )
    return body
