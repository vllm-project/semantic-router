...
```bash
./tools/dev/kind/generate-kind-config.sh
kind create cluster --config tools/dev/kind/kind-config.yaml
...
writes the ignored `tools/dev/kind/kind-config.yaml` from the checked-in template.
