...
function usage {
  ...
  echo "Example 0: tools/dev/local-up-router.sh -h  (this 'help' usage description)"
  echo "Example 1: tools/dev/local-up-router.sh -o bin (run from make build)"
  echo "Example 2: tools/dev/local-up-router.sh (build a local copy of the source)"
  echo "Example 3: ROUTER_CONFIG=\"config/config.yaml\" \\"
  echo "           tools/dev/local-up-router.sh (build a local copy of the source with a specific config file)"
  echo ""
