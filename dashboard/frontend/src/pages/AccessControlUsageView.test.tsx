...
describe('access usage view', () => {
  it('merges access posture into the complete usage experience', () => {
    ...
    expect(markup).toContain('$12.345678 USD')
    expect(markup).toContain('Settled from actual usage')
    expect(markup).toContain('1 hour per point')
    expect(markup).toContain('Granularity')
    expect(markup).toContain('Spend by API key')
    expect(markup).toContain('API keys')
  })

  it('does not turn unauthorized statistics into zero-valued disclosure', () => {
    const markup = renderToStaticMarkup(
      createElement(AccessControlUsageView, {
        overview: {
          ...overview,
    ...
    expect(markup).not.toContain('Identities')
    expect(markup).not.toContain('Access groups')
    expect(markup).not.toContain('Budgets')
    expect(markup).toContain('API keys')
  })
})
