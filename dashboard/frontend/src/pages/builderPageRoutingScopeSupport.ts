...
export function summarizeBuilderRoutingScopes(
  ...
  const programs = ast
    ? [ast, ...(ast.recipes?.map((recipe) => recipe.program) ?? [])]
    : []
  ...
  return {
    signalCount:
      programs.length > 0 ? sum((program) => program.signals.length) : symbols?.signals.length ?? 0,
    ...
    routeCount:
      programs.length > 0 ? sum((program) => program.routes.length) : symbols?.routes.length ?? 0,
    pluginCount:
      programs.length > 0 ? sum((program) => program.plugins.length) : symbols?.plugins.length ?? 0,
