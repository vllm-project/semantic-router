...
describe('Mixture-of-Models workspace contracts', () => {
  ...
  it('uses the installed CLI catalog as the primary distribution surface without ZIP import UI', () => {
    ...
    const catalogStyles = readSource('./ConfigPageMoMCatalogPanel.module.css')
    ...
    expect(catalogStyles).toMatch(/\.modelGrid\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\)/s)
    expect(catalogStyles).not.toContain('repeat(auto-fit')
