...
import type { ASTSignalDecl, DSLFieldObject, DSLFieldScalar, DSLFieldValue } from '@/types/dsl'
import {
...
function appendYamlField(lines: string[], key: string, value: DSLFieldValue, level: number): void {
...
function appendYamlObject(lines: string[], value: DSLFieldObject, level: number): void {
  yamlObjectEntries(value).forEach(([childKey, childValue]) =>
    appendYamlField(lines, childKey, childValue, level),
  )
}

function appendYamlArrayItem(lines: string[], value: DSLFieldValue, level: number): void {
  if (
    value === null ||
    value === undefined ||
    typeof value === 'string' ||
  ...
  if (
    ...
    lines.push(`${yamlIndent(level)}- ${firstKey}: ${yamlScalar(firstValue as DSLFieldScalar)}`)
  } else {
  ...
  entries
    .slice(1)
    .forEach(([childKey, childValue]) => appendYamlField(lines, childKey, childValue, level + 1))
}

function generateGlobalOverridePreview(fields: DSLFieldObject): string {
  const lines: string[] = []
  ;(
    Object.entries(fields).filter(([, value]) => value !== undefined && value !== null) as Array<
...
const AlgorithmSchemaEditor: React.FC<{
  ...
  onUpdate: (fields: DSLFieldObject) => void
  modelFree?: boolean
}> = ({ algoType, fields, onUpdate, modelFree = false }) => {
  const schema = useMemo(() => {
    const all = getAlgorithmFieldSchema(algoType)
    if (!modelFree) return all
    ...
    return all.filter((field) => !modelFields.has(field.key))
  }, [algoType, modelFree])
