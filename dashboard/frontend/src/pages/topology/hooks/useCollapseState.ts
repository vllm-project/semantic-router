...
export function useCollapseState(): UseCollapseStateResult {
  const [collapseState, setCollapseState] = useState<CollapseState>({
    signalGroups: Object.fromEntries(SIGNAL_TYPES.map((t) => [t, false])) as Record<
      SignalType,
      boolean
    >,
    decisions: {},
  ...
  const toggleCollapse = useCallback(
    (type: 'signalGroup' | 'decision' | 'pluginChain', key: string) => {
  ...
  const collapseAll = useCallback(() => {
    setCollapseState((prev) => ({
      signalGroups: Object.fromEntries(SIGNAL_TYPES.map((t) => [t, true])) as Record<
        SignalType,
        boolean
      >,
  ...
  const isCollapsed = useCallback(
    (type: 'signalGroup' | 'decision' | 'pluginChain', key: string): boolean => {
      if (type === 'signalGroup') {
