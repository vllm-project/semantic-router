...
import {
  DENSITY_FRAME_PADDING_SCALE,
  DENSITY_HORIZONTAL_GAP_SCALE,
  DENSITY_LANE_GAP_SCALE,
  DENSITY_SPACING_SCALE,
  HORIZONTAL_GAP_BY_LAYER,
  ORDERED_LAYERS,
  getAdaptiveLayerSpacing,
  isTopologyNodeHighlighted,
  type LayerFrame,
  type LayerName,
  type LayoutOptions,
  type LayoutResult,
} from './layoutCalculatorSupport'

export type { DecisionDensityMode } from './layoutGraphBuilderSupport'
...
export function calculateFullLayout(
  ...
  const isHighlighted = (id: string) => isTopologyNodeHighlighted(id, highlightedPath)
