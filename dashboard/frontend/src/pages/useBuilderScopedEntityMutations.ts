...
export const useBuilderScopedEntityMutations = ({
  ...
  const handleDeleteEntity = useCallback(
    (kind: EntityKind, name: string, subType?: string) => {
      switch (kind) {
        case "model":
          useDSLStore.getState().deleteModel(name);
          break;
        case "signal":
          if (subType) {
  ...
  const handleUpdateModelFields = useCallback((name: string, fields: DSLFieldObject) => {
    useDSLStore.getState().mutateModel(name, fields);
  }, []);

  const handleAddModel = useCallback(
    (name: string, fields: DSLFieldObject) => {
      useDSLStore.getState().addModel(name, fields);
  ...
  const handleUpdateProjectionPartitionFields = useCallback(
    (name: string, fields: DSLFieldObject) => {
      applyRoutingMutation((source) =>
        dslMutations.updateProjectionPartition(source, name, fields),
      );
    },
    [applyRoutingMutation],
  );
  ...
  const handleUpdateProjectionScoreFields = useCallback(
    (name: string, fields: DSLFieldObject) => {
      applyRoutingMutation((source) =>
        dslMutations.updateProjection(source, "score", name, fields),
      );
    },
    [applyRoutingMutation],
  );
