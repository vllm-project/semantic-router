...
const promoteLegacyGlobalBlocks = (cfg: ConfigData) => {
  ...
  placeBlockIfMissing(['model_catalog', 'embeddings', 'bert'], cfg.bert_model)
  ...
  if (legacyGlobalModels) {
    ...
    if (embeddings) {
      ...
      placeBlockIfMissing(['model_catalog', 'embeddings', 'bert'], embeddings.bert)
    }
    ...
    for (const [key, value] of Object.entries(legacyRuntime)) {
      ...
      switch (key) {
        ...
        case 'embedding_models':
          ...
          break
        case 'bert_model':
          placeBlockIfMissing(['model_catalog', 'embeddings', 'bert'], value)
          break
