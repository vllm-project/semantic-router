      ...
      .then((THREE) => {
        ...
        const renderFrame = (staticFrame = false) => {
          ...
          nodeRings.forEach((ring, index) => {
            ...
            ;(ring.material as MeshBasicMaterial).opacity = 0.1 + activeAmount * 0.38
          })
          ...
          particles.forEach((particle, index) => {
            ...
            ;(particle.material as MeshBasicMaterial).opacity = particleActive
        ...
        disposeScene = () => {
          ...
          nodeRings.forEach((ring) => (ring.material as Material).dispose())
          particleGeometry.dispose()
          particles.forEach((particle) => (particle.material as Material).dispose())
          arcGeometry.dispose()
          arcMaterial.dispose()
...
const AuthTransitionPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const completeTransition = () => {
      ...
      isComplete = true
      if (timers.progress !== undefined) {
        window.clearInterval(timers.progress)
      }
      if (timers.completion !== undefined) {
