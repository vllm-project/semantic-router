...
const extractTextContent = (value: unknown): string => {
  ...
  return asString(record.text) || asString(record.content) || asString(record.output_text)
}
...
export const getFirstChoice = (
  choiceContents: Map<number, ChoiceAccumulator>,
): ChoiceAccumulator | undefined => choiceContents.get(0)
...
export const parseChatCompletionObject = (value: unknown): ParsedChatCompletion | null => {
  ...
  const choices = rawChoices.flatMap((value, rawIndex) => {
    ...
    const toolCalls = [
      ...extractToolCalls(delta, 'delta'),
      ...extractToolCalls(message, 'snapshot'),
    ]
