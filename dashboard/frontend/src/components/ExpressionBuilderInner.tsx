...
const ExpressionBuilderInner: React.FC<ExpressionBuilderInnerProps> = ({
  ...
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; path: NodePath } | null>(
    null,
  )
  ...
  const [insertSiblingTarget, setInsertSiblingTarget] = useState<{
    parentPath: NodePath
    index: number
  } | null>(null)
  ...
  const handleInsertSiblingPick = useCallback(
    (newNode: RuleNode) => {
      if (!insertSiblingTarget || !tree) {
        setInsertSiblingTarget(null)
        return
      }
      pushHistory(tree)
      setTree(
        insertAtPath(tree, insertSiblingTarget.parentPath, insertSiblingTarget.index, newNode),
      )
      const label = isLeaf(newNode)
        ? `${newNode.signalType}("${newNode.signalName}")`
        : (newNode as Exclude<RuleNode, { signalType: string }>).operator
      ...
      .map(
        ([type, signals]) =>
          [
            type,
            signals.filter(
              (s) => s.name.toLowerCase().includes(q) || s.signalType.toLowerCase().includes(q),
            ),
  ...
  const handleDeleteNode = useCallback(
    (path: NodePath) => {
      ...
      const label = node
        ? isLeaf(node)
          ? `${node.signalType}("${node.signalName}")`
          : node.operator
        : 'node'
      pushHistory(tree)
  ...
  const handleDropOnNode = useCallback(
    (targetPath: NodePath, dragData: DragData) => {
      if (!tree) return
  ...
  const handleAddChildPick = useCallback(
    (newNode: RuleNode) => {
      if (!addingToPath || !tree) {
  ...
  useEffect(() => {
    ...
    const { nodes: rawNodes, edges: newEdges } = treeToFlowElements(
      tree,
  ...
  const handleWrap = useCallback(
    (path: NodePath, op: 'AND' | 'OR' | 'NOT') => {
      if (!tree) return
      const node = getNodeAtPath(tree, path)
      if (!node) return
      pushHistory(tree)
      const wrapped: RuleNode =
        op === 'NOT'
  ...
  const handleChangeOp = useCallback(
    (path: NodePath, newOp: 'AND' | 'OR') => {
      if (!tree) return
      const node = getNodeAtPath(tree, path)
      if (!node || !isOperator(node)) return
      pushHistory(tree)
      setTree(
        replaceAtPath(tree, path, {
          ...node,
          operator: newOp,
  ...
  const validationIssues = useMemo(
    () => validateTree(tree, availableSignals),
    [tree, availableSignals],
  )
    ...
    >
          ...
          <div className={styles.zoomBar}>
            <div className={styles.toolbarGroup}>
              <button
                ...
                disabled={!canUndo}
                title="Undo (Ctrl+Z)"
                aria-label="Undo expression change"
              >
              ...
              <button
                ...
                disabled={!canRedo}
                title="Redo (Ctrl+Y)"
                aria-label="Redo expression change"
              >
                <ProductIcon name="redo" />
              </button>
            </div>
            ...
            {selectedPath && (
              <span className={styles.zoomHint}>
                Selected:{' '}
                {(() => {
                  const n = tree && getNodeAtPath(tree, selectedPath)
                  return n ? (isLeaf(n) ? `${n.signalType}("${n.signalName}")` : n.operator) : '—'
                })()}
              </span>
            )}
            ...
            <button
              ...
              aria-label={
                maximized
                  ? 'Exit fullscreen expression builder'
                  : 'Open fullscreen expression builder'
              }
            >
              <ProductIcon name="fullscreen" />
            </button>
          </div>
      ...
      <div className={styles.result}>
        ...
        {tree ? (
          ...
          <span className={styles.resultPlaceholder}>
            No condition — route matches all requests
          </span>
        )}
      </div>
      ...
      {validationIssues.length > 0 && (
        <div>
          {validationIssues.map((warning, index) => (
            <div key={`${warning}-${index}`} className={styles.validationWarn}>
              <ProductIcon name="alert" aria-hidden="true" /> {warning}
            </div>
          ))}
        </div>
      )}
      {tree && validationIssues.length === 0 && (
        <div className={styles.validationOk}>
          <ProductIcon name="check" aria-hidden="true" /> All referenced signals exist
        </div>
      )}
