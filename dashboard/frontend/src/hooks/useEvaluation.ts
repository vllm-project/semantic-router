...
export function useTasks(autoRefresh = false, refreshInterval = 5000) {
  ...
  const fetchTasks = useCallback(
    async (options: EvaluationLoadOptions = {}) => {
...
export function useTask(taskId: string | null, autoRefresh = false, refreshInterval = 1000) {
  ...
  const request = useMemo(
    () => (taskId ? createEvaluationRequest((signal) => api.getTask(taskId, signal)) : null),
    [taskId],
  )
...
export function useResults(taskId: string | null) {
  ...
  const request = useMemo(
    () => (taskId ? createEvaluationRequest((signal) => api.getResults(taskId, signal)) : null),
    [taskId],
  )
...
export function useDatasets() {
  ...
  const request = useMemo(() => createEvaluationRequest((signal) => api.getDatasets(signal)), [])
...
export function useTaskMutations() {
  ...
  const createTask = useCallback(
    async (request: CreateTaskRequest): Promise<EvaluationTask | null> => {
      setLoading(true)
      setError(null)
      try {
...
export function useTaskCreationForm() {
  ...
  const [dimensions, setDimensions] = useState<EvaluationDimension[]>(
    getDefaultDimensionsForLevel('router'),
  )
  ...
  const getConfig = useCallback((): CreateTaskRequest => {
    ...
    const filteredSelectedDatasets = filterSelectedDatasetsByDimensions(
      selectedDatasets,
