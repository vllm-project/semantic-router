...
export function useMLPipelineWizard() {
  ...
  const canAdvanceToStep = useCallback(
    (step: number): boolean => {
      switch (step) {
        case 1:
          ...
          return (
            benchmarkSkipped ||
            (benchmarkProgress.completed && benchmarkProgress.job?.status === 'completed')
          )
        case 2:
    ...
    [
      benchmarkSkipped,
      benchmarkProgress.completed,
      benchmarkProgress.job?.status,
      trainProgress.completed,
      trainProgress.job?.status,
    ],
  )
