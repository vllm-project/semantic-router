...
import { containsInlineImageData, jsonByteLength } from './persistenceSafety'
...
export const DEFAULT_MAX_QUEUE_STORAGE_BYTES = 1024 * 1024
    ...
    Object.entries(queues)
      .map(
        ([conversationId, tasks]) =>
          [conversationId, tasks.slice(-maxTasksPerConversation)] as const,
      )
...
export const normalizePlaygroundQueues = (
  ...
  const queues = Object.entries(value).reduce<PlaygroundTaskQueues>(
    (acc, [conversationId, tasks]) => {
...
export const preparePlaygroundQueuesForPersistence = (
  queues: PlaygroundTaskQueues,
  maxBytes = DEFAULT_MAX_QUEUE_STORAGE_BYTES,
): PlaygroundTaskQueues => {
  const persisted: PlaygroundTaskQueues = {}
  for (const [conversationId, tasks] of Object.entries(queues)) {
    for (const task of tasks) {
      if (containsInlineImageData(task)) continue
      const candidate = {
        ...persisted,
        [conversationId]: [...(persisted[conversationId] ?? []), task],
      }
