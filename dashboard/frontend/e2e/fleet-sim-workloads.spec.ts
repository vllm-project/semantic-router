...
test('opens uploaded trace preview in a dialog from the trace table', async ({ page }) => {
  ...
  const viewButton = traceRow.getByRole('button', { name: 'View' })
  const overflowBeforeDialog = await page.evaluate(() => document.body.style.overflow)
  await viewButton.click()

  const dialog = page.getByRole('dialog')
  const headerCloseButton = dialog.getByRole('button', { name: 'Close trace preview' })
