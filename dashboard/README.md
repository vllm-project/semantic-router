...
- Dashboard auth accepts the exact-HS256 session JWT from `Authorization: Bearer <token>` or the HttpOnly `vsr_session` cookie for protected `/api/*` and `/embedded/*` requests. Every token must have an expiration and a non-empty server-owned session ID.
- Login, bootstrap, and password change set an HttpOnly, `SameSite=Lax` session cookie. Logout revokes its server-side session and clears the cookie. Legacy embedded transports may also carry `authToken=<token>`; replacing script-visible storage and reusable URL credentials is tracked in [#2465](https://github.com/vllm-project/semantic-router/issues/2465).
