...
func srBenchRouteContracts() []auth.RouteContract {
	...
	contracts := []auth.RouteContract{
		...
		auth.Route(base+"/dataset-preparations",
			auth.ReadPolicy(http.MethodGet, auth.PermEvalRead, auth.SensitivitySensitive, auth.ResourceOwnerEvaluation),
			auth.MutationPolicy(http.MethodPost, auth.PermEvalWrite, "sr_bench.dataset_preparation.create", auth.SensitivitySensitive, auth.ResourceOwnerEvaluation, maxSRBenchBodyBytes),
		),
		read("/dataset-preparations/options"), read("/dataset-preparations/{id}"),
