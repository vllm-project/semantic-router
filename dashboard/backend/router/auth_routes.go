...
func setupAuthRoutes(
	mux *http.ServeMux,
	cfg *config.Config,
	modelUsers *accesscontrol.Service,
) *auth.Service {
	...
	if modelUsers != nil {
		authSvc.ConfigureModelUsers(modelUsers)
	}
	authSvc.SetAllowOpenBootstrap(cfg.AllowOpenBootstrap)
	authSvc.SetSetupMode(cfg.SetupMode)
	authSvc.ConfigureInvitations(auth.SMTPInvitationMailer{
	...
	if err := authSvc.ReconcileModelUsers(context.Background()); err != nil {
		log.Printf("failed to reconcile Dashboard users with model access: %v", err)
	}
