...
func (m *PolicyMux) HandlePolicy(contract RouteContract, handler http.Handler) {
...
func (m *PolicyMux) HandlePolicyFunc(contract RouteContract, handler http.HandlerFunc) {
	m.HandlePolicy(contract, handler)
}

// Plain registration is deliberately unavailable on the production mux. It
// satisfies the standard registrar interface used by isolated route tests,
// while making a newly added production route without policy fail at startup.
