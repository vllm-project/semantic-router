...
func builtinSuites() []CatalogSuite {
	return []CatalogSuite{
		...
		{ID: "router-learning-core", Name: "Router Learning policy comparison", Description: "A deterministic sequential replay comparing a static base policy, the current routing_sampling score equation, and a simple Beta-Bernoulli policy over paired trial seeds. It is diagnostic and not a production causal claim.", Executors: map[Mode]string{ModeReplay: routerLearningReplayExecutorID}, TrackIDs: []TrackID{"joint"}, Modes: []Mode{ModeReplay}, EvidenceLevel: "E0", CaseCount: 12, Revision: "router-learning-core-v1", Tags: []string{"router-learning", "paired-seeds", "deterministic", "diagnostic-only"}, Methods: []CatalogMethod{
			configuredCatalogMethod("joint.router-learning-policy-comparison.v1", "joint", nil, CatalogMethodEvidenceSourceDiagnosticFixture),
		}},
