...
func (p *Profile) Setup(ctx context.Context, opts *framework.SetupOptions) error {
	...
	p.log("Step 1/4: Deploying nginx Ingress Controller")
	if err := p.deployNginxIngress(ctx, deployer, opts); err != nil {
		return fmt.Errorf("failed to deploy nginx ingress: %w", err)
	}

	// Step 2: Deploy LLM backend FIRST (backends must be ready before vSR routes to them)
	p.log("Step 2/4: Deploying LLM backend")
	if err := p.deployLLMBackend(ctx, deployer, opts); err != nil {
		return fmt.Errorf("failed to deploy LLM backend: %w", err)
	}

	// Step 3: Deploy Semantic Router (after backends are ready)
	p.log("Step 3/4: Deploying Semantic Router")
	...
	p.log("Step 4/4: Deploying Ingress resources")
...
func (p *Profile) Teardown(ctx context.Context, opts *framework.TeardownOptions) error {
	p.verbose = opts.Verbose
	p.log("Tearing down nginx ingress test environment")

	deployer := helm.NewDeployer(opts.KubeConfig, opts.Verbose)

	// Clean up in reverse order of deployment (Ingress → vSR → LLM backend → nginx)
