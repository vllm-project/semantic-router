...
const (
	...
	// File path constants
	valuesFile = "e2e/profiles/multi-endpoint/values.yaml"
...
// Setup deploys all required components for multi-endpoint testing
func (p *Profile) Setup(ctx context.Context, opts *framework.SetupOptions) error {
	p.verbose = opts.Verbose
	p.log("Setting up Multi-Endpoint test environment")

	deployer := helm.NewDeployer(opts.KubeConfig, opts.Verbose)

	// Step 1: Deploy Semantic Router with multi-endpoint config
	p.log("Step 1/4: Deploying Semantic Router with multi-endpoint config")
	...
	p.log("Multi-Endpoint test environment setup complete")
	return nil
}
...
func (p *Profile) Teardown(ctx context.Context, opts *framework.TeardownOptions) error {
	p.verbose = opts.Verbose
	p.log("Tearing down Multi-Endpoint test environment")
	...
	p.log("Multi-Endpoint test environment teardown complete")
	return nil
}

// GetTestCases returns the list of test cases for this profile.
// These cover per-environment safety policy enforcement (PII/jailbreak)
// and standard chat-completions through the multi-endpoint setup.
//
// NOTE: "keyword-routing" is intentionally excluded. The shared test
// expects routing-strategies keywords (urgent_request, sensitive_data,
// regex_pattern_match) which don't exist in this profile's config.
// Our keyword rules (code_keywords, general_keywords) route to
// Dev vs Prod — validated indirectly by PII/jailbreak tests that
// send model="MoM" requests through the full routing pipeline.
func (p *Profile) GetTestCases() []string {
	return []string{
		// Safety policies — PII and jailbreak enforcement per environment
		"pii-detection",
		"jailbreak-detection",
		// Standard functional test — validates end-to-end through both backends
...
func (p *Profile) deployGatewayResources(ctx context.Context, opts *framework.SetupOptions) error {
	// Apply Dev + Prod vLLM simulator backends (two separate deployments/services)
	if err := p.kubectlApply(ctx, opts.KubeConfig, "e2e/profiles/multi-endpoint/gateway-resources/backend.yaml"); err != nil {
		return fmt.Errorf("failed to apply multi-endpoint backend resources: %w", err)
	}

	// Apply gateway API resources with AIGatewayRoute rules for qwen14b-dev / qwen14b-prod
	if err := p.kubectlApply(ctx, opts.KubeConfig, "e2e/profiles/multi-endpoint/gateway-resources/gwapi-resources.yaml"); err != nil {
	...
	p.log("Waiting for vllm-14b-dev deployment to be ready...")
	if err := deployer.WaitForDeployment(ctx, "default", "vllm-14b-dev", timeoutDeploymentWait); err != nil {
		return fmt.Errorf("vllm-14b-dev deployment not ready: %w", err)
	}

	p.log("Waiting for vllm-14b-prod deployment to be ready...")
	if err := deployer.WaitForDeployment(ctx, "default", "vllm-14b-prod", timeoutDeploymentWait); err != nil {
		return fmt.Errorf("vllm-14b-prod deployment not ready: %w", err)
	}
...
func (p *Profile) cleanupGatewayResources(ctx context.Context, opts *framework.TeardownOptions) error {
	p.kubectlDelete(ctx, opts.KubeConfig, "e2e/profiles/multi-endpoint/gateway-resources/gwapi-resources.yaml")
	p.kubectlDelete(ctx, opts.KubeConfig, "e2e/profiles/multi-endpoint/gateway-resources/backend.yaml")
	return nil
}
...
func (p *Profile) log(format string, args ...interface{}) {
	if p.verbose {
		fmt.Printf("[Multi-Endpoint] "+format+"\n", args...)
	}
}
