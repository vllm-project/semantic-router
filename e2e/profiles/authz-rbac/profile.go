...
func (p *Profile) Teardown(ctx context.Context, opts *framework.TeardownOptions) error {
	...
	if err := p.cleanupGatewayResources(ctx, opts); err != nil {
		p.log("Warning: failed to cleanup gateway resources: %v", err)
	}

	p.log("Uninstalling Envoy AI Gateway")
	if err := deployer.Uninstall(ctx, releaseAIGatewayCRD, namespaceAIGateway); err != nil {
		p.log("Warning: failed to uninstall %s/%s: %v", namespaceAIGateway, releaseAIGatewayCRD, err)
	}
	if err := deployer.Uninstall(ctx, releaseAIGateway, namespaceAIGateway); err != nil {
		p.log("Warning: failed to uninstall %s/%s: %v", namespaceAIGateway, releaseAIGateway, err)
	}

	p.log("Uninstalling Envoy Gateway")
	if err := deployer.Uninstall(ctx, releaseEnvoyGateway, namespaceEnvoyGateway); err != nil {
		p.log("Warning: failed to uninstall %s/%s: %v", namespaceEnvoyGateway, releaseEnvoyGateway, err)
	}

	p.log("Uninstalling Semantic Router")
	if err := deployer.Uninstall(ctx, releaseSemanticRouter, namespaceSemanticRouter); err != nil {
		p.log("Warning: failed to uninstall %s/%s: %v", namespaceSemanticRouter, releaseSemanticRouter, err)
	}
...
func (p *Profile) verifyEnvironment(ctx context.Context, opts *framework.SetupOptions) error {
	...
	if err := p.waitForEnvoyService(ctx, client); err != nil {
		return err
	}

	if err := p.verifyDeployments(ctx, client); err != nil {
		return err
	}
...
func (p *Profile) waitForEnvoyService(ctx context.Context, client *kubernetes.Clientset) error {
	startTime := time.Now()
...
func (p *Profile) verifyDeployments(ctx context.Context, client *kubernetes.Clientset) error {
...
func (p *Profile) cleanupGatewayResources(ctx context.Context, opts *framework.TeardownOptions) error {
	if err := p.kubectlDelete(ctx, opts.KubeConfig, "e2e/profiles/authz-rbac/gateway-resources/gwapi-resources.yaml"); err != nil {
		p.log("Warning: failed to delete gwapi-resources: %v", err)
	}
	if err := p.kubectlDelete(ctx, opts.KubeConfig, "e2e/profiles/authz-rbac/gateway-resources/backend.yaml"); err != nil {
		p.log("Warning: failed to delete backend resources: %v", err)
	}
	return nil
}
