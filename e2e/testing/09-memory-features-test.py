...
class PluginCombinationTest(MemoryFeaturesTest):
    ...
    def test_01_memory_with_system_prompt_both_present(self):
        ...
        """
        self.print_test_header("Memory + System Prompt: Both Present")
        ...
        # Step 3: Query for the fact - if memory works with system prompt, 
        # the LLM should be able to recall it
        print(f"\n🔍 Step 3: Querying for the fact...")
        ...
        # If "phoenix" appears, the memory was successfully retrieved and injected
        # This proves both system_prompt AND memory are working together
        if "phoenix" in output:
            self.print_test_result(
                True,
                "Memory retrieved with system_prompt enabled! "
                "Both plugins work together correctly."
            )
        else:
            # If phoenix is not found, either:
            # 1. The bug still exists (memory overwritten by system prompt)
            # 2. Memory extraction/retrieval failed for other reasons
            self.print_test_result(
                False,
                f"Memory NOT retrieved! Response doesn't contain 'phoenix'. "
                f"This may indicate the Memory+SystemPrompt bug still exists. "
                f"Response: {output[:100]}..."
            )
            # Don't fail hard - the LLM might just not have mentioned it
    ...
    def test_02_verify_system_prompt_persona_present(self):
        ...
        if found_persona:
            self.print_test_result(
                True,
                "System prompt persona is being applied correctly"
            )
        else:
            self.print_test_result(
