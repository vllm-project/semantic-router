...
class TestGrafanaLiveIntegration(ServeSessionMixin, CLITestBase):
    ...
    def _dashboard_session_token(self, timeout: float = 60) -> str:
        # A running Dashboard container may not yet accept HTTP requests.
        deadline = time.monotonic() + timeout
        last_error = None
        while (remaining := deadline - time.monotonic()) > 0:
            try:
                return self._login_dashboard(timeout=min(10, remaining))
            except (ConnectionError, TimeoutError) as error:
                last_error = error
            time.sleep(min(2, max(0, deadline - time.monotonic())))
        raise TimeoutError(
            "Dashboard login did not become ready in time"
        ) from last_error

    def _login_dashboard(self, *, timeout: float) -> str:
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.runtime_stack.dashboard_port, timeout=timeout
        )
        try:
            connection.request(
                "POST",
