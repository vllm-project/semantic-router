...
if [[ "${VLLM_SR_TEST_ISOLATED:-0}" != "1" ]]; then
    exec python3 "${REPO_ROOT}/tools/dev/with_test_resources.py" --isolate-stack -- bash "${BASH_SOURCE[0]}" "$@"
fi
...
ROUTER_ENDPOINT="${ROUTER_ENDPOINT:-http://localhost:${ROUTER_ENDPOINT_PORT}}"

TEST_DIR="${MEMORY_TEST_DIR:-$(mktemp -d -t vsr-memory-test-XXXXXX)}"
export VLLM_SR_STATE_ROOT_DIR="${VLLM_SR_STATE_ROOT_DIR:-${TEST_DIR}}"
...
cleanup() {
    ...
    local log_dump_dir="${MEMORY_OUTPUT_DIR:-${TEST_DIR}}/logs"
    ...
    if [[ -n "${VLLM_SR_PID}" ]]; then
        VLLM_SR_STATE_ROOT_DIR="${VLLM_SR_STATE_ROOT_DIR}" vllm-sr stop >/dev/null 2>&1 || true
    fi
    ...
    if [[ -n "${MEMORY_OUTPUT_DIR:-}" && -f "${SERVE_LOG}" ]]; then
        cp "${SERVE_LOG}" "${MEMORY_OUTPUT_DIR}/serve.log"
    fi
