...
func init() {
	pkgtestcases.Register("llm-classifier-distribution-routing", pkgtestcases.TestCase{
		Description: "Verify a complete LLM-reported score distribution drives a generic classifier decision",
		Tags:        []string{"classifier", "llm", "routing"},
		Fn:          testLLMClassifierDistributionRouting,
	})
}

func testLLMClassifierDistributionRouting(
	ctx context.Context,
	...
	cases := []struct {
	...
	}{
		{
			...
			prompt:    "__LLM_CLASSIFIER_TOXIC__ verify the LLM classifier path",
			wantMatch: true,
		},
		{
	...
	for _, testCase := range cases {
		...
		matched := decision == llmClassifierDecision
		if matched != testCase.wantMatch {
			return fmt.Errorf(
				"%s: x-vsr-selected-decision=%q, expected LLM classifier match=%t",
				testCase.name,
				decision,
				testCase.wantMatch,
			)
