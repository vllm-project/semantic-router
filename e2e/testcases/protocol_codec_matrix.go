...
func requestProtocolMatrix(
	...
	if err != nil {
		return protocolMatrixHTTPResult{}, fmt.Errorf("marshal request: %w", err)
	}
	request, err := http.NewRequestWithContext(ctx, http.MethodPost, baseURL+path, bytes.NewReader(body))
	if err != nil {
		return protocolMatrixHTTPResult{}, fmt.Errorf("create request: %w", err)
	}
	...
	if err != nil {
		return protocolMatrixHTTPResult{}, fmt.Errorf("send request: %w", err)
	}
	defer response.Body.Close()
	responseBody, err := io.ReadAll(response.Body)
	if err != nil {
		return protocolMatrixHTTPResult{}, fmt.Errorf("read response: %w", err)
	}
...
func requestProtocolToolMatrix(
	...
	body, err := json.Marshal(payload)
	if err != nil {
		return protocolMatrixHTTPResult{}, fmt.Errorf("marshal tool request: %w", err)
	}
	request, err := http.NewRequestWithContext(ctx, http.MethodPost, baseURL+path, bytes.NewReader(body))
	if err != nil {
		return protocolMatrixHTTPResult{}, fmt.Errorf("create tool request: %w", err)
	}
	request.Header.Set("Content-Type", "application/json")
	if stream {
		request.Header.Set("Accept", "text/event-stream")
	...
	response, err := client.Do(request)
	if err != nil {
		return protocolMatrixHTTPResult{}, fmt.Errorf("send tool request: %w", err)
	}
	defer response.Body.Close()
	responseBody, err := io.ReadAll(response.Body)
	if err != nil {
		return protocolMatrixHTTPResult{}, fmt.Errorf("read tool response: %w", err)
	}
	return protocolMatrixHTTPResult{statusCode: response.StatusCode, contentType: response.Header.Get("Content-Type"), body: responseBody}, nil
}
