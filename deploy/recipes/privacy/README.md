...
### Per-category knowledge base classification

- `privacy_classifier` (type: `category_kb`)

Loads per-category JSON knowledge bases from `knowledge_bases/` and computes max-cosine-similarity per category. Produces a contrastive score (max private-tier similarity minus max public-tier similarity) that feeds `privacy_contrastive_score`, which maps into:
