...
type semanticRouterValidationFamily struct {
	validate func(*SemanticRouter) error
}

// Keep family-specific admission checks in adjacent files so this entrypoint
// stays focused on webhook wiring and validation dispatch.
var semanticRouterValidationFamilies = []semanticRouterValidationFamily{
	{validate: (*SemanticRouter).validateAutoscalingConfig},
	{validate: (*SemanticRouter).validatePersistenceConfig},
	{validate: (*SemanticRouter).validateProbeConfigs},
	{validate: (*SemanticRouter).validateIngressConfig},
}
...
func (r *SemanticRouter) validateSemanticRouter() error {
	for _, family := range semanticRouterValidationFamilies {
		if err := family.validate(r); err != nil {
			return err
		}
	}
