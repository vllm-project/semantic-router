...
func TestDurableServicesSeparateInferenceAndManagement(t *testing.T) {
	...
	if dispatch == nil || len(dispatch.Spec.Ports) != 1 ||
		...
		t.Fatalf("backend dispatch Service = %#v", dispatch)
	}
	...
	if metrics == nil {
		t.Fatalf("durable internal Services = %#v", byName)
	}
...
func TestDurableDeploymentUsesHTTPSReadinessAndTopologySpread(t *testing.T) {
	...
	for _, port := range container.Ports {
		if port.ContainerPort != testDurableBootstrap.BackendDispatchPort {
			continue
		}
		dispatchPortFound = true
		if port.Name != backendDispatchPortName || len(port.Name) > 15 {
			t.Fatalf("backend dispatch container port = %#v", port)
		}
	}
	...
	if container.ReadinessProbe == nil || container.ReadinessProbe.HTTPGet == nil {
		t.Fatalf("Management readiness probe = %#v", container.ReadinessProbe)
	}
	if container.ReadinessProbe.HTTPGet.Scheme != corev1.URISchemeHTTPS ||
		...
		t.Fatalf("Management readiness HTTPGet = %#v", container.ReadinessProbe.HTTPGet)
	}
...
`},
	...
	sr := &vllmv1alpha1.SemanticRouter{
		...
		Spec: vllmv1alpha1.SemanticRouterSpec{
			...
			Env: []corev1.EnvVar{
				{Name: "TEST_DATABASE_URL", Value: "redacted-for-test"},
