...
func (r *SemanticRouterReconciler) reconcileMigrationJob(
	...
	if err := ensureControlledBy(found, sr, "managed schema migration Job"); err != nil {
		return nil, err
	}
	...
	for _, condition := range found.Status.Conditions {
		if condition.Type == batchv1.JobFailed && condition.Status == corev1.ConditionTrue {
			return &vllmv1alpha1.MigrationStatus{JobName: found.Name, State: migrationStateFailed},
				fmt.Errorf("managed schema migration Job %s failed", found.Name)
		}
...
func (r *SemanticRouterReconciler) deleteStaleMigrationJobs(
	...
	if err := r.List(
		...
		client.MatchingLabels{
			...
			"app.kubernetes.io/component": "access-migration",
		},
	); err != nil {
		return fmt.Errorf("list managed schema migration Jobs: %w", err)
	}
	for index := range jobs.Items {
		...
		if err := r.Delete(ctx, job); err != nil && client.IgnoreNotFound(err) != nil {
			return fmt.Errorf("delete stale managed schema migration Job %s: %w", job.Name, err)
		}
	}
	return nil
}
...
func (r *SemanticRouterReconciler) generateMigrationJob(
	...
	labels["app.kubernetes.io/component"] = "access-migration"
			...
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					...
					Containers: []corev1.Container{{
						...
						Command:         []string{"/usr/local/bin/access-migrate"},
						Args:            args,
						Env:             sr.Spec.Env,
...
func migrationJobName(
	...
	if err != nil {
		return "", fmt.Errorf("encode managed schema migration inputs: %w", err)
	}
	digest := sha256.Sum256(payload)
	suffix := fmt.Sprintf("%x", digest[:6])
	base := sr.Name
	const reserved = len("-access-migrate-") + 12
	if len(base) > 63-reserved {
		base = strings.TrimRight(base[:63-reserved], "-")
	}
	return base + "-access-migrate-" + suffix, nil
}
...
func (r *SemanticRouterReconciler) reconcilePodDisruptionBudget(
	...
	if !modeAwareEnabled(sr.Spec.PodDisruptionBudget.Enabled, mode == controlPlaneModeManaged) {
...
func (r *SemanticRouterReconciler) reconcileNetworkPolicy(
	...
	key := types.NamespacedName{Name: sr.Name, Namespace: sr.Namespace}
	if !modeAwareEnabled(sr.Spec.NetworkPolicy.Enabled, bootstrap.Mode == controlPlaneModeManaged) {
