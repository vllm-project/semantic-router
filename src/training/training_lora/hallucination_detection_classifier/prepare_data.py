...
try:
    ...
    HAS_DATASETS = False
    print("Warning: 'datasets' library not installed. Cannot download from HuggingFace.")
...
def download_dart_e2e_from_huggingface(output_dir: Path) -> tuple[Optional[Path], Optional[Path]]:
    """
...
def main():
    ...
    print(f"✓ Saved {len(train_samples) + len(dev_samples)} combined samples to: {combined_path}")
