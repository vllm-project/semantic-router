...
from src.training.kv_mapper.fit_run import (
    _channel_scores,
    fit_capture,
        ...
        for i in range(3):
            for j in range(2):
                np.testing.assert_allclose(
                    scores[i, j], ols_r2_per_head(source[i], target[j]), atol=1e-7
                )
    ...
    def test_chunk_to_artifact_with_rotated_source(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            meta = ActivationRunMeta(
                ...
                token_sha256=token_fingerprint(tokens),
                num_sequences=4,
                source_layers=[0, 1],
                target_layers=[0],
                num_kv_heads=2,
            ...
            target = (source[1] @ np.roll(np.eye(3), 1, axis=0) + 0.3).astype(
                np.float32
            )
            for index in range(4):
                sl = slice(index * 4, (index + 1) * 4)
                write_activation_chunk(
                    run / "source" / f"{index:06d}.npz",
                    list(source[:, sl]),
                    list(source[:, sl]),
                )
                write_activation_chunk(
                    run / "target" / f"{index:06d}.npz", [target[sl]], [target[sl]]
                )
            artifact = fit_capture(
                run,
            ...
            prediction = (
                source[1].reshape(16, -1) @ tensors["target.0.k.W"]
                + tensors["target.0.k.b"]
            )
            np.testing.assert_allclose(prediction, target.reshape(16, -1), atol=1e-4)
            with self.assertRaisesRegex(FileExistsError, "bundle-version"):
                fit_capture(
                    run,
                    Path(directory) / "output",
                    pair_slug="synth",
            ...
            self.assertTrue(second.name.endswith("-b2"))
