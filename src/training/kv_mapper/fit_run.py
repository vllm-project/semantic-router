...
from src.training.kv_mapper.collect import read_run_metadata, token_fingerprint, validate_activation_chunk
...
def load_captures(run_dir: Path):
    ...
    if tokens.shape != (meta.num_sequences, meta.seq_len) or token_fingerprint(tokens) != meta.token_sha256:
        raise ValueError("token windows do not match run metadata")
    rows = len(range(0, meta.seq_len, meta.fitting_token_step))
    captures = {}
    for side, layers in (("source", meta.source_layers), ("target", meta.target_layers)):
        shape = (len(layers), meta.num_sequences * rows, meta.num_kv_heads, meta.head_dim)
        ...
        for sequence in range(meta.num_sequences):
            ...
            if not validate_activation_chunk(path, len(layers), rows, meta.num_kv_heads, meta.head_dim):
...
def _channel_scores(source: np.ndarray, target: np.ndarray) -> np.ndarray:
    ...
    for head in range(heads):
        x = np.ascontiguousarray(source[:, :, head, :].transpose(1, 0, 2).reshape(rows, n_src * dim), dtype=np.float64)
        y = np.ascontiguousarray(target[:, :, head, :].transpose(1, 0, 2).reshape(rows, n_tgt * dim), dtype=np.float64)
        ...
        total = np.square(y).sum(axis=0).reshape(n_tgt, dim).sum(axis=1) - np.square(sy).sum(axis=1) / rows
        for source_layer in range(n_src):
            ...
            scores[source_layer, :, head] = np.where(total > 0, 1 - np.maximum(total - explained, 0) / np.maximum(total, 1e-30), 0)
        del x, y, cross
    return scores
...
def select_shared_layers(captures: dict, k: int) -> list[list[int]]:
    ...
    scores = (_channel_scores(source_k, target_k) + _channel_scores(source_v, target_v)) / 2
    scores = scores.mean(axis=2)
    return [[int(i) for i in np.argsort(-scores[:, t], kind="stable")[:k]] for t in range(target_k.shape[0])]


def _work_dir(run_dir: Path, out_dir: Path, meta, topk: int, ridge_alpha: float) -> Path:
    recipe = {"run": meta.to_dict(), "topk": topk, "ridge_alpha": ridge_alpha, "chunks": []}
    for side in ("source", "target"):
        for sequence in range(meta.num_sequences):
            recipe["chunks"].append((run_dir / side / f"{sequence:06d}.sha256").read_text().strip())
...
def _partial(path: Path, weight: np.ndarray | None = None, bias: np.ndarray | None = None):
    ...
    if not path.exists():
        return None
    if not checksum.exists() or hashlib.sha256(path.read_bytes()).hexdigest() != checksum.read_text().strip():
        raise ValueError(f"corrupt fit partial: {path}")
    with np.load(path, allow_pickle=False) as data:
        return data["weight"], data["bias"]


def fit_capture(run_dir: Path, out_dir: Path, *, pair_slug: str, topk: int = 8, ridge_alpha: float = 0.01) -> Path:
    ...
    for target_layer, indices in enumerate(chosen):
        for channel, source, target in (("k", source_k, target_k), ("v", source_v, target_v)):
            path = work / f"target.{target_layer}.{channel}.npz"
            partial = _partial(path)
            if partial is None:
                fitted = fit_full_head(
                    [([source[i] for i in range(source.shape[0])], [target[target_layer]])],
                    [indices], n_kv_heads=meta.num_kv_heads,
                    head_dim=meta.head_dim, ridge_alpha=ridge_alpha, channel=channel,
                )
                partial = _partial(path, fitted[f"target.0.{channel}.W"], fitted[f"target.0.{channel}.b"])
            tensors[f"target.{target_layer}.{channel}.W"] = partial[0]
            tensors[f"target.{target_layer}.{channel}.b"] = partial[1]
    compatibility = CompatibilitySpec(
        source_model=meta.source_model, source_revision=meta.source_revision,
        target_model=meta.target_model, target_revision=meta.target_revision,
        variant="full_head", precision=meta.precision, source_tp=1, target_tp=1,
        head_order="contiguous", num_kv_heads=meta.num_kv_heads, head_dim=meta.head_dim,
    )
    mapping = {str(t): [meta.source_layers[i] for i in indices] for t, indices in enumerate(chosen)}
    mapper_id = write_fitted_artifact(
        out_dir, compat=compatibility, tensors=tensors, topk=topk,
        ridge_alpha=ridge_alpha, source_layers_per_target={"k": mapping, "v": mapping},
        pair_slug=pair_slug, calibration=meta.to_dict(),
    )
...
def main() -> None:
    ...
    result = fit_capture(args.run_dir, args.output_dir, pair_slug=args.pair_slug, topk=args.topk, ridge_alpha=args.ridge_alpha)
