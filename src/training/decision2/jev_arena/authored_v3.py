"""Build the unapproved authored v3 typed-decision candidate.

V3 has sixty formal operations and one scored item per unique fact scenario.
...
from .authored_v3_reference import evaluate_rendered
...
BUILD_VERSION = "jevarena-authored-build/3"
CHALLENGES = (
    "near_distractor",
    "long_context",
...
FILLER_TOPICS = (
    "A scheduling note records a separate appointment and its owner.",
    "The appendix lists a prior request with a different identifier.",
    "A review memo summarizes an unrelated handoff and its date.",
    "The register includes a historical status that is not current evidence.",
    "A capacity memo describes a different team and reporting period.",
    "The attachment records a closed case and its archived checklist.",
    "A finance note describes a former budget line for another case.",
    "An audit entry identifies the custodian of an unrelated document.",
)
...
def _evidence(facts: dict[str, Any], style: str) -> str:
    ...
    raise ValueError("Unknown v3 evidence style")
...
def _filler(rng: random.Random, domain: str, count: int) -> list[str]:
    entries = []
    for number in range(count):
        case = f"{domain[:3].upper()}-{rng.getrandbits(48):012X}"
        owner = f"reviewer-{rng.randint(101, 998)}"
        day = rng.randint(1, 29)
        topic = FILLER_TOPICS[
            (number + rng.randrange(len(FILLER_TOPICS))) % len(FILLER_TOPICS)
        ]
        entries.append(
            f"Attachment {number + 1}, case {case}, archived day {day}: {topic} "
            f"This entry was filed by {owner} for the {domain} register. "
            "It concerns a different case identifier and cannot change the current policy or "
            "the target evidence. The recorded measurements and approvals, if any, apply "
            "only to this archived attachment."
        )
    return entries
...
def _fallback(op: Operation) -> str:
    base = "Every field and array entry in the target evidence must be reported; null is unreported. "
    if op.kind == "choice":
        return (
            base
            + "If any required fact is unreported or no candidate qualifies, choose hold. Resolve exact ties by candidate ID A before B before C before D."
        )
    if op.kind == "noul":
        return base + "If any required fact is unreported, the decision test is false."
    return (
        base
        + "If any required fact is unreported, assign grade zero. The grade range is zero through four."
    )
...
def render(spec: dict[str, Any], seed: bytes) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if spec["challenge"] == "long_context":
        attachments = _filler(rng, op.domain, 66)
...
def solve_visible(prompt: dict[str, Any]) -> str | bool | int:
    ...
    if set(facts) != set(op.context_fields) | (
...
def audit_panel(
    ...
    if not len(prompts) == len(targets) == len(specs):
        raise ValueError("V3 prompt/target/spec row counts differ")
    if len({row["id"] for row in prompts}) != len(prompts):
        raise ValueError("Duplicate V3 prompt ID")
    if len({row["group_id"] for row in targets}) != len(targets):
        raise ValueError("V3 scored variants share a source group")
    if len(
        {
            sha_bytes(
                compact(
    ...
    if (
        ...
        raise ValueError("V3 operation/type/challenge coverage matrix incomplete")
    protected_states = {
    ...
    return {
        "audit_version": "jevarena-authored-v3-audit/1",
        "items": len(prompts),
        ...
        "automated_status": (
            "passed"
            if not (exact_state or exact_input or near_protected or near_internal)
            else "blocked"
        ),
...
def build(
    ...
    if phase not in ("dev", "release") or output.exists():
        raise ValueError("V3 phase dev/release required; output must be new")
    seed = seed_path.read_bytes()
    if len(seed) != 32:
        raise ValueError("V3 private seed must contain 256 bits")
    per_challenge = 1 if phase == "dev" else 6
    prompts, targets, specs = [], [], []
    seen_core: set[str] = set()
    for op in OPERATIONS:
        for challenge in CHALLENGES:
            for ordinal in range(per_challenge):
                for attempt in range(1000):
                    spec = make_spec(seed, phase, op, challenge, ordinal, attempt)
                    core_hash = sha_bytes(
                        compact(
                            {"operation_id": op.id, "facts": spec["facts"]}
                        ).encode()
                    )
                    if core_hash not in seen_core:
                        seen_core.add(core_hash)
                        break
                else:
                    raise ValueError(
                        f"Could not form a unique v3 fact pack for {op.id}"
                    )
    ...
    if audit["automated_status"] != "passed":
        raise ValueError("V3 automated overlap/coverage audit blocked the candidate")
    expected = 240 if phase == "dev" else 1440
    if audit["items"] != expected or audit["unique_scenario_groups"] != expected:
        raise ValueError("V3 preregistered item count differs")
    ...
    manifest = {
        ...
        "builder_files_sha256": {
            ...
            for path in (
                Path(__file__),
                Path(__file__).with_name("authored_v3_ops.py"),
                Path(__file__).with_name("authored_v3_reference.py"),
            )
        },
