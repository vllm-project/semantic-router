"""Sixty formal decision operations for the authored v3 candidate.
...
NOUL = _ops(
    ...
    [
        ...
        (
            ...
            "If the check at the first listed index is verified, is the check at the second index also verified?",
            "checks indices",
            "",
        ),
        (
        ...
        (
            "path-reachability",
            "Can node zero reach node four along directed dependencies?",
            "edges",
            "",
        ),
        ...
        (
            ...
            "Are all named prerequisites included among completed tasks?",
            "required_tags completed_tags",
            "",
        ),
        (
            "exception-exemption",
...
SCORE = _ops(
    ...
    [
        ...
        (
            ...
            "Multiply each value by its weight, sum, then clamp at four.",
            "values weights",
            "",
        ),
        (
        ...
        (
            ...
            "Grade the longest simple directed dependency path from node zero (never revisit a node), capped at four edges.",
            "edges",
            "",
        ),
        (
        ...
        (
            "forecast-error-band",
            "Give four for exact forecast, then subtract floor(absolute error divided by three), floored at zero.",
            "forecast actual",
            "",
        ),
        (
...
def _base_facts(rng: random.Random) -> dict[str, Any]:
    ...
    rng.shuffle(checks)
    workflow_order = ["intake", "review", "verify", "approve", "close"]
    rng.shuffle(workflow_order)
...
def inject_missing(facts: dict[str, Any]) -> dict[str, Any]:
    """Replace one needed, visible fact with null for the insufficient challenge."""
    import copy

    result = copy.deepcopy(facts)
    key = next(iter(result))
    if key == "checks" and "indices" in result:
        # A check outside the named indices is not evidence for this decision.
        result["checks"][result["indices"][0]] = None
        return result
    if key == "signers" and "attested" in result:
        # An unattested signer's name is immaterial; the attestation flag is needed.
        result["attested"][0] = None
        return result
    value = result[key]
    if isinstance(value, list) and value:
        if isinstance(value[0], dict):
            subkey = next(item for item in value[0] if item != "id")
            value[0][subkey] = None
...
def evaluate(op: Operation, facts: dict[str, Any]) -> str | bool | int:
    ...
    if op.kind == "noul":
        ...
        if name == "dependency-completion":
            return set(facts["required_tags"]) <= set(facts["completed_tags"])
    ...
    if op.kind == "score":
        ...
        if name == "forecast-error-band":
            return clamp(4 - abs(facts["forecast"] - facts["actual"]) // 3)
    ...
    raise ValueError(f"Unsupported authored v3 operation {op.id}")
