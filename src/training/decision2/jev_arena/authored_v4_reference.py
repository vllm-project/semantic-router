"""Independent rendered-fact oracle for authored v4 operations.
...
from .authored_v4_ops import Operation, OPTIONS, possible_worlds
...
def evaluate_rendered(op: Operation, facts: dict[str, Any]) -> str | bool | int:
    if "admissible_uncertainty" in facts:
        first, second = possible_worlds(facts)
        answers = (evaluate_rendered(op, first), evaluate_rendered(op, second))
        return (
            answers[0]
    ...
    if op.kind == "choice":
        ...
        if name == "dependency-completion":
            needed = set(facts["required_tags"])
            return not needed.difference(
                facts["completed_tags"]
            ) and not needed.intersection(facts["revoked_tags"])
        ...
        if name == "forecast-error-band":
            return bound(4 - ((abs(facts["actual"] - facts["forecast"]) + 2) // 3))
