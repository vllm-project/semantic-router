...
def load_feedback_data(data_source: str, max_samples: int = None):
    ...
    if "/" in data_source and not Path(data_source).exists():
        ...
        try:
            ...
            try:
                ...
                dataset = load_dataset(data_source, data_files={
                    'train': 'train.jsonl',
                    'validation': 'val.jsonl'
                })
            
            for split, examples in [("train", train_examples), ("validation", val_examples)]:
                if split in dataset:
            ...
            train_sat_count = sum(1 for ex in train_examples if ex["label_name"] == "SAT")
    ...
    for split_name, examples in [("train", train_examples), ("val", val_examples)]:
        ...
        for pattern in [f"{split_name}.jsonl", f"{split_name}_feedback.jsonl", f"feedback_{split_name}.jsonl"]:
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="models/mmbert_feedback_detector")
    
    # Data
    parser.add_argument("--data_source", type=str, default="llm-semantic-router/feedback-detector-dataset",
    ...
    print(f"LoRA: {'Yes (rank={}, alpha={})'.format(args.lora_rank, args.lora_alpha) if args.use_lora else 'No (full fine-tune)'}")
    print("\nLabels:")
    ...
    train_examples, val_examples = load_feedback_data(args.data_source, args.max_samples)
    ...
    val_dataset = Dataset.from_list(val_examples) if val_examples else train_dataset.select(range(min(100, len(train_examples))))
    ...
    if args.use_lora:
        ...
        lora_config = LoraConfig(
            ...
            lora_dropout=0.1,
            target_modules=["attn.Wqkv", "attn.Wo", "mlp.Wi", "mlp.Wo"],  # mmBERT/ModernBERT
            bias="none",
        )
    ...
    train_dataset = train_dataset.map(tokenize_fn, batched=True, remove_columns=["text", "label_name"])
    val_dataset = val_dataset.map(tokenize_fn, batched=True, remove_columns=["text", "label_name"])
