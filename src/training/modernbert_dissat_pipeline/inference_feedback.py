...
class FeedbackDetector:
    ...
    def classify_batch(self, texts: List[str]) -> List[FeedbackResult]:
        ...
        for i in range(len(texts)):
            ...
            all_scores = {ID2LABEL[j]: sample_probs[j].item() for j in range(len(ID2LABEL))}
...
def demo():
    ...
    print("\n" + "=" * 70)
    print(f"🌍 Multilingual Total: {total_correct}/{len(multilingual_cases)} ({total_correct/len(multilingual_cases)*100:.1f}%)")
    print("=" * 70)
