"""
Dataset download and labeling utilities for training pipelines.

Downloads all required public datasets:

Feedback Detector Datasets:
- MIMICS / MIMICS-Duo (Microsoft) - Search clarification
- INSCIT (Google Research) - Information-seeking conversations
- MultiWOZ - Task-oriented dialogue
- SGD (Schema-Guided Dialogue) - Google Research
- ReDial - Recommendation dialogues
- Hazumi (via HuggingFace) - Japanese dialogue
- Customer/Consumer Complaints - For WRONG_ANSWER class

Jailbreak Detection Datasets:
- AEGIS (nvidia/Aegis-AI-Content-Safety-Dataset-2.0) - 30K samples, 12 hazard types
- Salad-Data (OpenSafetyLab/Salad-Data) - Attack prompts
- Toxic Chat (lmsys/toxic-chat) - Real-world toxic prompts
- Jailbreak Classification (jackhhao/jailbreak-classification) - Labeled jailbreak/benign

Labeling uses GPT-OSS-120B via vLLM for the 4-class feedback schema:
...
class LabelingConfig:
    ...
    garbage_consecutive_threshold: int = 5  # Consecutive garbage to trigger circuit breaker
    ...
    def should_pause(self) -> tuple:
        ...
        with self.lock:
            ...
            if self.circuit_open:
                pause_time = min(
                    self.cooldown_base * self.cooldown_multiplier,
                    self.max_cooldown
                )
...
class CheckpointManager:
    ...
    def __init__(self, checkpoint_dir: str, dataset_name: str):
        ...
        self.checkpoint_file = os.path.join(checkpoint_dir, f"{dataset_name}_checkpoint.json")
        self.labeled_file = os.path.join(checkpoint_dir, f"{dataset_name}_labeled.jsonl")
    ...
    def _load_checkpoint(self):
        ...
        if os.path.exists(self.checkpoint_file):
            try:
                ...
                logger.info(f"Loaded checkpoint: {len(self.processed_hashes)} already processed")
...
def download_aegis(raw_data_dir: str) -> bool:
    """
    Download AEGIS AI Content Safety Dataset 2.0 from HuggingFace.
    
    ~30K samples with 12 hazard types for jailbreak/safety detection.
    ...
    try:
        ...
        dest_dir = os.path.join(raw_data_dir, "aegis")
        os.makedirs(dest_dir, exist_ok=True)

        print("Downloading AEGIS dataset from HuggingFace...")
        dataset = load_dataset("nvidia/Aegis-AI-Content-Safety-Dataset-2.0")

        # Save to disk
        dataset.save_to_disk(dest_dir)
        print(f"AEGIS dataset saved to {dest_dir}")
        return True

    except Exception as e:
        print(f"Error downloading AEGIS dataset: {e}")
        print("Try: pip install datasets")
        return False


def download_salad_data(raw_data_dir: str) -> bool:
    """
    Download Salad-Data from HuggingFace.
    
    Attack prompts for jailbreak detection from OpenSafetyLab.
    ...
    try:
        ...
        dest_dir = os.path.join(raw_data_dir, "salad_data")
        os.makedirs(dest_dir, exist_ok=True)

        print("Downloading Salad-Data from HuggingFace...")
        dataset = load_dataset("OpenSafetyLab/Salad-Data")

        # Save to disk
        dataset.save_to_disk(dest_dir)
        print(f"Salad-Data saved to {dest_dir}")
        return True

    except Exception as e:
        print(f"Error downloading Salad-Data: {e}")
        print("Try: pip install datasets")
        return False


def download_toxic_chat(raw_data_dir: str) -> bool:
    """
    Download Toxic Chat dataset from HuggingFace.
    
    Real-world toxic/jailbreak prompts from lmsys.
    ...
    try:
        ...
        dest_dir = os.path.join(raw_data_dir, "toxic_chat")
        os.makedirs(dest_dir, exist_ok=True)

        print("Downloading Toxic Chat dataset from HuggingFace...")
        dataset = load_dataset("lmsys/toxic-chat")

        # Save to disk
        dataset.save_to_disk(dest_dir)
        print(f"Toxic Chat dataset saved to {dest_dir}")
        return True

    except Exception as e:
        print(f"Error downloading Toxic Chat dataset: {e}")
        print("Try: pip install datasets")
        return False


def download_jailbreak_classification(raw_data_dir: str) -> bool:
    """
    Download Jailbreak Classification dataset from HuggingFace.
    
    Labeled jailbreak/benign prompts for classification.
    ...
    try:
        ...
        dest_dir = os.path.join(raw_data_dir, "jailbreak_classification")
        os.makedirs(dest_dir, exist_ok=True)

        print("Downloading Jailbreak Classification dataset from HuggingFace...")
        dataset = load_dataset("jackhhao/jailbreak-classification")

        # Save to disk
        dataset.save_to_disk(dest_dir)
        print(f"Jailbreak Classification dataset saved to {dest_dir}")
        return True

    except Exception as e:
        print(f"Error downloading Jailbreak Classification dataset: {e}")
        print("Try: pip install datasets")
        return False


def download_jailbreak_datasets(raw_data_dir: str = None) -> dict:
    """
    Download all jailbreak detection datasets.
    ...
    print("=" * 60)
    print("Downloading jailbreak detection datasets")
    print("=" * 60)

    results = {}

    print("\n[1/4] Downloading AEGIS (nvidia)...")
    results["aegis"] = download_aegis(raw_data_dir)

    print("\n[2/4] Downloading Salad-Data (OpenSafetyLab)...")
    results["salad_data"] = download_salad_data(raw_data_dir)

    print("\n[3/4] Downloading Toxic Chat (lmsys)...")
    results["toxic_chat"] = download_toxic_chat(raw_data_dir)

    print("\n[4/4] Downloading Jailbreak Classification (jackhhao)...")
    results["jailbreak_classification"] = download_jailbreak_classification(raw_data_dir)

    # Summary
    print("\n" + "=" * 60)
    print("Jailbreak Datasets Summary:")
    print("=" * 60)
...
def download_all_datasets(config: Optional[DataConfig] = None) -> dict:
    ...
    # Feedback detector datasets
    print("\n" + "=" * 40)
    print("FEEDBACK DETECTOR DATASETS")
    print("=" * 40)
    
    print("\n[1/13] Downloading MIMICS / MIMICS-Duo...")
    results["mimics"] = download_mimics(raw_data_dir)

    print("\n[2/13] Downloading INSCIT...")
    results["inscit"] = download_inscit(raw_data_dir)

    print("\n[3/13] Downloading MultiWOZ...")
    results["multiwoz"] = download_multiwoz(raw_data_dir)

    print("\n[4/13] Downloading SGD...")
    results["sgd"] = download_sgd(raw_data_dir)

    print("\n[5/13] Downloading ReDial...")
    results["redial"] = download_redial(raw_data_dir)

    print("\n[6/13] Downloading Hazumi...")
    results["hazumi"] = download_hazumi(raw_data_dir)

    # Complaint datasets for WRONG_ANSWER class
    print("\n[7/13] Downloading Customer Complaints (for WRONG_ANSWER)...")
    results["customer_complaints"] = download_customer_complaints(raw_data_dir)

    print("\n[8/13] Downloading Consumer Complaints Medium (for WRONG_ANSWER)...")
    results["consumer_complaints_medium"] = download_consumer_complaints_medium(raw_data_dir)

    print("\n[9/13] Downloading Turkish Complaints (multilingual)...")
    results["turkish_complaints"] = download_turkish_complaints(raw_data_dir)

    # Jailbreak detection datasets
    print("\n" + "=" * 40)
    print("JAILBREAK DETECTION DATASETS")
    print("=" * 40)
    
    print("\n[10/13] Downloading AEGIS (nvidia)...")
    results["aegis"] = download_aegis(raw_data_dir)

    print("\n[11/13] Downloading Salad-Data (OpenSafetyLab)...")
    results["salad_data"] = download_salad_data(raw_data_dir)

    print("\n[12/13] Downloading Toxic Chat (lmsys)...")
    results["toxic_chat"] = download_toxic_chat(raw_data_dir)

    print("\n[13/13] Downloading Jailbreak Classification (jackhhao)...")
    results["jailbreak_classification"] = download_jailbreak_classification(raw_data_dir)
    ...
    print("  - Complaints (WRONG_ANSWER): customer_complaints, consumer_complaints_medium")
    print("  - Multilingual: hazumi (Japanese), turkish_complaints (Turkish)")
    print("  - Jailbreak Detection: aegis, salad_data, toxic_chat, jailbreak_classification")
    ...
    for attempt in range(config.max_retries):
        try:
            ...
            if is_garbage_output(content):
                garbage_count += 1
                logger.debug(f"Garbage output detected (attempt {attempt + 1}): {repr(content[:50])}")
                
                # If we're getting consistent garbage, back off more aggressively
                if garbage_count >= 2:
                    backoff = config.retry_delay * (4 ** attempt) + random.uniform(1, 5)
                    logger.debug(f"Multiple garbage responses, backing off {backoff:.1f}s")
                    time.sleep(backoff)
                else:
            ...
            try:
                ...
                with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    for row in reader:
                        ...
                        text = row.get('query', '') or row.get('question', '') or row.get('text', '')
...
def extract_multiwoz_examples(raw_data_dir: str) -> Generator[Dict[str, str], None, None]:
    """
                    ...
                    if filepath.endswith('.jsonl'):
                        for line in f:
                            if line.strip():
                                ...
                                yield from _extract_dialogue_turns(data, "redial", filepath)
                    else:
                        data = json.load(f)
                        if isinstance(data, list):
                            for item in data:
                                yield from _extract_dialogue_turns(item, "redial", filepath)
            except Exception as e:
                logger.warning(f"Error reading {filepath}: {e}")
...
def extract_customer_complaints_examples(raw_data_dir: str) -> Generator[Dict[str, str], None, None]:
    """
    ...
    try:
        ...
        for split_name in dataset.keys():
            for example in dataset[split_name]:
                ...
                text = (
                    example.get('Consumer_complaint_narrative', '') or
                    example.get('Consumer complaint narrative', '') or
                    example.get('complaint', '') or
                    example.get('text', '') or
                    example.get('narrative', '')
                )
                
                if text and len(text) > 20:
                    # Extract product/sub-product for metadata
                    product = example.get('Product', '') or example.get('product', '')
                    sub_product = example.get('Sub_product', '') or example.get('sub_product', '')
...
def extract_consumer_complaints_medium_examples(raw_data_dir: str) -> Generator[Dict[str, str], None, None]:
    """
    ...
    try:
        ...
        for split_name in dataset.keys():
            for example in dataset[split_name]:
                ...
                text = (
                    example.get('Consumer Complaint', '') or
                    example.get('consumer_complaint', '') or
                    example.get('text', '')
                )
...
def extract_turkish_complaints_examples(raw_data_dir: str) -> Generator[Dict[str, str], None, None]:
    """
        ...
        for split_name in dataset.keys():
            for example in dataset[split_name]:
                ...
                text = (
                    example.get('Text', '') or
                    example.get('text', '') or
                    example.get('complaint', '')
                )
                category = example.get('Label', '') or example.get('label', '') or example.get('category', '')
...
def _extract_dialogue_turns(data: Dict, source: str, filepath: str) -> Generator[Dict[str, str], None, None]:
    """
        ...
        if turns_key in data and isinstance(data[turns_key], list):
            for turn in data[turns_key]:
                if isinstance(turn, str):
                    if len(turn) > 10:
                        yield {"text": turn, "source": source, "file": os.path.basename(filepath)}
                elif isinstance(turn, dict):
                    for text_key in text_keys:
                        if text_key in turn and turn[text_key]:
                            text = turn[text_key]
                            if len(text) > 10:
                                yield {"text": text, "source": source, "file": os.path.basename(filepath)}
                            break
            return
    ...
    for text_key in text_keys:
        if text_key in data and data[text_key] and len(str(data[text_key])) > 10:
            yield {"text": str(data[text_key]), "source": source, "file": os.path.basename(filepath)}
            return
...
def label_dataset(
    ...
    if not config.skip_health_check:
        ...
        if not is_healthy:
            ...
            logger.error("Please ensure the vLLM server is running and healthy before labeling.")
    ...
    examples_to_process = [
        ...
        if not checkpoint.is_processed(ex["text"])
    ]
    
    if len(examples_to_process) < len(examples):
        logger.info(f"Resuming: {len(examples) - len(examples_to_process)} already processed")
    ...
    with tqdm(total=len(examples_to_process), desc=f"Labeling {dataset_name}") as pbar:
        ...
        for batch_start in range(0, len(examples_to_process), batch_size):
            ...
            if should_pause:
                logger.warning(f"⚠️ High garbage rate detected. Pausing for {pause_duration:.0f}s...")
            ...
            with ThreadPoolExecutor(max_workers=config.workers) as executor:
                futures = {
                    executor.submit(label_one, ex): ex 
                    for ex in batch
                }
                ...
                for future in as_completed(futures):
                    ...
                    if config.garbage_max_total > 0 and total_garbage >= config.garbage_max_total:
                        aborted = True
                        abort_reason = f"Max garbage limit reached ({total_garbage})"
                        break
                    
                    gc_stats = garbage_detector.get_stats()
                    if gc_stats["consecutive_garbage"] >= config.garbage_consecutive_threshold * 2:
                        aborted = True
                        abort_reason = f"Too many consecutive garbage responses ({gc_stats['consecutive_garbage']})"
                        break
                    
                    # Update progress bar with stats
                    success_rate = (checkpoint.stats["total"] - checkpoint.stats["errors"]) / max(1, checkpoint.stats["total"]) * 100
                    ...
                    if processed_count % config.checkpoint_interval == 0:
                        ...
                        logger.info(f"Checkpoint saved: {processed_count} processed, {total_garbage} garbage")
    ...
    if aborted:
        ...
        logger.error(f"   Progress saved. Run with --resume to continue when model is healthy.")
...
def label_all_datasets(
    ...
    extractors = {
        ...
        "customer_complaints": (extract_customer_complaints_examples, "Customer Complaints [WRONG_ANSWER]"),
        "consumer_complaints_medium": (extract_consumer_complaints_medium_examples, "Consumer Complaints [WRONG_ANSWER]"),
        "turkish_complaints": (extract_turkish_complaints_examples, "Turkish Complaints [Multilingual]"),
    }
    ...
    for name, (extractor, description) in extractors.items():
        ...
        print(f"    Labels: SAT={stats['SAT']}, NEED_CLAR={stats['NEED_CLARIFICATION']}, "
    ...
    for name, stats in all_stats.items():
        ...
        print(f"  {name:12s}: {success:6,}/{stats['total']:6,} ({rate:5.1f}%) [garbage: {garbage}]{status}")
        for key in total:
            if key in stats:
                total[key] += stats[key]
        if aborted:
...
"""
    ...
    parser.add_argument("--download-only", action="store_true", 
                        help="Only download, don't label")
    parser.add_argument("--label-only", action="store_true", 
                        help="Only label (skip download)")
    ...
    # Garbage protection
    parser.add_argument("--garbage-threshold", type=int, default=5,
                        help="Consecutive garbage responses to trigger circuit breaker (default: 5)")
    parser.add_argument("--garbage-max", type=int, default=100,
                        help="Max total garbage before abort, 0=no limit (default: 100)")
    parser.add_argument("--garbage-cooldown", type=float, default=30.0,
