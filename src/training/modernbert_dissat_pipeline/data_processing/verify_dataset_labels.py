    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                logger.debug(f"Could not extract JSON after {max_retries} attempts. Content: {content[:200]}")
...
def verify_single_example(
    ...
    return VerificationResult(
        ...
        confidence=result.get("confidence", "high"),  # Default to high since model is confident
...
def verify_dataset(
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for idx in indices:
            ...
            label = example.get("label_name", ID2LABEL.get(example.get("label", 0), "SAT"))
            ...
            try:
                ...
                if result.is_correct:
                    ...
                    if original in stats.label_stats:
                        ...
                        # Track confusion
                        if predicted not in stats.label_stats[original]["confused_with"]:
...
def print_report(stats: VerificationStats, results: List[VerificationResult]):
    ...
    print(f"   Correct labels: {stats.correct} ({stats.correct/max(1,stats.total)*100:.1f}%)")
    print(f"   Incorrect labels: {stats.incorrect} ({stats.incorrect/max(1,stats.total)*100:.1f}%)")
    print(f"   Uncertain: {stats.uncertain} ({stats.uncertain/max(1,stats.total)*100:.1f}%)")
    ...
    for label, label_stats in stats.label_stats.items():
        ...
        if label_stats["confused_with"]:
            ...
            for confused_label, count in sorted(
                label_stats["confused_with"].items(),
                key=lambda x: -x[1]
            ):
                print(f"         → {confused_label}: {count}")
    
    # Show some examples of incorrect labels
    incorrect_examples = [r for r in results if not r.is_correct and r.confidence != "low"]
...
def main():
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Verify entire dataset (overrides --sample)"
    )
    ...
    parser.add_argument(
        "--correct",
        action="store_true",
        help="Apply corrections to the dataset"
    )
    parser.add_argument(
        "--confidence",
        type=str,
