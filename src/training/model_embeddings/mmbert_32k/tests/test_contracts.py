...
class SourceContractTest(unittest.TestCase):
    ...
    def test_artifact_manifest_maps_each_family_to_local_source(self) -> None:
        manifest = _load_json("artifacts.json")
        self.assertEqual(manifest["owner"], "src/training/model_embeddings/mmbert_32k")
        artifacts = {item["family"]: item for item in manifest["artifacts"]}
        self.assertEqual(set(artifacts), set(TRAINERS))
        for family, item in artifacts.items():
            self.assertEqual(ROOT / item["entrypoint"], TRAINERS[family])
