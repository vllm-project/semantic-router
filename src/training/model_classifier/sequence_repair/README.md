...
For a predeclared fixed training endpoint, pass `--train-only` and omit `--dev`.
This mode never reads development data or evaluates intermediate checkpoints;
it saves only the final `last-model` or `last-adapter` with the usual complete
tensor checks. Development selection options and `--probe-only` cannot be
combined with `--train-only`. Keep later evaluation separate and qualify the
result before publication; training completion alone is not a quality gate.
...
Choose task languages with `--languages` (`en`, `zh`, `es`, `fr`, `de`, `ja`,
or `ar`). Supply background text for each selected language in every split;
task markers use the corresponding language and preserve the original payload.
