...
def task_payload(row):
    """Mark a task in its declared language without rewriting its source text."""
    language = LANGUAGE_ALIASES.get(row["language"], row["language"])
    if language not in TASK_MARKERS:
        raise ValueError(f"No context task marker for language: {language}")
    return TASK_MARKERS[language] + row["text"]
...
def main():
    ...
    parser.add_argument(
        "--languages", nargs="+", choices=sorted(TASK_MARKERS), default=["en", "zh"]
    )
