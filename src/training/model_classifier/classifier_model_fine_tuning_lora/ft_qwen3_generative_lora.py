...
CAUSAL_LOGITS_DIMS = 3
TOKEN_IDS_DIMS = 2
IGNORE_TOKEN_ID = -100
MIN_PREDICTION_WORDS = 2
EVALUATION_PREVIEW_COUNT = 5
...
def compute_metrics_generative(eval_pred, tokenizer, label2id):
    ...
    if len(predictions.shape) == CAUSAL_LOGITS_DIMS:
        # Logits shape: apply argmax to get token IDs
        pred_tokens = np.argmax(predictions, axis=-1)
    elif len(predictions.shape) == TOKEN_IDS_DIMS:
        # Already token IDs
...
def main(
    ...
    for i in range(num_test_samples):
        ...
        if predicted_category == "unknown" and answer_text:
            ...
            if len(words) >= MIN_PREDICTION_WORDS:
        ...
        if (
            i < EVALUATION_PREVIEW_COUNT
            or i >= num_test_samples - EVALUATION_PREVIEW_COUNT
        ):
