...
def _prepare_merge_paths(
    ...
    return run_path, adapter_dir, merged_path


def _load_adapter_for_export(
    torch_module: Any,
    peft_model: Any,
    auto_model: Any,
    auto_tokenizer: Any,
    ...
    dtype, device = _export_runtime(torch_module, use_cpu=use_cpu)
    tokenizer = auto_tokenizer.from_pretrained(adapter_dir, use_fast=True)
    ...
    adapter_model = peft_model.from_pretrained(base_model, adapter_dir)
    ...
    return tokenizer, adapter_model, inputs, dtype
...
def _merge_and_compare(
    ...
    torch_module.testing.assert_close(
        adapter_logits, merged_logits, atol=atol, rtol=rtol
    )
    prediction_match = bool(
        torch_module.equal(
            adapter_logits.argmax(dim=-1),
            merged_logits.argmax(dim=-1),
        )
    )
    ...
    parity = {
        ...
        "max_abs_logit_difference": float(
            (adapter_logits - merged_logits).abs().max().item()
        ),
        "prediction_match": prediction_match,
        "fixtures": len(_representative_texts(task_name)),
    }
    return merged_model, parity


def _save_merged_artifacts(
    merged_model: Any,
    tokenizer: Any,
    run_path: Path,
    adapter_dir: Path,
...
def merge_adapter(
    ...
    run_path, adapter_dir, merged_path = _prepare_merge_paths(run_root, merged_dir)
    tokenizer, adapter_model, inputs, dtype = _load_adapter_for_export(
        torch,
        PeftModel,
        AutoModelForSequenceClassification,
        AutoTokenizer,
    ...
    merged_model, parity = _merge_and_compare(
        torch,
        adapter_model,
        inputs,
        task,
