// Command management-migrate applies the explicit forward-only Management
// schema. Router replicas never run migrations during startup.
package main
...
func main() {
	if err := run(os.Args[1:], os.Stdout); err != nil {
		fmt.Fprintln(os.Stderr, "management-migrate:", err)
		os.Exit(1)
	}
}

func run(args []string, output io.Writer) (returnErr error) {
	flags := flag.NewFlagSet("management-migrate", flag.ContinueOnError)
	...
	_, runErr = fmt.Fprintf(output, "Management schema is current at version %d\n", latest)
	return runErr
}
