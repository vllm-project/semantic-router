...
func (r *OpenAIRouter) modifyRequestBodyForLooper(
	...
	if request == nil {
		return nil, fmt.Errorf("neutral looper request is unavailable")
	}
	...
	if decisionName != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("apply looper system instruction: %w", err)
		}
...
// buildLooperBackendDispatchResponse keeps internal Looper hops on the same
// immutable BackendInvoker boundary as primary requests. The verified grant in
// the request context constrains the logical model; no physical backend or
// provider credential is resolved in ExtProc.
func (r *OpenAIRouter) buildLooperBackendDispatchResponse(
	modelName string,
	modifiedBody []byte,
	ctx *RequestContext,
) *ext_proc.ProcessingResponse {
	response := r.buildBackendDispatchResponse(modelName, modifiedBody, ctx)
	common := response.GetRequestBody().GetResponse()
	if common == nil {
		return r.createErrorResponse(500, "Failed to build backend dispatch")
	}
	...
	return r.finalizeBackendDispatchResponse(modelName, response, ctx)
}
...
func (r *OpenAIRouter) handleLooperInternalRequest(
	...
	modifiedBody, err := r.encodeDispatchRequest(ctx)
	if err != nil {
		return r.createErrorResponse(500, "Failed to encode looper request"), nil
	}
	ctx.VSRSelectedModel = modelName
	ctx.RequestModel = modelName
	return r.buildLooperBackendDispatchResponse(modelName, modifiedBody, ctx), nil
}
