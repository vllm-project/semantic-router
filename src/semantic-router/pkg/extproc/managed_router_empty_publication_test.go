...
func TestManagedRouterRegistryActivatesCanonicalEmptyPublication(t *testing.T) {
	...
	if err := registry.Activate(context.Background(), loaded); err != nil {
		t.Fatalf("activate empty publication: %v", err)
	}
	lease, err := registry.Acquire(managedRouterPin(loaded))
	if err != nil {
