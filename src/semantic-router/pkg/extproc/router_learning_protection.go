...
func (r *OpenAIRouter) applyProtectionSwitch(
	...
	result, ok := r.selectProtectionResultForContext(preflight.config, baseResult, learningCtx, input.ctx)
	if !ok {
...
func (r *OpenAIRouter) selectProtectionResultForContext(
	...
	r.applySwitchGateToResult(cfg, ctx, learningCtx, selector, result, selection.SwitchOriginEscalation)
	return result, true
}
