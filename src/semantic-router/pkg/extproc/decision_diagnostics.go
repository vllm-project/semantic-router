...
func matchedDecisionDiagnosticSignals(ctx *RequestContext) map[string]bool {
	...
	add(config.SignalTypeMetadata, ctx.VSRMatchedMetadata)
	add(config.SignalTypeClassifier, ctx.VSRMatchedClassifier)
	add(config.SignalTypeInputModality, ctx.VSRMatchedInputModality)
