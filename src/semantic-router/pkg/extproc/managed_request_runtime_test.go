...
func TestManagedRequestRuntimeAuthenticatesAndAcquiresExactPublication(t *testing.T) {
	registry, _ := newManagedRouterRegistryForTest(t)
	publication := managedRouterPublicationState(
		managedRouterPublication(t, "namespace-a", "partition-a", 3, 7, "a"),
	...
	runtime, err := NewManagedRequestRuntime(ManagedRequestRuntimeOptions{
...
func TestManagedRequestRuntimeRequiresEveryDependency(t *testing.T) {
	if _, err := NewManagedRequestRuntime(ManagedRequestRuntimeOptions{}); err == nil {
		t.Fatal("empty managed request runtime was accepted")
	}
}

func TestManagedRoutingOnlyAcquiresConfiguredPublicNamespace(t *testing.T) {
	registry, _ := newManagedRouterRegistryForTest(t)
	publication := managedRouterPublicationState(
		managedRouterPublication(t, "11111111-1111-4111-8111-111111111111", "partition-public", 3, 7, "p"),
		accesspublisher.PublicationStateActive,
	)
	if err := registry.Activate(context.Background(), publication); err != nil {
		t.Fatal(err)
