...
func (r *OpenAIRouter) persistResponseObject(ctx *RequestContext) {
	...
	if !state.ShouldStore || !state.Owner.Valid() || ctx.SemanticResponse.Error != nil ||
	...
	stored := &responseapi.StoredResponse{
		Owner: state.Owner, ID: response.ID, Object: "response",
	...
	if err := r.ResponseAPIFilter.store.StoreResponse(ctx.TraceContext, state.Owner, stored); err != nil {
		logResponseObjectPersistenceFailure(ctx, err)
	}
}
