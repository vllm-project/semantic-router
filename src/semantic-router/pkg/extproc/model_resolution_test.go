...
entrypoints:
  - model_names: [router/default]
    ...
    assignments:
      default_route:
        models: [{model: known-model}]
`

func newModelResolutionTestRouter(t *testing.T) (*OpenAIRouter, *config.RouterConfig) {
	t.Helper()
	cfg, err := parseExtProcAuthoringConfig(t, testRouterConfigYAML)
	require.NoError(t, err, "parse test config")
	cfg.DefaultModel = "known-model"
	router := &OpenAIRouter{Config: cfg, DispatchCapabilities: dispatchCapabilityRuntimeStub{}}
	return router, cfg
}

func newModelResolutionTestContext(t *testing.T) *RequestContext {
	t.Helper()
	traceContext, err := routingcontext.WithGeneration(context.Background(), routingcontext.Generation{
		NamespaceID: "test-namespace", QuotaPartition: "test-quota",
