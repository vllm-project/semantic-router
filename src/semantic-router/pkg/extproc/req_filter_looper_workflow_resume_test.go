...
func TestRestoreLooperWorkflowTrace_FailedModelsWithoutIntermediates(t *testing.T) {
	t.Parallel()

	buffered := []byte(`{"id":"c","object":"chat.completion","flow":{"failed_models":[{"model":"worker-model","error":"timeout"}]},"choices":[{"index":0,"message":{"role":"assistant","content":"hi"}}]}`)
	sse := []byte("data: {\"id\":\"c\",\"object\":\"chat.completion.chunk\",\"flow\":{\"failed_models\":[{\"model\":\"worker-model\",\"error\":\"timeout\"}]},\"choices\":[{\"index\":0,\"delta\":{\"role\":\"assistant\"}}]}\n\ndata: [DONE]\n")
	for _, tc := range []struct {
		name string
		body []byte
		sse  bool
	}{
		{name: "buffered", body: buffered},
		{name: "sse", body: sse, sse: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			stripped, flow := isolateLooperWorkflowFlow(tc.body)
			...
			decision := workflowTestDecisionWithIntermediateResponses(false)
			restored := restoreLooperWorkflowTrace(stripped, flow, &RequestContext{VSRSelectedDecision: &decision})
			parsed, ok := workflowClientFlow(restored)
			if !ok {
				t.Fatalf("failed_models trace dropped: %s", restored)
			}
			failed, _ := parsed["failed_models"].([]interface{})
			if len(failed) != 1 {
				t.Fatalf("failed_models = %#v", parsed["failed_models"])
			}
			if tc.sse && !strings.Contains(string(restored), `"delta"`) {
				t.Fatalf("SSE lost completion payload: %s", restored)
			}
		})
	}
}
