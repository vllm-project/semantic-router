...
func (r *OpenAIRouter) handleRouterReplayRecordAPI(method string, replayID string, rawQuery string) *ext_proc.ProcessingResponse {
	...
	return r.createRouterReplayJSONResponse(200, publicRouterReplayRecord(record))
}
...
func buildRouterReplayListPayload(
	...
	page := make([]routerreplay.RoutingRecord, end-offset)
	for i, record := range records[offset:end] {
		if !query.showDetails {
			record = routerreplay.ListSummaryRecord(record)
		}
		page[i] = publicRouterReplayRecord(record)
	}
...
func publicRouterReplayRecord(record routerreplay.RoutingRecord) routerreplay.RoutingRecord {
	record.RoutingScope = ""
	return record
}
