...
type publicationCoordinator interface {
	accesspublisher.RuntimeStore
	publicationreplica.Store
}
...
func composePublicationPipeline(
	...
	postgresDSN string,
	nativeAccess bool,
) (accesspublisher.Processor, *accesspublisher.Worker, publicationCoordinator, error) {
	outboxOptions := accesspublisher.PostgresStoreOptions{Projector: publicationProjectorID}
	if !nativeAccess {
		outboxOptions.NotificationDSN = postgresDSN
	}
	outbox, err := accesspublisher.NewPostgresStore(database, outboxOptions)
	if err != nil {
	...
	var applied publicationCoordinator = outbox
	if nativeAccess {
