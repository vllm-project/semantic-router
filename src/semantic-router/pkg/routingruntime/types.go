...
// ManagementAPI is mounted on the Router's existing Management listener. Its
...
// identity, authorization, and domain route composition. Durable startup must
// fail when it is absent or returns nil; no allow-all or Dashboard fallback is
// permitted.
type ManagementFactory interface {
	Build(context.Context, ManagementDependencies) (ManagementAPI, error)
}

type ManagementFactoryFunc func(context.Context, ManagementDependencies) (ManagementAPI, error)
