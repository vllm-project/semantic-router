// Package routingruntime owns process-wide resources composed from configured
// stores and services. It deliberately contains no Dashboard integration.
...
const routingHKDFSalt = "vllm-sr/routing-runtime/hkdf-sha256/v1"
...
var routingHMACDomains = []HMACDomain{
...
func (keyring HMACKeyring) activeKey() (string, []byte, error) {
	...
	if !found || len(key) != derivedHMACKeyBytes {
		return "", nil, errors.New("active routing HMAC key is unavailable")
	}
...
// RoutingKeyrings are independent HKDF domains derived from one dedicated
// deployment root. Root bytes never leave construction and are not reused as
// an application HMAC key.
type RoutingKeyrings struct {
...
func deriveRoutingKeyrings(root securitykeyring.Symmetric) (RoutingKeyrings, error) {
	if strings.TrimSpace(root.ActiveVersion) == "" || len(root.Keys) == 0 {
		return RoutingKeyrings{}, errors.New("routing HMAC root keyring is empty")
	}
	if _, found := root.Keys[root.ActiveVersion]; !found {
		return RoutingKeyrings{}, errors.New("active routing HMAC root version is unavailable")
	}
	byDomain := make(map[HMACDomain]HMACKeyring, len(routingHMACDomains))
	for _, domain := range routingHMACDomains {
		...
		for version, rootKey := range root.Keys {
			if len(rootKey) < derivedHMACKeyBytes {
				return RoutingKeyrings{}, fmt.Errorf("routing HMAC root %q is weaker than 256 bits", version)
			}
			info := []byte("vllm-sr/routing-runtime/" + string(domain) + "/v1\x00" + version)
			keyring.keys[version] = hkdfSHA256(rootKey, []byte(routingHKDFSalt), info, derivedHMACKeyBytes)
		}
		byDomain[domain] = keyring
	}
	return RoutingKeyrings{
...
func (keyrings RoutingKeyrings) clone() RoutingKeyrings {
	return RoutingKeyrings{
...
func (keyrings *RoutingKeyrings) zero() {
	if keyrings == nil {
		return
	}
...
func (keyrings DeploymentKeyrings) clone() DeploymentKeyrings {
	cloned := DeploymentKeyrings{
		...
		Routing: keyrings.Routing.clone(),
	}
