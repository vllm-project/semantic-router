...
func TestReplicaColdStartCoordinatesTwoReplicasAndRestart(t *testing.T) {
	...
	newReplica := func(replicaID string, memberships []providercatalog.RolloutGroup) *Replica {
		...
		replica, err := NewReplica(barrier, registry, ReplicaOptions{
			ReplicaID: replicaID, RolloutGroups: memberships, RequiredRolloutGroups: groups,
	...
	first := newReplica("router-cold-a", groups[:1])
	second := newReplica("router-cold-b", groups[1:])
...
WHERE revision = $1 AND replica_id IN ('router-cold-a', 'router-cold-b')`, wantRevision).Scan(&acknowledgements); err != nil {
	...
	restartedCoordinator, err := catalogpostgres.New(db, registry)
	if err != nil {
		t.Fatal(err)
	}
	restarted, err := NewReplica(restartedCoordinator, registry, ReplicaOptions{
	...
	if err := restarted.EnsureColdStart(ctx); err != nil {
		t.Fatal(err)
	}
...
func TestApplicationComposesInstalledRegistriesAndManagementServer(t *testing.T) {
	db := managedCatalogIntegrationDatabase(t)
	discovery, testApplicationComposesInstalledRegistriesAndManagementServerErr := providerdiscovery.BuiltinRegistry()
	if testApplicationComposesInstalledRegistriesAndManagementServerErr != nil {
		t.Fatal(testApplicationComposesInstalledRegistriesAndManagementServerErr)
	}
	policy, testApplicationComposesInstalledRegistriesAndManagementServerErr := backendegress.Compile(backendegress.Config{
	...
	claims, testApplicationComposesInstalledRegistriesAndManagementServerErr := providerdiscovery.NewClaimCodec(providerdiscovery.ClaimKeyset{
		ActiveKeyID: "active", Keys: map[string][]byte{"active": bytes.Repeat([]byte{'c'}, 32)},
	})
	if testApplicationComposesInstalledRegistriesAndManagementServerErr != nil {
		t.Fatal(testApplicationComposesInstalledRegistriesAndManagementServerErr)
	}
	application, testApplicationComposesInstalledRegistriesAndManagementServerErr := NewApplication(ApplicationOptions{
