	...
	for range coldStartTransitions {
		...
		case state.ActiveRevision == installedRevision:
			if err := r.Reconcile(ctx); err != nil {
				return fmt.Errorf("reconcile application-installed Provider Catalog: %w", err)
			}
			return nil
		case state.ActiveRevision != "":
			return nil
		case state.DesiredRevision == "":
			if _, err := r.BootstrapRegistry(ctx, state.Generation); err != nil {
				if errors.Is(err, providercatalog.ErrPublicationConflict) {
					continue
				}
				return fmt.Errorf("stage application-installed Provider Catalog: %w", err)
			}
			continue
		...
		if err := r.Reconcile(ctx); err != nil {
			return fmt.Errorf("acknowledge application-installed Provider Catalog: %w", err)
		}
		state, err = r.coordinator.State(ctx)
		if err != nil {
