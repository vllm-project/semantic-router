...
type routingHTTPFixture struct {
	ctx          context.Context
	db           *sql.DB
	namespaceID  string
	partition    string
...
func TestRoutingHTTPPublishesExactClosureFromPostgresToRedis(t *testing.T) {
	fixture := newRoutingHTTPFixture(t)
	publishRoutingHTTPResources(t, fixture)
	assertRoutingHTTPProjection(t, fixture)
	assertRoutingHTTPDurability(t, fixture)
}

func newRoutingHTTPFixture(t *testing.T) routingHTTPFixture {
	t.Helper()
	...
	egress, err := backendegress.Compile(backendegress.Config{
	...
	application, err := NewApplication(ApplicationOptions{
		...
		ModelCompiler: providercatalog.ModelCompiler{
			...
			Registry: registry, Egress: egress,
		},
		ValidatePublication: config.ValidateDurableRoutingSnapshot,
		CommandCodec:        commandCodec, IdempotencyTTL: time.Hour,
		Namespaces: managementserver.ExplicitNamespaceResolver{},
		Sessions:   routingHTTPSession{}, Authorization: routingHTTPAuthorization{},
	...
	return routingHTTPFixture{
		ctx: ctx, db: db, namespaceID: namespaceID, partition: partition,
		redisAddress: redisAddress, serverURL: server.URL,
	}
}

func publishRoutingHTTPResources(t *testing.T, fixture routingHTTPFixture) {
	t.Helper()
	routingHTTPMutation(t, fixture.serverURL, fixture.namespaceID, "/management/v1/routing/models", "model_one", `{
  "id":"model_one","name":"Model One",
  "control":{"retry":{"count":2,"on":["unavailable"]},"timeout":{"request":"30s","stream":"2m"}},
  ...
  "backends":[{"providerId":"provider_one","providerModelId":"upstream/model-one"}]
	...
	routingHTTPMutation(t, fixture.serverURL, fixture.namespaceID, "/management/v1/routing/recipes", "recipe_one", `{
  "id":"recipe_one","name":"Recipe One",
  "document":{"signals":{},"projections":{},"decisions":[{"name":"Simple","rules":{}}]}
}`)
	decisionID := routingHTTPRecipeDecisionID(t, fixture.serverURL, fixture.namespaceID, "recipe_one")
	routingHTTPMutation(t, fixture.serverURL, fixture.namespaceID, "/management/v1/routing/entrypoints", "entrypoint_one", `{
  "id":"entrypoint_one","name":"Entrypoint One","aliases":["vllm-sr/one"],
  "rules":[{"id":"rule_one","name":"Default","recipeId":"recipe_one","assignments":{"`+decisionID+`":{"models":[{"modelId":"model_one","weight":"1"}]}}}]
}`)
	request, err := http.NewRequest(
		http.MethodPost,
		fixture.serverURL+"/management/v1/routing/entrypoints/entrypoint_one:publish",
		nil,
	)
	if err != nil {
		t.Fatal(err)
	}
	routingHTTPHeaders(request, fixture.namespaceID, "publish-entrypoint-one")
	request.Header.Set(managementapi.HeaderIfMatch, `"ep:1"`)
	...
	if response.StatusCode != http.StatusAccepted || !strings.Contains(string(body), `"desiredRevision":1`) {
		t.Fatalf("publish status = %d, body = %s", response.StatusCode, body)
	}
}

func assertRoutingHTTPProjection(t *testing.T, fixture routingHTTPFixture) {
	t.Helper()
	client := redis.NewClient(&redis.Options{Addr: fixture.redisAddress})
	if err := client.Ping(fixture.ctx).Err(); err != nil {
		t.Fatal(err)
	}
	prefix := "routing-http-it:" + strings.ReplaceAll(uuid.NewString(), "-", "")
	t.Cleanup(func() {
		deleteRoutingHTTPRedisPrefix(client, prefix+":*")
		_ = client.Close()
	})
	runtimeStore, err := accesspublisher.NewRedisStore(accesspublisher.RedisStoreOptions{
		Client: client, KeyPrefix: prefix, ReplicaLease: 20 * time.Second,
	})
	if err != nil {
		t.Fatal(err)
	}
	outbox, err := accesspublisher.NewPostgresStore(
		fixture.db, accesspublisher.PostgresStoreOptions{Projector: "routing-http-it"},
	)
	if err != nil {
		t.Fatal(err)
	}
	desired, err := accesspublisher.NewPostgresDesiredStateReader(fixture.db)
	if err != nil {
		t.Fatal(err)
	}
	...
	result, err := engine.ProcessOnce(fixture.ctx)
	if err != nil || result.Disposition != accesspublisher.ProcessApplied || result.Revision != 1 {
		t.Fatalf("ProcessOnce() = %+v, %v", result, err)
	}
	readiness, err := runtimeStore.Readiness(fixture.ctx, fixture.namespaceID, fixture.partition)
	if err != nil || !readiness.Ready || readiness.AppliedRevision != 1 ||
		readiness.AccessGate == "" || readiness.AccessGate != readiness.RoutingGate {
		t.Fatalf("published readiness = %+v, %v", readiness, err)
	}
}

func assertRoutingHTTPDurability(t *testing.T, fixture routingHTTPFixture) {
	t.Helper()
...
FROM routing_snapshot_members WHERE namespace_id=$1 AND routing_revision=1`, fixture.namespaceID).
	...
	if err := fixture.db.QueryRowContext(
		fixture.ctx, `SELECT count(*) FROM access_audit_events WHERE namespace_id=$1`, fixture.namespaceID,
	).Scan(&auditCount); err != nil {
		t.Fatal(err)
	}
	if err := fixture.db.QueryRowContext(
		fixture.ctx, `SELECT count(*) FROM management_idempotency WHERE namespace_id=$1`, fixture.namespaceID,
	).Scan(&commandCount); err != nil {
		t.Fatal(err)
	}
	if err := fixture.db.QueryRowContext(
		fixture.ctx, `SELECT count(*) FROM policy_outbox WHERE namespace_id=$1`, fixture.namespaceID,
...
func routingHTTPRecipeDecisionID(t *testing.T, baseURL, namespaceID, recipeID string) string {
	t.Helper()
	request, err := http.NewRequest(http.MethodGet, baseURL+"/management/v1/routing/recipes/"+recipeID, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	defer response.Body.Close()
	var detail managementapi.RoutingRecipeDetail
	if response.StatusCode != http.StatusOK {
		payload, _ := io.ReadAll(response.Body)
		t.Fatalf("GET Recipe status = %d, body = %s", response.StatusCode, payload)
	}
	if err := json.NewDecoder(response.Body).Decode(&detail); err != nil {
		t.Fatal(err)
	}
	if len(detail.Data.Decisions) != 1 || detail.Data.Decisions[0].ID == "" {
		t.Fatalf("Recipe decisions = %#v", detail.Data.Decisions)
	}
	return detail.Data.Decisions[0].ID
}
