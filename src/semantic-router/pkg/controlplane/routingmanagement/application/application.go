// Package application composes Router-native Model, Recipe, and Entrypoint
...
type ApplicationOptions struct {
	...
	Now                      func() time.Time
	ManifestCodec            routingmanagement.ManifestCodec
}
...
func NewApplication(options ApplicationOptions) (*Application, error) {
	if options.DB == nil || options.ValidatePublication == nil || options.CommandCodec == nil || options.Namespaces == nil ||
		...
		return nil, fmt.Errorf("durable Routing application requires database, publication validation, command, namespace, session, and authorization dependencies")
	}
	...
	service, err := routingmanagement.NewService(routingmanagement.ServiceOptions{
		...
		ManifestCodec: options.ManifestCodec,
	})
	if err != nil {
		return nil, err
	}
...
// ReconcileBuiltInRecipes is the startup gate. The Management listener is not
...
func (application *Application) ReconcileBuiltInRecipes(ctx context.Context) error {
	if application == nil || application.BuiltInRecipes == nil {
		return errors.New("durable built-in Recipes are unavailable")
	}
...
func (application *Application) Register(mux *http.ServeMux) {
	if application == nil || application.Routes == nil {
		panic("durable Routing application is unavailable")
	}
...
func (application *Application) Ready(ctx context.Context) error {
	if application == nil || application.db == nil || application.Routes == nil {
		return errors.New("durable Routing application is unavailable")
	}
	if err := application.db.PingContext(ctx); err != nil {
		return fmt.Errorf("durable Routing PostgreSQL is unavailable: %w", err)
	}
...
func (results postgresCommandResults) LookupManifestDiff(
	ctx context.Context, namespaceID, operationID string,
) (routingmanagement.ManifestDiff, error) {
	var targetScope []byte
	if err := results.db.QueryRowContext(ctx, `SELECT target_scope FROM management_operations
