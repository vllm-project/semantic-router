...
const maximumRoutingRecipeDocumentBytes = 2 << 20

// RoutingRecipeDocument is the strict model-free Recipe source stored beside
...
type RoutingRecipeDocument struct {
...
func ParseRoutingRecipeDocument(document json.RawMessage) (RoutingRecipeDocument, json.RawMessage, error) {
	if len(document) == 0 || len(document) > maximumRoutingRecipeDocumentBytes || !json.Valid(document) {
		return RoutingRecipeDocument{}, nil, fmt.Errorf("routing Recipe document must be valid JSON no larger than %d bytes", maximumRoutingRecipeDocumentBytes)
	}
	decoder := json.NewDecoder(bytes.NewReader(document))
	decoder.UseNumber()
	var root map[string]any
	if err := decoder.Decode(&root); err != nil || root == nil {
		return RoutingRecipeDocument{}, nil, fmt.Errorf("routing Recipe document must be a JSON object")
	}
	if err := requireRoutingRecipeEOF(decoder); err != nil {
		return RoutingRecipeDocument{}, nil, err
	}
	...
	for field := range root {
		if _, ok := allowed[field]; !ok {
			return RoutingRecipeDocument{}, nil, fmt.Errorf("routing Recipe document contains unsupported field %q", field)
		}
	}
	var parsed RoutingRecipeDocument
	if err := yaml.UnmarshalStrict(document, &parsed); err != nil {
		return RoutingRecipeDocument{}, nil, fmt.Errorf("decode routing Recipe document: %w", err)
	}
	if len(parsed.Decisions) > 256 {
		return RoutingRecipeDocument{}, nil, fmt.Errorf("routing Recipe must define at most 256 decisions")
	}
	for index := range parsed.Decisions {
		if parsed.Decisions[index].ID != "" {
			return RoutingRecipeDocument{}, nil, fmt.Errorf(
				"routing Recipe decisions[%d].id is compiler-owned; use the Decision name in authoring documents",
				index,
			)
		}
	}
	if err := validateRecipeDocumentModelFree(CanonicalRoutingFromRecipeDocument(parsed)); err != nil {
		return RoutingRecipeDocument{}, nil, fmt.Errorf("routing Recipe document: %w", err)
	}
	canonical, err := json.Marshal(root)
	if err != nil {
		return RoutingRecipeDocument{}, nil, fmt.Errorf("canonicalize routing Recipe document: %w", err)
	}
	return parsed, canonical, nil
}

func requireRoutingRecipeEOF(decoder *json.Decoder) error {
	var trailing any
	if err := decoder.Decode(&trailing); err != io.EOF {
		if err == nil {
			return fmt.Errorf("routing Recipe document contains trailing JSON")
		}
		return fmt.Errorf("decode routing Recipe document trailer: %w", err)
	}
	return nil
}

// CompileDurableRoutingSnapshot applies one already-compiled publication to a
// bootstrap config. It never converts strict resources back through the human
// authoring DTOs.
func CompileDurableRoutingSnapshot(
	base *RouterConfig,
	...
	if base.AccessStore == nil {
		return nil, fmt.Errorf("durable routing snapshot requires a durable Management store")
	}
	if snapshot == nil {
		return nil, fmt.Errorf("durable routing snapshot is required")
	}
	verified, err := verifyDurableRoutingSnapshot(snapshot)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("clone durable routing bootstrap: %w", err)
	}
	if err := applyRoutingSnapshotState(compiled, verified); err != nil {
		return nil, fmt.Errorf("compile durable routing snapshot: %w", err)
	}
	compiled.ConfigBaseDir = base.ConfigBaseDir
	compiled.DocumentHash = verified.SemanticDigest
	compiled.SkipExternalAssetValidation = base.SkipExternalAssetValidation
	if err := finalizeParsedConfig(compiled); err != nil {
		return nil, fmt.Errorf("validate durable routing snapshot: %w", err)
	}
	return compiled, nil
}

// ValidateDurableRoutingSnapshot validates the complete routing contract
...
// bootstrap compiler is unavailable. Production durable composition validates
// with CompileDurableRoutingSnapshot so bootstrap-specific contracts are also
// enforced before a desired revision is committed.
func ValidateDurableRoutingSnapshot(snapshot *routingsnapshot.Snapshot) error {
	verified, err := verifyDurableRoutingSnapshot(snapshot)
	if err != nil {
		return err
	}
	...
	if err := applyRoutingSnapshotState(&validation, verified); err != nil {
		return fmt.Errorf("materialize durable routing snapshot: %w", err)
	}
	if err := validateConfigStructure(&validation); err != nil {
		return fmt.Errorf("validate durable routing snapshot: %w", err)
	}
	return nil
}

func verifyDurableRoutingSnapshot(snapshot *routingsnapshot.Snapshot) (*routingsnapshot.Snapshot, error) {
	if snapshot == nil {
		return nil, fmt.Errorf("durable routing snapshot is required")
	}
	verified, err := routingsnapshot.Compile(snapshot.Bundle)
	if err != nil {
		return nil, fmt.Errorf("verify durable routing snapshot: %w", err)
	}
	if snapshot.Digest == "" || snapshot.Digest != verified.Digest ||
		snapshot.SemanticDigest == "" || snapshot.SemanticDigest != verified.SemanticDigest {
		return nil, fmt.Errorf("durable routing snapshot digest mismatch")
	}
	return verified, nil
}
	...
	for _, model := range models {
		...
		cfg.ModelConfig[model.Name] = ModelParams{
			...
			Execution: ModelExecutionSettings{
				MaxRetries: model.Execution.MaxRetries, RetryOn: append([]string(nil), model.Execution.RetryOn...),
...
func applySnapshotRecipes(cfg *RouterConfig, recipes []routingsnapshot.Recipe) error {
	...
	for _, recipe := range recipes {
		...
		decisions, err := materializeRoutingRecipeDecisions(recipe, document.Decisions)
		if err != nil {
			return err
		}
...
func materializeRoutingRecipeDecisions(
