...
func ValidateConversationRuleContract(rule ConversationRule) error {
	...
	if !validConversationSourceTypes[rule.Feature.Source.Type] {
		return fmt.Errorf("unsupported feature.source.type %q; valid types: message, tool_definition, assistant_tool_call, assistant_tool_cycle, active_tool_loop, flow_tool_state, image_content", rule.Feature.Source.Type)
	}
