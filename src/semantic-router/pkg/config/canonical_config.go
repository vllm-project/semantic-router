...
type CanonicalSignals struct {
	...
	// ResponseJailbreak is observed after the model answers; see SignalStageOf.
	ResponseJailbreak []ResponseJailbreakRule `yaml:"response_jailbreak,omitempty"`
}
...
func normalizeSignals(signals CanonicalSignals, decisions []Decision) Signals {
	result := Signals{
		...
		ResponseJailbreakRules: append([]ResponseJailbreakRule(nil), signals.ResponseJailbreak...),
	}
