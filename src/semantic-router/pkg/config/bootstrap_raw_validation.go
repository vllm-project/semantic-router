...
// rejectBootstrapSecretLiterals runs on the raw document before environment
...
func rejectBootstrapSecretLiterals(raw map[string]interface{}) error {
	...
	checks := []literalFieldCheck{
		{path: "global.stores.management.postgres", node: nestedMapAt(stores, "management", "postgres"), fields: []string{"dsn", "database_url", "connection_string", "password"}},
		{path: "global.stores.runtime.redis", node: nestedMapAt(stores, "runtime", "redis"), fields: []string{"url", "redis_url", "password"}},
		...
		{path: "global.services.management_api.auth", node: nestedMapAt(services, "management_api", "auth"), fields: []string{"token_signing_keyring", "service_account_hmac_keyring", "invitation_hmac_keyring", "response_kek_keyring"}},
		{path: "global.services.routing_security", node: nestedMapAt(services, "routing_security"), fields: []string{"hmac_keyring"}},
...
func validateBootstrapFieldNames(raw map[string]interface{}) error {
	...
	checks := []struct {
		...
		{"global.stores.runtime.redis", nestedMapAt(global, "stores", "runtime", "redis"), []string{"url_file", "url_env", "key_prefix"}},
		...
		{"global.services.management_api", nestedMapAt(global, "services", "management_api"), []string{"enabled", "bind_address", "port", "remote_exposure", "auth", "tls"}},
		{"global.services.management_api.auth", nestedMapAt(global, "services", "management_api", "auth"), []string{"mode", "tokens", "roles", "token_signing_keyring_file", "token_signing_keyring_env", "service_account_hmac_keyring_file", "service_account_hmac_keyring_env", "invitation_hmac_keyring_file", "invitation_hmac_keyring_env", "response_kek_keyring_file", "response_kek_keyring_env", "bootstrap", "recovery"}},
		{"global.services.routing_security", nestedMapAt(global, "services", "routing_security"), []string{"hmac_keyring_file", "hmac_keyring_env"}},
