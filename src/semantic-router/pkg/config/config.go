...
type EmbeddingModels struct {
	...
	// EmbeddingClassifier optimization settings
	// These settings control the preloading and HNSW indexing for embedding-based classification
	EmbeddingClassifierOptimization EmbeddingClassifierOptimization `yaml:"embedding_classifier_optimization,omitempty"`
}

// EmbeddingClassifierOptimization contains settings for optimizing the embedding classifier
// by preloading candidate embeddings at startup and using HNSW for fast similarity search
type EmbeddingClassifierOptimization struct {
...
func (c EmbeddingClassifierOptimization) WithDefaults() EmbeddingClassifierOptimization {
	result := c
