...
const fileRoutingRevision int64 = 1

// CompileFileRoutingSnapshot is the human-authoring to strict runtime
// boundary. It resolves readable names and Provider Integrations once, then
// returns the same immutable snapshot consumed by dynamic publication.
func CompileFileRoutingSnapshot(
	canonical CanonicalConfig,
	connectionCompiler modelauthoring.ConnectionCompiler,
) (*routingsnapshot.Snapshot, error) {
	bundle, err := buildFileAuthoringBundle(&canonical)
	if err != nil {
		return nil, err
	}
	return compileFileAuthoringBundle(bundle, &canonical, connectionCompiler)
}

func compileFileAuthoringBundle(
	authoring fileAuthoringBundle,
	canonical *CanonicalConfig,
	connectionCompiler modelauthoring.ConnectionCompiler,
) (*routingsnapshot.Snapshot, error) {
	models, modelsByName, err := compileAuthoringModels(context.Background(), authoring.Models, connectionCompiler)
	if err != nil {
		return nil, err
	}
	recipes, recipesByName, err := compileAuthoringRecipes(authoring.Recipes)
	if err != nil {
		return nil, err
	}
	entrypoints, err := compileAuthoringEntrypoints(authoring.Entrypoints, recipesByName, modelsByName)
	if err != nil {
		return nil, err
	}
	...
	}{authoring.Models, authoring.Recipes, authoring.Entrypoints})
	if err != nil {
		return nil, fmt.Errorf("encode file routing identity: %w", err)
	}
	currency, _ := canonicalBillingCurrency(canonical)
	bundle := routingsnapshot.Bundle{
		NamespaceID: uuid.NewSHA1(
			uuid.NameSpaceOID,
			append([]byte("vllm-sr/file-routing/v1\x00"), identityPayload...),
		).String(),
		Revision: fileRoutingRevision, Currency: currency,
		Models: models, Recipes: recipes, Entrypoints: entrypoints,
	}
	verified, err := routingsnapshot.Compile(bundle)
	if err != nil {
		return nil, fmt.Errorf("compile file routing snapshot: %w", err)
	}
	return verified, nil
}
...
func compileAuthoringRecipes(
	...
	for _, source := range sources {
		...
		documentJSON, err := MarshalRoutingRecipeDocument(document)
		if err != nil {
