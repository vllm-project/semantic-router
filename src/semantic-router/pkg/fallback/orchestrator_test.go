...
func TestOrchestrator(t *testing.T) {
	...
	t.Run("successful fallback on attempt 2 after 503 on primary", func(t *testing.T) {
		...
		cand, err := orch.SelectNextCandidate(exec, eligibleRefs, nil)
		if err != nil {
			t.Fatalf("expected next candidate, got error: %v", err)
		}
		if cand.Model != "model-fallback-1" {
			t.Errorf("expected model-fallback-1, got %s", cand.Model)
		}
	...
	t.Run("budget limit: max attempts exceeded", func(t *testing.T) {
		...
		_, err := tightOrch.SelectNextCandidate(exec, eligibleRefs, nil)
		if err != nil {
	...
	t.Run("candidate exhaustion", func(t *testing.T) {
		...
		cand, err := orch.SelectNextCandidate(exec, eligibleRefs, nil)
		if cand != nil {
			t.Errorf("expected no candidate, got %v", cand)
		}
	...
	t.Run("circuit breaker integration: skip open backend candidate to next", func(t *testing.T) {
		...
		cand, err := orchWithCB.SelectNextCandidate(exec, eligibleRefs, resolver)
		if err != nil {
			t.Fatalf("unexpected select error: %v", err)
		}
		if cand.Model != "model-fallback-2" {
			t.Errorf("expected candidate model-fallback-2 (skipping open model-fallback-1), got %s", cand.Model)
		}
	})
	...
	t.Run("candidate is skipped when backendResolver returns error", func(t *testing.T) {
		...
		cand, err := orch.SelectNextCandidate(exec, eligibleRefs, errResolver)
		if err != nil {
			t.Fatalf("unexpected select error: %v", err)
		}
		if cand.Model != "model-fallback-2" {
			t.Errorf("expected model-fallback-2 (skipping model-fallback-1 on resolver error), got %s", cand.Model)
		}
	})
	...
	t.Run("RequestContext creates context with TotalTimeout", func(t *testing.T) {
		reqCtx, reqCancel := orch.RequestContext(context.Background())
		defer reqCancel()
		...
		expected := time.Now().Add(orch.Policy().TotalTimeout)
		diff := deadline.Sub(expected)
		if diff < -time.Second || diff > time.Second {
			t.Errorf("expected deadline close to %v, got %v", expected, deadline)
		}
	})
