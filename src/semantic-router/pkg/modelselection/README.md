...
│                                                                     │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────────────┐  │
│  │ MMLU/Custom  │───▶│  Benchmark   │───▶│ benchmark_training   │  │
│  │   Queries    │    │   Runner     │    │    _data.jsonl       │  │
│  └──────────────┘    └──────────────┘    └──────────────────────┘  │
│                             │                       │               │
│                             ▼                       ▼               │
│                      ┌─────────────┐        ┌─────────────┐        │
...
├── README.md                    # This file
├── selector.go                  # Core algorithm implementations
├── trainer.go                   # Training logic and record generation
├── persistence.go               # JSON serialization/deserialization
├── selector_load_test.go        # Unit and integration tests
└── data/
    ├── training_data_with_category.jsonl  # Source queries with categories
    ├── benchmark_training_data.jsonl                 # Generated benchmark data
    └── trained_models/
...
│                                                                     │
│  embedding = [0.0234, -0.1456, 0.0821, ..., 0.0512]  ← 768 dims    │
└─────────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
...
> ⚠️ **IMPORTANT:** Real Qwen3 embeddings are required for both training and production.
...
> ✅ **Verify Candle is working:** If you see warnings like `"Candle embedding failed, falling back to hash-based"`,
...
- `--query-limit`: Number of queries to benchmark (×4 models = total API calls)
...
|------|-----------|-----------------|----------|
| (default) | Qwen3 | ✅ Yes | Production training |
| `--embedding-model gemma` | Gemma | ✅ Yes | Faster training |
| `--use-fallback` | Hash-based | ❌ No | Testing/development |
...
4. Build Feature Vector
   └── embedding + one_hot(category) = 782 dims
                    │
                    ▼
