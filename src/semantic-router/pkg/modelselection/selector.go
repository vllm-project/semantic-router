...
func (s *KNNSelector) trainMLBinding() {
	...
	records := make([]ml_binding.KNNTrainingRecord, len(training))
...
// Now passes quality and latency for quality-based training (filters low-quality samples)
func (s *SVMSelector) trainMLBinding() {
	...
	records := make([]ml_binding.SVMTrainingRecord, len(training))
	for i, rec := range training {
