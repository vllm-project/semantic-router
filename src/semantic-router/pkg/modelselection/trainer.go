...
type Trainer struct {
	// EmbeddingDim is the dimension of query embeddings (e.g., 768)
	EmbeddingDim int
...
func NewTrainer(embeddingDim int) *Trainer {
	return &Trainer{
		...
		FeatureDim:     embeddingDim + len(VSRCategories), // 768 + 14 = 782
...
func (t *Trainer) ConvertToTrainingRecords() []TrainingRecord {
	...
	for i, rd := range t.RoutingData {
		...
		// This creates the full 782-dimension feature vector
