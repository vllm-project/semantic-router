...
func TestFusionLooperExecutesPanelJudgeAndFinal(t *testing.T) {
	...
	require.Equal(t, "application/json", responseContentTypeForTest(resp))
	...
	assert.Equal(t, "judge", fusionTrace.JudgeModel)
}
...
func TestFusionLooperRecordsPartialPanelFailures(t *testing.T) {
	...
	fusionTrace, ok := resp.IntermediateResponses.(*FusionTrace)
	require.True(t, ok)
	require.Len(t, fusionTrace.FailedModels, 1)
	assert.Equal(t, "panel-b", fusionTrace.FailedModels[0].Model)
}
...
func TestFusionLooperPanelQuorumSkipsSlowWorker(t *testing.T) {
	...
	resp, err := NewFusionLooper(&config.LooperConfig{Endpoint: server.URL}).Execute(context.Background(), req)
	require.NoError(t, err)
	assert.Less(t, time.Since(started), 2*time.Second, "quorum must not wait for the slow worker")
	assert.Equal(t, "final from quorum panel", semanticTextForTest(t, resp))
...
func TestFusionPanelCollectorAccountsDroppedCompletedResultAfterQuorum(t *testing.T) {
	collector := newFusionPanelCollector(fusionExecutionConfig{
		AnalysisModels:         []string{"panel-a", "panel-b", "panel-c"},
		MinSuccessfulResponses: 1,
	}, func() {})

	collector.handleResult(fusionPanelResult{
		index: 0,
		model: "panel-a",
		resp:  &ModelResponse{Model: "panel-a", Usage: TokenUsage{TotalTokens: 7}},
	})
	queued := make(chan fusionPanelResult, 2)
	queued <- fusionPanelResult{
		index: 1,
		model: "panel-b",
		resp:  &ModelResponse{Model: "panel-b", Usage: TokenUsage{TotalTokens: 11}},
	}
	remaining := 2 // one queued completion plus one still-running worker
	drainFusionPanelResults(queued, collector, &remaining)

	require.Len(t, collector.responses(), 1, "quorum output must remain frozen")
	assert.Equal(t, 1, remaining, "non-blocking drain must leave the slow worker outstanding")
	accountingResponses := collector.execution().accountingResponses
	require.Len(t, accountingResponses, 2, "completed sibling must remain available for accounting")
	assert.Equal(t, int64(18), SumUsage(accountingResponses...).TotalTokens)
}
