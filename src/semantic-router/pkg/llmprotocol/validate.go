...
func validateSampling(sampling Sampling, limits Limits) error {
	...
	if limits.StopSequences > 0 && len(sampling.Stop) > limits.StopSequences {
		return NewError(ErrorInvalidRequest, "stop_limit", "too many stop sequences", nil)
	}
	stopBytes := 0
	for _, stop := range sampling.Stop {
		if stop == "" {
...
func recordToolLink(content Content, calls, results map[string]struct{}, retainedHistory bool) error {
	if content.Kind == ContentToolCall && content.ToolCall != nil {
		if _, resultAlreadySeen := results[content.ToolCall.ID]; resultAlreadySeen {
			return NewError(ErrorInvalidRequest, "tool_result_order", "tool result must follow its request tool call", nil)
		}
		if _, duplicate := calls[content.ToolCall.ID]; duplicate {
	...
	if content.Kind == ContentToolResult && content.ToolResult != nil {
		...
		results[content.ToolResult.CallID] = struct{}{}
	}
	return nil
}
...
func validateContent(content Content, blocks *int, limits Limits, depth int) error {
	...
	case ContentText, ContentRefusal, ContentReasoning:
		if content.ToolCall != nil || content.ToolResult != nil || content.URL != "" || content.Data != "" ||
		...
		if err := ValidateTextCitations(content.Text, content.Citations, limits); err != nil {
			return err
		}
	case ContentImage, ContentAudio, ContentVideo, ContentFile:
		...
		if content.ToolCall != nil || content.ToolResult != nil || content.Text != "" || content.Signature != "" || len(content.Citations) != 0 {
		...
		if content.URL != "" {
			if err := validateMediaURL(content.URL); err != nil {
				return err
			}
		}
		if content.Data != "" {
	...
	case ContentToolCall:
		if content.ToolCall == nil || strings.TrimSpace(content.ToolCall.ID) == "" || strings.TrimSpace(content.ToolCall.Name) == "" || !jsonObject([]byte(content.ToolCall.Arguments)) {
			return NewError(ErrorInvalidRequest, "invalid_tool_call", "tool call requires an ID, name, and JSON arguments", nil)
		}
		if exceeds(content.ToolCall.ID, limits.IdentifierBytes) || exceeds(content.ToolCall.Name, limits.ToolNameBytes) {
			return NewError(ErrorInvalidRequest, "tool_call_limit", "tool call ID or name exceeds the configured limit", nil)
		}
		if limits.ToolArgumentsBytes > 0 && len(content.ToolCall.Arguments) > limits.ToolArgumentsBytes {
			return NewError(ErrorInvalidRequest, "tool_arguments_limit", "tool arguments exceed the configured limit", nil)
		}
		if content.ToolResult != nil || content.Text != "" || content.URL != "" || content.Data != "" || len(content.Citations) != 0 ||
			content.FileID != "" || content.MediaType != "" || content.Filename != "" || content.Detail != "" || content.Signature != "" {
			return NewError(ErrorInvalidRequest, "invalid_content", "tool call contains fields from another content kind", nil)
		}
	case ContentToolResult:
		if content.ToolResult == nil || strings.TrimSpace(content.ToolResult.CallID) == "" {
			return NewError(ErrorInvalidRequest, "invalid_tool_result", "tool result requires a call ID", nil)
		}
		if exceeds(content.ToolResult.CallID, limits.IdentifierBytes) {
			return NewError(ErrorInvalidRequest, "tool_result_id_limit", "tool result call ID exceeds the configured limit", nil)
		}
		if content.ToolCall != nil || content.Text != "" || content.URL != "" || content.Data != "" || len(content.Citations) != 0 ||
			content.FileID != "" || content.MediaType != "" || content.Filename != "" || content.Detail != "" || content.Signature != "" {
			return NewError(ErrorInvalidRequest, "invalid_content", "tool result contains fields from another content kind", nil)
		}
		*blocks += len(content.ToolResult.Content)
		if limits.ContentBlocks > 0 && *blocks > limits.ContentBlocks {
			return NewError(ErrorInvalidRequest, "content_limit", "content block limit exceeded", nil)
		}
	...
	for _, sequence := range append([][]OutputItem{response.Output}, response.Alternatives...) {
		...
		for index, item := range sequence {
			...
			for _, content := range item.Content {
				if item.Role == RoleAssistant && content.Kind == ContentToolResult ||
					item.Role == RoleTool && content.Kind != ContentToolResult {
...
func ValidateUsage(usage Usage) error {
	...
	for _, count := range counts {
		...
		if count.Provenance != "" && count.Provenance != UsageAuthoritative && count.Provenance != UsageDerived &&
			count.Provenance != UsageEstimated && count.Provenance != UsageUnknown {
	...
	inputMatches, inputOverflow := countEqualsSum(usage.InputTotal, usage.InputUncached, usage.InputCacheRead, usage.InputCacheWrite)
	outputMatches, outputOverflow := countEqualsSum(usage.OutputTotal, usage.OutputReasoning, usage.OutputOther)
	totalMatches, totalOverflow := countEqualsSum(usage.Total, usage.InputTotal, usage.OutputTotal)
	if inputOverflow || outputOverflow || totalOverflow {
		return NewError(ErrorUpstreamUnavailable, "usage_overflow", "upstream usage totals overflow", nil)
	}
	if !inputMatches || !outputMatches || !totalMatches {
