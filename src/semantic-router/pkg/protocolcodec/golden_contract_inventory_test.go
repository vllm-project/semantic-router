...
func TestGoldenCoreMatrixInventoryIsClosed(t *testing.T) {
	...
	for _, scenario := range inventory.Scenarios {
		...
		case "request", "response", "error", "stream":
		default:
			t.Fatalf("matrix scenario %q has invalid kind %q", scenario.Name, scenario.Kind)
		}
		if len(scenario.Inputs) != len(wantProtocols) {
			t.Fatalf("matrix scenario %q has %d sources, want %d", scenario.Name, len(scenario.Inputs), len(wantProtocols))
		}
		for _, protocol := range wantProtocols {
			...
			if filepath.Base(name) != name || !strings.HasSuffix(name, "-in.json") ||
				!strings.Contains(name, "-"+protocol+"-") {
