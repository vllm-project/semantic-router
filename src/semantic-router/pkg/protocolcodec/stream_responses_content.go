	...
	case "function_call", "custom_tool_call":
		if !decoder.toolArgumentsDone[index] {
			return invalidProviderResponse(
				...
				"Responses tool-call item completed before its input done event",
			)
		}
		complete := item.Arguments
		if item.Type == "custom_tool_call" {
			complete = item.Input
		}
		if string(decoder.toolArguments[index]) != complete {
			return invalidProviderResponse(
				"stream_tool_arguments_mismatch",
				"Responses completed tool-call input does not match streamed input",
			)
		}
		return nil
	default:
