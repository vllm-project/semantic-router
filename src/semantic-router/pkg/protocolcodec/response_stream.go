	...
	for outputIndex, output := range response.Output {
		for contentIndex := range output.Content {
			...
			context := responseEventContext{
				...
				contentIndex: contentIndex, itemIndex: itemIndex,
			}
			contentEvents, err := neutralContentEvents(context, content)
			if err != nil {
				return nil, err
			}
...
func neutralContentEvents(context responseEventContext, content llmprotocol.Content) ([]llmprotocol.Event, error) {
	itemID := context.output.ID
	if len(context.output.Content) > 1 {
		itemID = llmprotocol.StableID(context.output.ID, fmt.Sprint(context.contentIndex))
	}
	if itemID == "" {
		itemID = llmprotocol.StableID(context.response.ID, fmt.Sprint(context.outputIndex), fmt.Sprint(context.contentIndex))
	}
	...
	case llmprotocol.ContentText, llmprotocol.ContentRefusal:
		return neutralTextEvents(started, completed, content), nil
	case llmprotocol.ContentReasoning:
		return neutralReasoningEvents(started, completed, content), nil
	case llmprotocol.ContentToolCall:
		return neutralToolCallEvents(started, completed, content)
	default:
...
func neutralTextEvents(started, completed llmprotocol.Event, content llmprotocol.Content) []llmprotocol.Event {
	...
	return []llmprotocol.Event{started, delta, completed}
}

func neutralReasoningEvents(started, completed llmprotocol.Event, content llmprotocol.Content) []llmprotocol.Event {
	started.Content = &llmprotocol.Content{Kind: content.Kind}
	completed.Content = &content
