...
type DecisionPlugin struct {
	// Type is the plugin type (decision_diagnostics, fast_response, hallucination, header_mutation, image_gen, memory, rag, request_params, response_jailbreak, router_replay, semantic-cache, system_prompt, tools)
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:Enum=decision_diagnostics;fast_response;hallucination;header_mutation;image_gen;memory;rag;request_params;response_jailbreak;router_replay;semantic-cache;system_prompt;tools
	Type string `json:"type" yaml:"type"`
