...
func TestMaintainedPrivacyRecipeParsesAndDecompilesWithoutError(t *testing.T) {
	...
	if err != nil {
		t.Skipf("privacy recipe YAML not found at %s: %v", assetPath, err)
	}
	...
	if !strings.Contains(dslText, "SIGNAL category_kb") {
		t.Error("decompiled DSL missing category_kb signal")
	}
...
func TestMaintainedPrivacyRecipeDSLRoundTrip(t *testing.T) {
	...
	if len(cfg.CategoryKBRules) == 0 {
		t.Error("expected at least one CategoryKBRule from privacy recipe")
	}
	...
	if len(cfg2.CategoryKBRules) != len(cfg.CategoryKBRules) {
		t.Errorf("round-trip CategoryKBRules: %d → %d",
			len(cfg.CategoryKBRules), len(cfg2.CategoryKBRules))
	}
