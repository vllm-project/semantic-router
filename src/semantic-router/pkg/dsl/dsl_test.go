...
func TestCompileAllAlgorithmTypes(t *testing.T) {
	algoDSLs := []struct {
		...
		},
		{
			...
			body:     `fallback_method: "static" min_turns_before_switch: 2 stay_bias: 0.4 quality_gap_multiplier: 1.2 handoff_penalty_weight: 0.8 remaining_turn_weight: 0.6`,
			verify: func(t *testing.T, algo *config.AlgorithmConfig) {
				if algo.SessionAware == nil {
					t.Fatal("expected session_aware config")
				}
				if algo.SessionAware.MinTurnsBeforeSwitch != 2 {
					t.Errorf("min_turns_before_switch = %d, want 2", algo.SessionAware.MinTurnsBeforeSwitch)
				}
				if algo.SessionAware.FallbackMethod != "static" {
					t.Errorf("fallback_method = %q, want static", algo.SessionAware.FallbackMethod)
				}
				if algo.SessionAware.StayBias != 0.4 {
					t.Errorf("stay_bias = %v, want 0.4", algo.SessionAware.StayBias)
				}
			},
		},
		{
...
func TestDecompileSessionAwareAlgorithmFields(t *testing.T) {
	input := `
SIGNAL domain test { description: "test" }
ROUTE test {
  PRIORITY 1
  ...
  ALGORITHM session_aware {
    ...
    stay_bias: 0.4
    quality_gap_multiplier: 1.2
    handoff_penalty_weight: 0.8
    remaining_turn_weight: 0.6
  }
}
`
	cfg, errs := Compile(input)
	...
	for _, expected := range []string{
		...
		"stay_bias",
		"quality_gap_multiplier",
		"handoff_penalty_weight",
		"remaining_turn_weight",
	} {
		if !strings.Contains(dslText, expected) {
			t.Errorf("missing %q in decompiled output", expected)
		}
	}
}
