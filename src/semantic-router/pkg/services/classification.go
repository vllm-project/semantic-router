...
// When using unified classifier, it still tries to get signal information from legacy classifier if available
func (s *ClassificationService) ClassifyIntentUnified(req IntentRequest) (*IntentResponse, error) {
	if s.unifiedClassifier != nil {
		...
		// Try to get signal information from legacy classifier if available
		var matchedSignals *MatchedSignals
		var decisionResult *DecisionResult
		if s.classifier != nil {
			// Evaluate signals once for both signal info and decision evaluation
			signals := s.classifier.EvaluateAllSignals(req.Text)
			if signals != nil {
			...
			// Evaluate decision if configured (reuse the same signals)
			if s.config != nil && len(s.config.IntelligentRouting.Decisions) > 0 {
				decisionRes, err := s.classifier.EvaluateDecisionWithEngine(signals)
				if err == nil && decisionRes != nil && decisionRes.Decision != nil {
					decisionResult = &DecisionResult{
						DecisionName: decisionRes.Decision.Name,
						Confidence:   decisionRes.Confidence,
						MatchedRules: decisionRes.MatchedRules,
					}
					// Use decision name as routing decision if available
					if decisionResult.DecisionName != "" {
						intentResult.Category = decisionResult.DecisionName
					}
				}
			}
		}

		// Determine recommended model and routing decision
		...
		// Override with decision result if available
		if decisionResult != nil && decisionResult.DecisionName != "" {
			routingDecision = decisionResult.DecisionName
			if s.config != nil {
				// Try to get model from decision
				for _, d := range s.config.IntelligentRouting.Decisions {
					if d.Name == decisionResult.DecisionName && len(d.ModelRefs) > 0 {
						modelRef := d.ModelRefs[0]
						if modelRef.LoRAName != "" {
							recommendedModel = modelRef.LoRAName
						} else {
							recommendedModel = modelRef.Model
						}
						break
					}
				}
	...
	// Fallback to legacy classifier (which now uses signal-driven architecture)
