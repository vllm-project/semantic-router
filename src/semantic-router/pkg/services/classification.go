...
func (s *ClassificationService) buildEvalResponse(
	...
	if decisionResult != nil && decisionResult.Decision != nil {
		// Extract used signals from decision's matched rules
		usedSignals := s.parseUsedSignals(decisionResult.MatchedRules)
...
// parseUsedSignals parses the matched rules from decision result and converts them to MatchedSignals format
// Input: matchedRules in format ["type:name", "type:name", ...]
// Output: MatchedSignals with signals grouped by type
func (s *ClassificationService) parseUsedSignals(matchedRules []string) *MatchedSignals {
	...
	for _, rule := range matchedRules {
		...
		signalType := strings.ToLower(strings.TrimSpace(parts[0]))
