...
func TestManagedInferenceProviderCredentialResolverUsesOnlyPublicationStore(t *testing.T) {
	...
	resolver, err := composeInferenceProviderCredentialResolver(
		store, providercredential.Codec{}, registry,
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := composeInferenceProviderCredentialResolver(nil, providercredential.Codec{}, registry); err == nil {
