...
func TestValkeyCacheIntegration_AddPendingRequest(t *testing.T) {
	...
	err := cache.AddPendingRequest(context.Background(), requestID, model, query, requestBody, ttlSeconds)
...
func TestValkeyCacheIntegration_UpdateWithResponse(t *testing.T) {
	...
	for i := 0; i < maxRetries; i++ {
		...
		updateErr = cache.UpdateWithResponse(context.Background(), requestID, responseBody, ttlSeconds)
		if updateErr == nil {
			break
		}
...
func TestValkeyCacheIntegration_UpdateWithResponseSpecialChars(t *testing.T) {
	...
	for _, tc := range specialIDs {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := cache.AddPendingRequest(context.Background(), tc.requestID, "gpt-4", tc.query, requestBody, ttlSeconds)
			...
			for i := 0; i < maxRetries; i++ {
				...
				updateErr = cache.UpdateWithResponse(context.Background(), tc.requestID, responseBody, ttlSeconds)
				if updateErr == nil {
					break
				}
			}
...
func TestValkeyCacheIntegration_DisabledCache(t *testing.T) {
	...
	err = cache.AddPendingRequest(context.Background(), "req_2", "gpt-4", "test", []byte("{}"), 300)
...
func TestValkeyCacheIntegration_TTLZeroSkipsCaching(t *testing.T) {
	...
	err = cache.AddPendingRequest(context.Background(), requestID2, "gpt-4", "test query 2", []byte("{}"), 0)
...
func TestValkeyCacheIntegration_MultipleEntries(t *testing.T) {
	...
	// the first one.
	for i, entry := range entries {
		lookupResult, err := cache.FindSimilar(context.Background(), "gpt-4", entry.query)
		if !assert.NoError(t, err, "FindSimilar should not error for %s", entry.query) {
			continue
		}
		if !assert.True(t, lookupResult.Found, "repeating %q verbatim must hit (similarity=%.4f, threshold=0.8)",
			entry.query, lookupResult.Similarity) {
			continue
		}
		assert.JSONEq(t, storedResponses[i], string(lookupResult.Body))
	}
}
...
func TestValkeyCacheIntegration_L2MetricType(t *testing.T) {
	...
	lookupResult, err := cache.FindSimilar(context.Background(), "gpt-4", "test L2 metric")
	require.NoError(t, err, "FindSimilar should work with L2 metric")
	require.True(t, lookupResult.Found, "repeating the cached query verbatim must hit (similarity=%.4f, threshold=0.5)",
		lookupResult.Similarity)
	assert.JSONEq(t, `{"result":"L2"}`, string(lookupResult.Body))
}
...
func TestValkeyCacheIntegration_IPMetricType(t *testing.T) {
	...
	lookupResult, err := cache.FindSimilar(context.Background(), "gpt-4", "test IP metric")
	require.NoError(t, err, "FindSimilar should work with IP metric")
	require.True(t, lookupResult.Found, "repeating the cached query verbatim must hit (similarity=%.4f, threshold=0.5)",
		lookupResult.Similarity)
	assert.JSONEq(t, `{"result":"IP"}`, string(lookupResult.Body))
}
