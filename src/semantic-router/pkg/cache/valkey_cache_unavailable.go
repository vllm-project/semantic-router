...
var errValkeyGlideUnavailable = fmt.Errorf("valkey-glide native client is unavailable on this platform (requires cgo and a supported architecture)")

// ValkeyCache is a compile-time stub when the native valkey-glide client is
// unavailable. valkey-glide links libglide_ffi and requires cgo plus a
...
func (c *ValkeyCache) embeddingDimension() (int, error) {
	...
	return resolveCacheBackendDimension(
		c.embeddingProvider,
		c.config.Index.VectorField.Dimension,
		c.embeddingModel,
		c.exactOnly,
	)
}
