...
// evaluator goroutine does not propagate to the caller and crash the router.
// Before this fix a panic would kill the whole process silently; now it is
// recovered, logged, and counted via llm_signal_panic_total.
func TestRunSignalDispatchersPanicRecovery(t *testing.T) {
	...
	okDispatcher := signalDispatch{
		signalType: "context",
		name:       "Context",
		evaluate:   func() { /* no-op — always succeeds */ },
	}
	...
	// Must not panic — if recoverSignalPanic is missing the test process crashes.
...
// goroutine does not prevent other signal evaluators from completing normally.
	...
	dispatchers := []signalDispatch{
		{
			signalType: "keyword",
			name:       "Keyword",
			evaluate:   func() { panic("keyword panic") },
		},
		{
			signalType: "context",
			name:       "Context",
			evaluate:   func() { completed.Store("context", true) },
		},
		{
			signalType: "embedding",
			name:       "Embedding",
			evaluate:   func() { panic("embedding panic") },
		},
		{
			signalType: "language",
			name:       "Language",
			evaluate:   func() { completed.Store("language", true) },
		},
	}
