...
func TestToAnthropicRequestBodyWithPassthrough_AutoCacheControl(t *testing.T) {
	req := &openai.ChatCompletionNewParams{
		Model: "claude-sonnet-4-5",
		Messages: []openai.ChatCompletionMessageParamUnion{
			systemStringMessage("stable system"),
			userStringMessage("first"),
			assistantStringMessage("ack"),
			userStringMessage("latest"),
		},
	}
	pt := &AnthropicPassthrough{
	...
	body, err := ToAnthropicRequestBodyWithPassthrough(req, pt)
	require.NoError(t, err)
	assert.Equal(t, "1h", gjson.GetBytes(body, "system.0.cache_control.ttl").String())
	assert.False(t, gjson.GetBytes(body, "messages.0.content.0.cache_control").Exists())
	assert.Equal(t, "1h", gjson.GetBytes(body, "messages.2.content.0.cache_control.ttl").String())
}

func TestToAnthropicRequestBodyWithPassthrough_ExplicitCacheControlDisablesAuto(t *testing.T) {
	req := &openai.ChatCompletionNewParams{
		Model: "claude-sonnet-4-5",
		Messages: []openai.ChatCompletionMessageParamUnion{
			userStringMessage("latest"),
	...
	body, err := ToAnthropicRequestBodyWithPassthrough(req, pt)
	require.NoError(t, err)
	assert.Equal(t, "5m", gjson.GetBytes(body, "system.0.cache_control.ttl").String())
	assert.False(t, gjson.GetBytes(body, "messages.0.content.0.cache_control").Exists())
}
