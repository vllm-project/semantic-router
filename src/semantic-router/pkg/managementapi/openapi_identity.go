...
func identitySchemas() map[string]JSONSchema {
	...
	invitedTokenExchangeProperties := make(map[string]JSONSchema, len(tokenExchangeProperties)+1)
	for name, schema := range tokenExchangeProperties {
		invitedTokenExchangeProperties[name] = schema
	}
	invitedTokenExchangeProperties["invitationToken"] = JSONSchema{
	...
	invitedTokenResponseProperties := make(map[string]JSONSchema, len(tokenResponseProperties)+1)
	for name, schema := range tokenResponseProperties {
		invitedTokenResponseProperties[name] = schema
	}
	invitedTokenResponseProperties["onboarding"] = refSchema("OnboardingResult")
