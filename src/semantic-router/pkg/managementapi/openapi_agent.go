...
func agentSchemas() map[string]JSONSchema {
	...
	stringArray := arraySchema(stringSchema)

	resourceProperties := map[string]JSONSchema{
	...
	profileProperties := cloneSchemas(resourceProperties)
	profileProperties["contentRevision"] = revision
	profileProperties["defaultTarget"] = refSchema("AgentTarget")
	profileProperties["minimumTargetCapabilities"] = stringArray
	profileProperties["supportedModes"] = arraySchema(JSONSchema{Type: "string", Enum: []string{"chat", "builder"}})
	profileProperties["defaultForModes"] = arraySchema(JSONSchema{Type: "string", Enum: []string{"chat", "builder"}})
	profileProperties["skills"] = arraySchema(refSchema("AgentSkillReference"))
	profileProperties["toolPolicy"] = refSchema("AgentToolPolicy")
	profileProperties["approvalPolicy"] = JSONSchema{Type: "string", Enum: []string{"required"}}
	profileProperties["maximumTurnSeconds"] = boundedIntegerSchema(10, 86400)
	profileProperties["maximumToolSteps"] = boundedIntegerSchema(1, 256)
	profileProperties["contextTokenBudget"] = boundedIntegerSchema(1024, 1048576)
	...
	skillProperties := cloneSchemas(resourceProperties)
	skillProperties["contentRevision"] = revision
	skillProperties["builtin"] = JSONSchema{Type: "boolean"}
	skillProperties["instructions"] = stringSchema
	skillProperties["requiredTools"] = stringArray
	skillProperties["minimumCapabilities"] = stringArray
	skillProperties["contentDigest"] = digest
	...
	toolSourceProperties := cloneSchemas(resourceProperties)
	toolSourceProperties["contentRevision"] = revision
	toolSourceProperties["kind"] = JSONSchema{Type: "string", Enum: []string{"remote"}}
	toolSourceProperties["transport"] = JSONSchema{Type: "string", Enum: []string{"streamable_http"}}
	toolSourceProperties["endpoint"] = JSONSchema{Type: "string", Format: "uri"}
	toolSourceProperties["credentialId"] = uuid
	toolSourceProperties["egressPolicy"] = refSchema("AgentEgressPolicy")
	toolSourceProperties["discoveredTools"] = arraySchema(refSchema("AgentToolDefinition"))
	toolSourceProperties["discoveryDigest"] = digest
	toolSourceProperties["approvedDiscoveryDigest"] = digest
	toolSourceProperties["availability"] = JSONSchema{Type: "string", Enum: []string{
	...
	agentEventSchema := func(eventType, payloadSchema string) JSONSchema {
		return objectSchema(
	...
	return map[string]JSONSchema{
		...
		"AgentSessionCreateRequest": objectSchema([]string{"mode", "target"}, map[string]JSONSchema{
			"mode": {Type: "string", Enum: []string{"chat", "builder"}}, "profileId": uuid,
			"effectiveTeamId": uuid, "target": refSchema("AgentTarget"), "title": stringSchema,
		}),
		...
		"AgentSession": objectSchema([]string{
			"id", "namespaceId", "ownerPrincipalId", "profileId", "profileRevision", "target", "mode", "title", "status", "revision", "createdAt", "updatedAt",
		}, map[string]JSONSchema{
			"id": uuid, "namespaceId": uuid, "ownerPrincipalId": uuid, "effectiveUserId": uuid, "effectiveTeamId": uuid,
			"profileId": uuid, "profileRevision": revision, "target": refSchema("AgentTarget"),
