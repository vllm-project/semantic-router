"""Grid-flex analysis and reporting helpers for fleet-sim optimizer."""
...
from .base import _calibrate, _p99_wait
...
def _throttled_profile(
    gpu: ManualProfile, n_max_cap: int, max_ctx: int
) -> ManualProfile:
...
def grid_flex_analysis(
    ...
    1. Derives ``n_max_cap`` by inverting the GPU power model.
    2. Estimates P99 TTFT analytically (M/G/c) with ``n_max_cap`` slots.
    3. Optionally runs a DES simulation at ``n_max_cap`` slots.

    Examples
    --------
    >>> from fleet_sim import H100_80GB
    >>> from fleet_sim.optimizer import grid_flex_analysis
    >>> import json
    >>> cdf = json.load(open("data/azure_cdf.json"))
    >>> curve = grid_flex_analysis(cdf, lam=200, n_gpus=40, gpu=H100_80GB, t_slo_ms=500)
    """
    if flex_pcts is None:
        flex_pcts = [0.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 40.0, 50.0]

    _ = gpu.power_at_concurrency(1)
    ...
    for flex_pct in flex_pcts:
        ...
        if n_sim_requests > 0:
            try:
                ...
                if verbose:
                    print(f"  [DES error at flex={flex_pct}%]: {exc}")
...
def print_grid_flex_table(
    ...
    if has_des:
        ...
        print(
            f"  {'Flex':>6} {'n_max':>6} {'W/GPU':>7} {'Fleet kW':>9} {'P99 TTFT':>9} {'SLO':>5}"
        )
        print(f"  {'-' * 6} {'-' * 6} {'-' * 7} {'-' * 9} {'-' * 9} {'-' * 5}")
        for pt in results:
            ok = "  OK" if pt.slo_met else "BREACH"
