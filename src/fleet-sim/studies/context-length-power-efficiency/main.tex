...
services---remains a stronger energy lever than buying newer hardware.
Working from published H100 power measurements~\citep{chung2026mlenergy}, a
calibrated logistic power model~\citep{liang2026g2g}, a roofline throughput
model~\citep{xu2026aiconfigurator}, and a low-concurrency B200 calibration
from one 8$\times$B200 vLLM serving run, we derive these results analytically
using the inference-fleet-sim framework~\citep{chen2026fleetsim}.  Two-pool
context-length routing (FleetOpt~\citep{chen2026fleetopt}) still delivers
roughly $2.5\times$ over a homogeneous H100 fleet, while the calibrated B200
generation gain rises to roughly $2\times$ at fixed topology.  On the
short-context-dominant Azure trace, combining FleetOpt with calibrated B200
reaches $5.76\times$ over the H100 homogeneous baseline.  H100 results are
anchored to published measurements; B200 remains \textbf{FAIR quality} because
the calibration uses only a single-concurrency serving point rather than a full
power-vs-batch sweep.
...
generation remain approximately multiplicative levers after calibration.
FleetOpt routing gives roughly $2.5\times$ over homogeneous on H100 and
$2.8\times$ on calibrated B200 for Azure; upgrading from H100 to calibrated
B200 gives roughly $2\times$ at fixed topology.  On short-context-dominant
workloads the two effects combine to roughly $5.8\times$.  Neither lever alone
gets you close to that total.
...
For B200-SXM, we now use a partial empirical calibration from one
8$\times$B200 node running vLLM 0.19.1 with Llama-3.1-70B at 8K context and
\texttt{max\_num\_seqs}$=1$.  The measured states are: true unloaded idle
$\approx 191$~W/GPU, model loaded but not serving $\approx 251$~W/GPU, and
active inference $\approx 513$~W mean / $606$~W p95 per GPU.  Because the
simulator assumes model weights are already resident, we map the loaded-idle
state to $\Pidle = 251$~W and the p95 active state to
$\Pnom = 606$~W.  With $k=1.0$, fitting Eq.~\eqref{eq:power} to the $b{=}1$
active mean gives $x_0 \approx -1.03$.  Throughout the paper, H100 power
estimates remain \textbf{HIGH quality} (directly measured), while B200 power
estimates are \textbf{FAIR quality}: empirically anchored at one serving point,
but not yet constrained by a full power-vs-concurrency sweep.
...
\caption{$\nmax$ and tok/W vs.\ context window for Llama-3.1-70B (TP=8, fp16).
  H100 power is directly measured (HIGH quality); B200 power uses a
  single-concurrency empirical anchor plus analytical scaling (FAIR quality).}
\label{tab:ctx-nmax}
\small
\begin{tabular}{rrrrrrrr}
\toprule
& \multicolumn{3}{c}{H100-SXM5} & \phantom{ab} & \multicolumn{3}{c}{B200-SXM6} \\
\cmidrule{2-4} \cmidrule{6-8}
Context (K) & $\nmax$ & $P_{\text{sat}}$ & tok/W &
            & $\nmax$ & $P_{\text{sat}}$ & tok/W \\
\midrule
 2  &   512 & 598\,W & 35.0 && 1725 & 795\,W &  37.09 \\
 4  &   256 & 593\,W & 17.6 &&  872 & 713\,W &  30.34 \\
 8  &   128 & 583\,W &  8.97 &&  439 & 655\,W &  20.55 \\
16  &    64 & 557\,W &  4.69 &&  220 & 596\,W &  13.02 \\
32  &    32 & 507\,W &  2.58 &&  110 & 590\,W &   7.89 \\
64  &    16 & 435\,W &  1.50 &&   55 & 637\,W &   4.30 \\
128 &     8 & 369\,W &  0.88 &&   28 & 723\,W &   1.14 \\
\bottomrule
\end{tabular}
\end{table}
...
B200 shifts the curve up but does not change the slope.  Under the calibrated
power model, B200 achieves roughly $1.5$--$2.5\times$ higher tok/W than H100
at matching context windows,
...
$1.81\times$ better than H100 (2.71 vs.\ 1.50 tok/W), down from $2.47\times$
at 4K.  The reason is no longer a large projected idle penalty; instead, the
calibrated B200 serving curve is already near its active ceiling across these
operating points, staying close to 606~W while H100 power falls more noticeably
at low batch.  At 64K, the B200 still has far more concurrency (41 sequences
vs.\ 16), but the watt denominator no longer drops with context length the way
it does on H100.  We return to the fleet-level
...
\caption{Single-GPU tok/W at $\nmax$ (8K context).  Dense model values
  ...
  v3.0, HIGH quality; B200 uses a single-concurrency measured power anchor,
  FAIR quality).
...
Model & TP & $\nmax$ & tok/s & tok/W & & $\nmax$ & tok/s & tok/W \\
\midrule
Llama-3.1-8B         & 1 &  58 & 3{,}350 &  6.46 &&  148 &  9{,}962 & 16.43 \\
Llama-3.1-70B        & 8 &  22 & 2{,}716 &  7.41 &&   58 & 12{,}960 & 21.39 \\
Llama-3.1-405B       & 8 &   1 &      26 &  0.09 &&   17 &  1{,}009 &  1.67 \\
Qwen3-235B-A22B$^\dagger$ & 8 & 24 & 11{,}521 & 37.82 && 146 & 80{,}584 & 132.93 \\
DeepSeek-V3$^\dagger$ (fp8) & 8 & 1 & 646 & 2.14 &&   11 &  8{,}162 & 13.55 \\
\bottomrule
\end{tabular}
\end{table}
...
uniform.  Under the calibrated power model, for sub-100B dense models it
ranges from $2.5\times$ to $2.9\times$,
...
dramatic: from 0.09 to 1.67 tok/W, an $\approx 19\times$ improvement.  This happens
...
\caption{Fleet token efficiency at $\lambda = 1{,}000$ req/s.  H100
  ...
  from H100 by the 2.62$\times$ KV-budget ratio with a single-concurrency
  measured power anchor (FAIR quality).  Topology: Homo = homogeneous 64K fleet; Pool =
...
\multirow{6}{*}{\makecell[l]{Azure\\(Arch.\ I)}}
  ...
  & ---         & Homo 64K       & B200 &   47 &  28.5 & 11.42 & +105\% \\
  & 4K          & Pool routing   & B200 &   25 &  15.1 & 19.37 & +247\% \\
  & 4K/$\gamma^*{=}2$ & FleetOpt & B200 &   17 &  10.1 & 32.12 & \textbf{+476\%} \\
\midrule
\multirow{6}{*}{\makecell[l]{LMSYS\\(Arch.\ I)}}
  ...
  & ---         & Homo 64K       & B200 &   24 &  14.5 &  9.36 & +96\% \\
  & 1.5K        & Pool routing   & B200 &   16 &   9.7 & 13.42 & +181\% \\
  & 1.5K/$\gamma^*{=}2$ & FleetOpt & B200 &  12 &   7.3 & 18.28 & \textbf{+283\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\[
  ...
  \Delta_{\text{topo}}(\text{B200}) = 32.12 / 11.42 = 2.81.
\]
\[
  \Delta_{\text{gen}}(\text{Homo}) = 11.42 / 5.58 = 2.05, \quad
  \Delta_{\text{gen}}(\text{FleetOpt}) = 32.12 / 14.08 = 2.28.
\]

The topology gain stays in the same ballpark between H100 and calibrated B200
($2.52$ vs.\ $2.81$), and the generation gain stays near $2\times$ between
homogeneous and FleetOpt ($2.05$ vs.\ $2.28$).  The two levers still operate
on largely different dimensions, though the calibrated B200 power floor makes
the generation effect somewhat stronger for short-context-heavy topologies.
For Azure, calibrated B200 FleetOpt delivers $32.12 / 5.58 = 5.76\times$ over
the H100 homogeneous baseline.  Neither lever alone gets you close---FleetOpt
with H100 gives $2.52\times$ and calibrated B200 with a homogeneous fleet gives
$2.05\times$.
...
GPUs without rethinking topology capture about $2\times$ and still leave a large
topology gain on the table.  Operators who invest in two-pool routing on
H100 capture $2.52\times$ and then can stack another $\approx 2.3\times$ on top
with a calibrated B200 upgrade in the Azure setting---arriving at a fleet that
delivers 5.76$\times$ better energy efficiency than the H100 homogeneous baseline.
...
\caption{GPU generation comparison for Llama-3.1-70B (TP=8, fp16) at 8K context.
  Power quality: H100 = HIGH (measured); B200 = FAIR (single-concurrency measured
  anchor); H200/GB200 = FAIR (projected, $\pm$15\%).}
...
B200-SXM  & 1000 & 251 & 2.95 &  58 &  606 & 21.39 & 64.0 &  0.73M \\
...
B200 pushes further in absolute tok/W (21.39 vs.\ 15.58) by fitting still
more sequences in flight ($\nmax = 58$ vs.\ 44).  Under the calibrated serving
model its effective idle floor is actually lower than the old TDP-fraction
projection (251~W vs.\ the previous 430~W), while the $2.62\times$ larger memory
budget more than compensates for the higher TDP.  On a tok/\$M basis, B200 comes out ahead:
...
B200-SXM, its per-GPU tok/W is lower (18.49 vs.\ 21.39) because the higher
...
lightly loaded.  FleetOpt on calibrated B200 reaches 32.1 tok/W---a +476\%
improvement over the H100 homogeneous baseline, composed of a $2.28\times$
generation gain and a $2.81\times$ topology gain that combine to $5.76\times$.
This is the
...
a homogeneous fleet, and calibrated B200 delivers roughly $2\times$ over H100
at fixed topology.  On the Azure trace these two improvements combine to
$5.76\times$---$2.5 + 2.0 \neq 5.76$ because the gains stack on orthogonal
dimensions and are only approximately constant across topologies.
...
validated against ML.ENERGY v3.0 and B200 re-anchored to one low-concurrency
serving calibration.  H200 and GB200 remain projections.  As fuller
multi-concurrency measurement data for B200 and H200 becomes available, we
expect the absolute numbers to shift further while the qualitative picture---
...
B200 power parameters are no longer pure TDP-fraction projections: they are
anchored to one 8$\times$B200 serving run at $b{=}1$, then extrapolated with
the same logistic functional form used for H100.  This substantially improves
the low-concurrency estimate, but it is still not a full $P(b)$ calibration.
We therefore keep a FAIR-quality label and expect the high-batch tail to remain
uncertain until a proper concurrency sweep is measured.
...
\paragraph{Only partial empirical validation on B200; none on H200.}
The B200 results now include a direct low-concurrency serving anchor, but we
still have not measured the full vLLM power-vs-concurrency curve on B200, and
we have no corresponding serving measurements on H200.  We therefore cannot
confirm that the calibrated B200 logistic shape remains correct at higher batch
sizes, nor can we validate the H200 projection.  Researchers with access to
these platforms should measure and publish the full $P(b)$ curve to fill this gap.
...
The most immediate follow-on is extending the current B200 $b{=}1$ anchor to a
full logistic $P(b)$ sweep and collecting the same data for H200-SXM using the
ML.ENERGY methodology~\citep{chung2026mlenergy}.  That would upgrade the B200
results from FAIR to HIGH quality and replace the remaining H200 projection.
...
32, 64, 128, 256\}$; fit error $<$3\%).  B200-SXM uses a single-concurrency
empirical anchor from one 8$\times$B200 serving run.  H200 and GB200 remain
first-principles projections.

\begin{table}[htbp]
\centering
\caption{GPU power model parameters.  HIGH = directly measured; FAIR = either
  a single-concurrency empirical anchor (B200) or a TDP-fraction projection
  calibrated to H100 ML.ENERGY data~\citep{chung2026mlenergy}.}
...
B200-SXM  & 1000 & 251 & 606 & 1.0 & -1.03 & 8$\times$B200 vLLM b=1 anchor & FAIR \\
