...
class PhysicalBatchBackend(DecisionBackend):
    ...
    share a bounded, round-robin row queue. A configurable row quantum can let
    one request finish several forwards before rotating to the next request;
    it is capped at four physical batches so a large job cannot monopolize the
    queue. The short coalescing window is a tunable latency/throughput tradeoff,
    while ``physical_batch_size`` caps the rows in one model forward.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if (
            type(max_rows_per_job_turn) is not int
            or not 1
            <= max_rows_per_job_turn
            <= MAX_JOB_TURN_BATCHES * physical_batch_size
        ):
            raise ValueError(
                "max_rows_per_job_turn must fit within four physical batches"
            )
        self._model = model
        ...
        self.max_rows_per_job_turn = max_rows_per_job_turn
    ...
    async def _submit(
        ...
        job = _RowJob(
            rows=rows,
            future=future,
            turn_rows_remaining=self.max_rows_per_job_turn,
        )
        async with self._condition:
            if self._closed:
        ...
        while self._pending and len(selected) < limit:
            ...
            for _ in range(round_size):
                ...
                if index is not None:
                    ...
                    job.turn_rows_remaining -= 1
                    selected.append(
                        _SelectedRow(job=job, index=index, row=job.rows[index])
                    )
                if job.remaining:
                    if index is not None and job.turn_rows_remaining:
                        self._pending.appendleft(job)
                    else:
                        job.turn_rows_remaining = self.max_rows_per_job_turn
                        self._pending.append(job)
