...
EXPECTED_SCHEMA = "decision.nano.score_path_capacity.v1"
...
EXPECTED_PACKING = {"max_length": 8192, "state_truncation": "error"}
...
EXPECTED_STATE_LAYOUT = {
    "arm": "all22",
    "choice_and_noul_heads_frozen": True,
    "cross_record_or_request_cache": False,
    "embedding_storage": "encoder.embeddings; exactly one frozen vocabulary",
    "forward_batch_policy": (
        "one embedding pass; all encoder paths keep full B x L; only heads "
        "gather type rows"
    ),
    "integrated_choice_storage": (
        "choice_blocks + choice_final_norm; original trained values, all frozen"
    ),
    "merged_weights": False,
    "score_shared_prefix_layers": 0,
    "score_suffix_copy_source": "original shared encoder, never trained Choice suffix",
    "shared_encoder_storage": "encoder.layers + encoder.final_norm; all frozen",
    "trainability_policy": "score_private_all22_and_head",
    "training_arm": "S22",
    "type_embedding_frozen": True,
...
def _validate_artifact(
    ...
    expected = {
        ...
        "parameters": EXPECTED_STATE_COUNTS["unique_parameters"],
        "rocm_contiguous_layout": True,
        "type_order": list(KINDS),
    }
    for key, value in expected.items():
        if config.get(key) != value:
            raise VelaRuntimeError(f"Vela checkpoint identity mismatch: {key}")
    if config.get("packing") != EXPECTED_PACKING:
        raise VelaRuntimeError("Vela checkpoint packing contract mismatch")
    if max_length > EXPECTED_PACKING["max_length"]:
        raise VelaRuntimeError("Vela max input length exceeds the release packing")
    _validate_state_layout(config.get("state_layout"))
    for relative in (
        "encoder/config.json",
        "encoder/model.safetensors",
        "decision_heads.safetensors",
    ...
    state_layout = _read_json(root / "STATE_LAYOUT.json")
    if state_layout != config["state_layout"]:
        raise VelaRuntimeError("Vela state-layout sidecar does not match config")
...
def _validate_state_layout(value: object) -> None:
    if not isinstance(value, dict):
        raise VelaRuntimeError("Vela state-layout metadata is invalid")
    for key, expected in EXPECTED_STATE_LAYOUT.items():
        if value.get(key) != expected:
            raise VelaRuntimeError(f"Vela state-layout mismatch: {key}")
    if value.get("counts") != EXPECTED_STATE_COUNTS:
        raise VelaRuntimeError("Vela state-layout count mismatch")
    copy_map = value.get("score_suffix_copy_map")
    if (
        not isinstance(copy_map, dict)
        or len(copy_map) != EXPECTED_STATE_COUNTS["score_suffix_tensors"]
    ):
        raise VelaRuntimeError("Vela score suffix copy-map is invalid")
    if any(
        not isinstance(destination, str)
        or not isinstance(source, str)
        or not destination.startswith(("score_blocks.", "score_final_norm."))
        or not source.startswith(("encoder.layers.", "encoder.final_norm."))
        for destination, source in copy_map.items()
    ) or len(set(copy_map.values())) != len(copy_map):
        raise VelaRuntimeError("Vela score suffix copy-map is not one-to-one")


def _validate_inventory(value: dict[str, Any]) -> dict[str, dict[str, tuple[int, ...]]]:
    expected_groups = {
        "encoder_shapes": EXPECTED_STATE_COUNTS["shared_encoder_tensors"],
        "choice_suffix_shapes": EXPECTED_STATE_COUNTS[
            "integrated_choice_suffix_tensors"
        ],
        "score_suffix_shapes": EXPECTED_STATE_COUNTS["score_suffix_tensors"],
        "head_shapes": EXPECTED_STATE_COUNTS["type_and_head_tensors"],
    }
    if value.get("counts") != EXPECTED_STATE_COUNTS:
        raise VelaRuntimeError("Vela tensor-inventory count mismatch")
    parsed: dict[str, dict[str, tuple[int, ...]]] = {}
    all_names: set[str] = set()
    for name, expected_count in expected_groups.items():
        group = value.get(name)
        if not isinstance(group, dict) or len(group) != expected_count:
    ...
    if len(all_names) != EXPECTED_STATE_COUNTS["unique_tensors"]:
        raise VelaRuntimeError("Vela tensor inventories do not cover the release")
