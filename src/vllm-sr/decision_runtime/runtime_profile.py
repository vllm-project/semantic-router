...
def load_runtime_profile(profile_id: str, *, revision: str) -> RuntimeProfile:
    ...
    matches = tuple(
        (registration.profile_directory, resource)
        for registration in registered_families()
        if (
            resource := root.joinpath(
                registration.profile_directory, f"{profile_id}.json"
            )
        ).is_file()
    )
    if len(matches) != 1:
    ...
    if registration is None or matches[0][0] != registration.profile_directory:
        raise RuntimeProfileError(
...
def parse_runtime_profile(payload: bytes, *, revision: str) -> RuntimeProfile:
    ...
    execution = (
        family_adapter(family).parse_execution(root["execution"])
