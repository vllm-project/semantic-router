...
- Treat `runtime_status.py` as the owner for local Docker `status` / `logs` probes, container-health checks, and filtered log command assembly.
...
- Keep `core.py` focused on startup and shutdown orchestration; new status/log probing, container-health checks, or filtered log-shell assembly belong in `runtime_status.py`.
