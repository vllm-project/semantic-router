...
from decision_runtime.family_registry import family_registration
from decision_runtime.runtime_profile import RuntimeProfileError
...
class IntegratedDecisionCatalogResolver:
    ...
    detect_backend: Callable[[], str] = field(default_factory=lambda: detect_backend)
    ...
    def resolve(self, request: DecisionRuntimeRequest) -> ResolvedDecisionRuntime:
        ...
        try:
            model.profile.validate_physical_batch_size(backend, max_batch)
        ...
        if registration is None:  # pragma: no cover - guarded by the profile parser
            raise DecisionCatalogError(
                "Decision runtime family has no image entrypoint"
            )
        python = registration.container_python
...
def detect_backend() -> str:
    """Select an installed execution path without importing inference frameworks."""
    ...
    # CUDA is a reserved extension point, not an installed Decision executor.
    # A CUDA-only Linux host can still serve the sub-1B CPU models.
