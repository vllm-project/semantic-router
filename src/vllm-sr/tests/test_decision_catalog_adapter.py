...
from cli.decision_runtime import catalog_adapter as cli_catalog_adapter
...
@pytest.mark.parametrize(
    ("rocm_visible", "cuda_visible", "expected"),
    (
        (False, False, "cpu"),
        (False, True, "cpu"),
        (True, False, "rocm"),
...
def test_auto_backend_selects_only_installed_executors(
    ...
    class DevicePath:
        ...
        def exists(self) -> bool:
            return {
                "/dev/kfd": rocm_visible,
                "/dev/nvidiactl": cuda_visible,
            }[self.value]

    monkeypatch.setattr(cli_catalog_adapter, "Path", DevicePath)
    monkeypatch.setattr(cli_catalog_adapter.sys, "platform", "linux")

    assert cli_catalog_adapter.detect_backend() == expected


def test_default_resolver_uses_real_auto_backend_detector(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    artifact_root = tmp_path / "artifact"
    artifact_root.mkdir()
    monkeypatch.setattr(cli_catalog_adapter, "detect_backend", lambda: "rocm")
    bridge = IntegratedDecisionCatalogResolver(
        artifacts=RecordingArtifacts(artifact_root),
        images={"rocm": IMAGE},
    )

    spec = bridge.resolve(request(backend="auto"))

    assert spec.backend == "rocm"
    assert spec.command[spec.command.index("--backend") + 1] == "rocm"
...
def test_sol_default_and_batch_override_keep_the_same_clean_launch_contract(
    tmp_path: Path,
) -> None:
    bridge = resolver(tmp_path)

    spec = bridge.resolve(request(model=SOL_MODEL))
    larger = bridge.resolve(request(model=SOL_MODEL, max_batch=16))

    assert spec.canonical_model == SOL_MODEL
    assert spec.backend == "rocm"
    assert spec.command[0] == "/opt/vllm-sr/venvs/qwen35/bin/python"
    assert spec.max_batch == 8
    assert larger.max_batch == 16
    assert spec.command[-6:] == (
        "--max-batch",
        "8",
        "--max-concurrency",
    ...
    assert [item.catalog.model_id for item in bridge.artifacts.calls] == [
        SOL_MODEL,
        SOL_MODEL,
...
@pytest.mark.parametrize(
    ("model", "backend", "max_batch", "message"),
    (
        (EOS_MODEL, "rocm", 9, "profile maximum 8"),
        (MODEL, "rocm", 4097, "between 1 and 4096"),
        (EOS_MODEL, "cpu", 4097, "between 1 and 4096"),
    ),
)
def test_batch_limits_fail_before_artifact_materialization(
    tmp_path: Path, model: str, backend: str, max_batch: int, message: str
) -> None:
    bridge = resolver(tmp_path, images={"rocm": IMAGE, "cpu": IMAGE})

    with pytest.raises(DecisionCatalogError, match=message):
        bridge.resolve(request(model=model, backend=backend, max_batch=max_batch))

    assert bridge.artifacts.calls == []  # type: ignore[attr-defined]


@pytest.mark.parametrize(
    ("model", "backend", "max_batch"),
    (
        (EOS_MODEL, "rocm", 8),
        (EOS_MODEL, "cpu", 9),
        (MODEL, "rocm", 4096),
...
def test_batch_limit_boundary_and_backend_scope_allow_artifact_materialization(
    tmp_path: Path, model: str, backend: str, max_batch: int
) -> None:
    bridge = resolver(tmp_path, images={"rocm": IMAGE, "cpu": IMAGE})

    spec = bridge.resolve(request(model=model, backend=backend, max_batch=max_batch))

    assert spec.max_batch == max_batch
    assert [item.catalog.model_id for item in bridge.artifacts.calls] == [  # type: ignore[attr-defined]
        model
    ]
