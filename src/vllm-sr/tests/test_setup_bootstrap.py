...
def test_first_serve_bootstrap_creates_v03_management_workspace(tmp_path):
    ...
    assert "control_plane" not in config["global"]
    assert config["providers"] == {"defaults": {}, "models": []}
    assert config["routing"] == {"modelCards": []}
    ...
    assert config["global"]["stores"]["management"]["postgres"]["dsn_env"]
    assert config["global"]["stores"]["runtime"]["redis"]["url_env"]
    ...
    bootstrap_dir = local_bootstrap_token_directory(tmp_path, layout)
...
def test_dashboard_bootstrap_authority_remains_isolated_after_consumption(tmp_path):
    config_path = tmp_path / "config.yaml"
