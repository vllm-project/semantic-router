...
from decision_runtime.model_inputs import QWEN_PROMPT_VERSION  # noqa: E402
...
def _artifact(
    tmp_path: Path,
    *,
    prompt_version: str | None = QWEN_PROMPT_VERSION,
    base_model: str | None = "Qwen/Qwen3.5-2B",
) -> Path:
    ...
    metadata = {"head_dim": 256}
    if prompt_version is not None:
        metadata["prompt_version"] = prompt_version
    if base_model is not None:
        metadata["base_model"] = base_model
    (root / "decision_config.json").write_text(json.dumps(metadata))
...
@pytest.mark.parametrize(
    ("prompt_version", "base_model"),
    (("revised-author-prompt", "other/model-identifier"), (None, None)),
)
def test_loader_accepts_evolving_author_metadata_before_gpu_import(
    tmp_path: Path,
    ...
    imported = []
    root = _artifact(tmp_path, prompt_version=prompt_version, base_model=base_model)

    class ModelImportReachedError(Exception):
        pass

    def stop_at_model_import(name: str) -> None:
        imported.append(name)
        raise ModelImportReachedError

    monkeypatch.setattr(qwen35_torch, "_required_module", stop_at_model_import)

    with pytest.raises(ModelImportReachedError):
...
def _profiled_artifact(
    tmp_path: Path, *, status: str = "hardware-qualified", value_heads: int = 16
) -> Path:
    root = _artifact(tmp_path)
    config_path = root / "backbone/config.json"
    backbone_config = json.loads(config_path.read_text())
    backbone_config["linear_num_value_heads"] = value_heads
    config_path.write_text(json.dumps(backbone_config))
    profile_root = root / "runtime-profile"
    profile_root.mkdir()
    entries = {}
    normalization_blocks_max = 32 if value_heads == 16 else 64
    for batch_size in range(1, normalization_blocks_max + 1):
        key = [
            128,
            batch_size,
            "torch.bfloat16",
    ...
    supported = {
        ...
        "NB_min": 1,
        "NB_max": normalization_blocks_max,
        "NB_formula": f"ceil(B*padded_tokens*{value_heads}/65536)",
        "unknown_key": "raise before kernel/autotune",
    }
    if value_heads == 16:
        supported["key_heads"] = 16
    else:
        supported.update(
...
def test_candidate_label_does_not_override_owned_kernel_contract(
    ...
    assert _validated_rocm_profile(root) is not None


@pytest.mark.parametrize(
    ("value_heads", "normalization_blocks_max"), ((16, 32), (32, 64))
)
def test_rocm_profile_selects_envelope_from_backbone_geometry(
    tmp_path: Path, value_heads: int, normalization_blocks_max: int
) -> None:
    profile = _validated_rocm_profile(
        _profiled_artifact(tmp_path, value_heads=value_heads)
    )
    assert profile is not None
    assert profile.normalization_blocks_max == normalization_blocks_max


def test_rocm_profile_uses_backbone_geometry_with_evolving_model_labels(
    tmp_path: Path,
) -> None:
    root = _profiled_artifact(tmp_path)
    (root / "decision_config.json").write_text(json.dumps({"head_dim": 256}))
    profile_path = root / "runtime-profile/profile.json"
    profile = json.loads(profile_path.read_text())
    profile["model_family"] = "author-renamed-family"
...
@pytest.mark.parametrize(
    ("field", "value", "message"),
    (
        ("model_type", "other_model", "backbone geometry"),
        ("linear_num_value_heads", 24, "backbone geometry"),
        ("linear_key_head_dim", 64, "backbone geometry"),
        ("linear_num_value_heads", 32, "execution envelope"),
    ),
)
def test_rocm_profile_rejects_incompatible_backbone_geometry_before_import(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    field: str,
    value: str | int,
