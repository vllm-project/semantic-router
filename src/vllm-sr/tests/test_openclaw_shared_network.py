...
def test_docker_start_vllm_sr_mounts_podman_socket_at_canonical_path(
    ...
    assert rc == 0
    dashboard_cmd = _find_container_run_cmd(captured, "vllm-sr-dashboard-container")
    # podman.sock is mounted at the canonical /var/run/docker.sock so the in-image
    ...
    assert "OPENCLAW_CONTAINER_RUNTIME=docker" in dashboard_cmd
    # Host docker CLI should NOT be mounted in podman mode — the runtime is podman,
    # not docker, so a host docker bin is irrelevant.
    assert not any(
...
def test_docker_start_vllm_sr_resolves_default_podman_socket(tmp_path, monkeypatch):
    ...
    # Pretend /run/podman/podman.sock exists, /var/run/docker.sock does not.
...
def test_docker_start_vllm_sr_warns_when_podman_socket_missing(
    ...
    assert rc == 0
    dashboard_cmd = _find_container_run_cmd(captured, "vllm-sr-dashboard-container")
    # No socket mount when none exists.
    assert not any("docker.sock" in part for part in dashboard_cmd), (
