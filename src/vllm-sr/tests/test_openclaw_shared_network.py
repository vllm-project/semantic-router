...
from cli import core, docker_cli, docker_services, docker_start
...
def test_resolve_runtime_stack_supports_custom_stack_name_and_port_offset():
    ...
    assert stack_layout.router_port == 50251
    assert stack_layout.metrics_port == 9390
    assert stack_layout.dashboard_port == 8900
    assert stack_layout.api_port == 8280
...
def test_start_vllm_sr_uses_isolated_network_and_container_names(monkeypatch):
    ...
    assert start_calls[0][2]["stack_layout"].container_name == "audit-a-vllm-sr-container"
