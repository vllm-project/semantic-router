...
def _socket_stat(*, uid: int = 0, gid: int, mode: int = 0o660) -> SimpleNamespace:
    return SimpleNamespace(st_uid=uid, st_gid=gid, st_mode=stat.S_IFSOCK | mode)
...
def test_dashboard_mounts_runtime_socket_with_private_nonroot_group(
    ...
    attached = container_openclaw_support._attach_container_socket(
        mounts, "docker", str(socket_path)
    )
...
def test_runtime_socket_group_safety_policy():
    for info, expected in (
        ...
        (_socket_stat(uid=-1, gid=1234), False),
        (_socket_stat(gid=0), False),
        (
            SimpleNamespace(st_uid=0, st_gid=1234, st_mode=stat.S_IFREG | 0o660),
            False,
        ),
        (_socket_stat(gid=1234, mode=0o600), False),
        (_socket_stat(gid=1234, mode=0o666), False),
        (_socket_stat(gid=1234, mode=0o760), False),
        (
            SimpleNamespace(st_uid=0, st_gid=1234, st_mode=stat.S_IFLNK | 0o777),
            False,
        ),
    ):
        assert (
...
def test_dashboard_skips_unsafe_runtime_socket(tmp_path, monkeypatch):
    ...
    assert (
        container_openclaw_support._attach_container_socket(
            mounts, "podman", str(socket_path)
        )
        is False
    )
    assert mounts == []
...
def test_dashboard_container_runtime_is_disabled_without_explicit_socket(
    tmp_path, monkeypatch
):
    mounts = []
    env_vars = {}
    ...
    container_openclaw_support.configure_openclaw_support(
        ...
        SimpleNamespace(network_name="test-network"),
        resolve_container_cli=lambda _candidate: (_ for _ in ()).throw(
            AssertionError("container CLI must not be resolved without explicit opt-in")
        ),
    )
...
@pytest.mark.parametrize(
    "socket_path",
    ["relative/docker.sock", "/run/../var/run/docker.sock", " /run/docker.sock"],
)
def test_dashboard_rejects_noncanonical_container_socket(
    socket_path, tmp_path, monkeypatch
):
    monkeypatch.setenv("VLLM_SR_CONTAINER_SOCKET", socket_path)

    with pytest.raises(ValueError, match="VLLM_SR_CONTAINER_SOCKET"):
        container_openclaw_support.configure_openclaw_support(
            [],
            {},
            str(tmp_path),
            ...
            "docker",
            SimpleNamespace(network_name="test-network"),
            resolve_container_cli=lambda _candidate: None,
        )
