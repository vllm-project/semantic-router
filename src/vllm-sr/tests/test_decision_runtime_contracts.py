...
@pytest.mark.parametrize("question_id", ["billing"])
@pytest.mark.parametrize("instructions", [None, pytest.param(..., id="missing")])
def test_noul_instructions_are_required_and_non_null(question_id, instructions):
    payload = request_payload()
    if instructions is ...:
...
@pytest.mark.parametrize("question_id", ["category", "urgency"])
@pytest.mark.parametrize("instructions", [None, pytest.param(..., id="missing")])
def test_choice_and_score_require_non_null_instructions(question_id, instructions):
    payload = request_payload()
    if instructions is ...:
        payload["questions"][question_id].pop("instructions")
    else:
    ...
    batch = batch_request_payload()
    batch["questions"] = payload["questions"]
    with pytest.raises(ValidationError):
        SystemOneBatchRequest.model_validate(batch)
...
def test_request_relative_response_validation_accepts_concentrated_answers():
    payload = request_payload()
    payload["questions"] = {
        "category": {
            "type": "choice",
    ...
    response = SystemOneResponse(
        ...
        answers={
            "category": ChoiceAnswer(
                ...
                choice="first",
                confidence=1.0,
                probabilities={"first": 1.0, "second": 0.0},
            ),
            "urgency": ScoreAnswer(
                type="score",
                score=0.0,
    ...
    response.answers["category"].confidence = 0.0
    with pytest.raises(ResponseContractError, match="confidence"):
...
@pytest.mark.parametrize("probabilities", [[], [0.5], [0.3, 0.3], [float("nan"), 1.0]])
