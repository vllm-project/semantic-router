...
def test_bounded_job_quantum_spreads_large_job_completions():
    async def completion_batches(quantum: int) -> list[int]:
        executor = RecordingExecutor()
        backend = PhysicalBatchBackend(
            MODEL,
            executor,
            physical_batch_size=8,
            max_rows_per_job_turn=quantum,
            coalesce_seconds=0,
        )
        ...
        try:
            results = await asyncio.gather(
                *(
                    backend.infer(request(str(index), question_ids))
                    for index in range(32)
                )
            )
        finally:
            await backend.aclose()
        assert all(len(result.predictions) == 32 for result in results)
        assert len(executor.calls) == 128
        assert all(len(call) == 8 for call in executor.calls)
        rows_seen = {str(index): 0 for index in range(32)}
        completed = {}
        for batch_index, call in enumerate(executor.calls, start=1):
            for row in call:
                state = str(row.state)
                rows_seen[state] += 1
                if rows_seen[state] == 32:
                    completed[state] = batch_index
        return sorted(completed.values())

    ordinary = asyncio.run(completion_batches(1))
    bounded = asyncio.run(completion_batches(32))
    assert ordinary[0] >= 125
    assert bounded[0] == 4
    assert bounded[15] == 64
    assert bounded[-1] == 128


def test_job_quantum_rotates_after_four_forwards_without_starvation():
    async def scenario():
        executor = RecordingExecutor()
        backend = PhysicalBatchBackend(
            MODEL,
        ...
        try:
            await asyncio.gather(
                backend.infer_batch(
                    tuple(
                        BackendBatchRequest(
                            state_id=f"first-{index}",
                            request=request(f"first-{index}", question_ids),
                        )
                ...
                backend.infer_batch(
                    tuple(
                        BackendBatchRequest(
                            state_id=f"second-{index}",
                            request=request(f"second-{index}", question_ids),
                        )
                        for index in range(2)
                    )
                ),
            ...
            await backend.aclose()
        # Each 64-row job receives at most four consecutive B8 forwards before
        # the other queued job is served.
        assert [call[0].state.split("-")[0] for call in executor.calls] == [
            "first",
            "first",
            "first",
            "first",
...
@pytest.mark.parametrize("quantum", [0, -1, 33, True, 1.5])
def test_job_quantum_rejects_unbounded_or_invalid_values(quantum):
    with pytest.raises(ValueError, match="max_rows_per_job_turn"):
        PhysicalBatchBackend(
            MODEL,
            RecordingExecutor(),
            physical_batch_size=8,
...
def test_incompatible_batch_keys_are_partitioned_without_losing_fairness(quantum):
    async def scenario():
...
def test_cancelled_job_releases_pending_capacity(quantum):
    async def scenario():
        gate = asyncio.Event()
        executor = RecordingExecutor(gate=gate)
        backend = PhysicalBatchBackend(
