...
@pytest.mark.parametrize("question_type", ["choice", "score"])
def test_structured_instructions_render_for_both_families(question_type) -> None:
    body = {
        "type": question_type,
        "instructions": {"question": "Choose carefully"},
        "criteria": (
    ...
    question_cls = ChoiceQuestion if question_type == "choice" else ScoreQuestion
    question = question_cls.model_validate(body)

    qwen_row = build_model_input(
        question_id="one",
        state="context",
        question=question,
        **QWEN_POLICY,
    )
    vela_row = build_model_input(
        question_id="one",
        state="context",
        question=question,
        **VELA_POLICY,
    )
    assert qwen_row.instructions == {"question": "Choose carefully"}
    assert len(qwen_row.candidates) == 2
    assert (
        'Question:\n{"question":"Choose carefully"}\nOptions:'
        in qwen_segments(qwen_row).prefix
    )
    assert vela_row.instructions == {"question": "Choose carefully"}
    assert (
        vela_text_input(vela_row).question
        == f'{question_type} question: {{"question":"Choose carefully"}}'
    )
