...
def test_fast_config_match_rejects_every_changed_launch_field(
    ...
    assert binder._config_matches(SimpleNamespace(**expected), expected)
    altered = {**expected, field: changed}
    assert not binder._config_matches(SimpleNamespace(**altered), expected)
    assert not binder._config_matches(altered, expected)
    assert not binder._config_matches({**expected, "unexpected": 1}, expected)
...
def test_digest_reads_complete_file_in_bounded_chunks(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path = tmp_path / "large-config.json"
    content = bytes(range(256)) * 513
    path.write_bytes(content)
    original_pread = os.pread
    reads = []

    def record_read(descriptor: int, count: int, offset: int) -> bytes:
        reads.append((count, offset))
        return original_pread(descriptor, count, offset)

    monkeypatch.setattr(binder.os, "pread", record_read)
    assert binder._sha256_file(path) == _sha256(content)
    assert len(reads) > 2
    assert all(count == 1 << 16 for count, _ in reads)
    assert [offset for _, offset in reads] == [0, 1 << 16, 2 << 16, len(content)]


def test_replacement_during_digest_stops_cached_launch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    profile, package, configs = _profile(tmp_path)
    modules, kernel, _, calls = _fake_fla_modules(package, configs)
    ...
    def replace_after_read(descriptor: int, count: int, offset: int) -> bytes:
        nonlocal replaced
        block = original_pread(descriptor, count, offset)
        if not replaced:
            replacement = tmp_path / "same-config.json"
            replacement.write_bytes(profile.kernel_config_path.read_bytes())
            replacement.replace(profile.kernel_config_path)
            replaced = True
        return block

    monkeypatch.setattr(binder.os, "pread", replace_after_read)
    with pytest.raises(binder.QwenRocmBindingError, match="configuration changed"):
        kernel.run(key=legal)
    assert replaced and calls == [legal]
