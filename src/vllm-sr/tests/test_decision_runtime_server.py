...
def test_profiled_qwen_graph_receives_verified_artifact_identity(
    ...
    from decision_runtime import qwen35_torch, runtime_factory  # noqa: PLC0415
    from decision_runtime.families import qwen35  # noqa: PLC0415
...
def test_qwen_calibration_is_read_from_verified_snapshot(tmp_path: Path):
    from decision_runtime.families.base import FamilyLoadError  # noqa: PLC0415
    ...
    assert qwen_temperature(artifact, fallback=9.0) == 1.75
    (tmp_path / "temperature.json").write_text(
        json.dumps({"temperature": -1}), encoding="utf-8"
    )
    with pytest.raises(FamilyLoadError, match="temperature"):
        qwen_temperature(artifact, fallback=9.0)
    (tmp_path / "temperature.json").write_text(
        json.dumps({"temperature": 10**400}), encoding="utf-8"
    )
    with pytest.raises(FamilyLoadError, match="temperature"):
        qwen_temperature(artifact, fallback=9.0)
