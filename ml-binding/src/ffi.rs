...
//!
//! This module provides inference-only functions for KNN, KMeans, SVM, and MLP.
...
use crate::{KMeansSelector, KNNSelector, MLPSelector, SVMSelector};
...
#[no_mangle]
pub extern "C" fn ml_mlp_new() -> *mut MLPHandle {
...
#[no_mangle]
pub extern "C" fn ml_mlp_new_with_device(device_type: c_int) -> *mut MLPHandle {
...
/// Free MLP selector
#[no_mangle]
pub extern "C" fn ml_mlp_free(handle: *mut MLPHandle) {
    if !handle.is_null() {
        unsafe { drop(Box::from_raw(handle)) };
    }
}
...
#[no_mangle]
pub extern "C" fn ml_mlp_is_trained(handle: *const MLPHandle) -> c_int {
    if handle.is_null() {
        return 0;
    }
    let selector = unsafe { &(*handle).0 };
...
/// Save MLP to JSON
#[no_mangle]
pub extern "C" fn ml_mlp_to_json(handle: *const MLPHandle) -> *mut c_char {
    if handle.is_null() {
        return ptr::null_mut();
    }
    let selector = unsafe { &(*handle).0 };
...
#[no_mangle]
pub extern "C" fn ml_mlp_from_json(json: *const c_char) -> *mut MLPHandle {
    let json_str = match unsafe { c_str_to_string(json) } {
        Some(s) => s,
        None => return ptr::null_mut(),
    };
...
/// device_type: 0 = CPU, 1 = CUDA (GPU), 2 = Metal (Apple Silicon)
#[no_mangle]
pub extern "C" fn ml_mlp_from_json_with_device(json: *const c_char, device_type: c_int) -> *mut MLPHandle {
    use candle_core::Device;
