...
\begin{enumerate}
  ...
  \item \textbf{Compilation}: The AST is compiled to the internal \texttt{RouterConfig} structure. This involves: (a) mapping each \texttt{SIGNAL} block to the appropriate signal configuration (keyword rules, embedding rules, domain categories, etc.); (b) flattening the Boolean expression tree in each \texttt{WHEN} clause into a \texttt{RuleCombination} tree with \texttt{AND}/\texttt{OR}/\texttt{NOT} operators; (c) resolving plugin references against top-level templates with field-level merge semantics (route-local fields override template defaults); and (d) mapping \texttt{BACKEND} blocks to endpoint, embedding model, or semantic cache configurations based on the backend type keyword.
...
\begin{tikzpicture}[
  ...
  % Authoring and standalone outputs
  \node[box, fill=red!6, font=\scriptsize, minimum width=1.8cm] (eyaml) at (10.2, -1.8) {Recipe YAML};
  \node[box, fill=red!6, font=\scriptsize, minimum width=1.8cm] (emerge) at (12.5, -1.8) {Base merge};
  ...
  \draw[arr] (cfg.south) -- ++(0,-0.5) -| (emerge.north);
  ...
  \caption{The DSL compilation pipeline. Source text is lexed, parsed into a resolved AST, and compiled to the internal \texttt{RouterConfig}. The emitter produces one model-free Recipe document, which can optionally replace the sole Recipe document in a complete v0.3 base manifest. The decompiler reverses the pipeline for round-trip editing, while the validator operates directly on the AST.}
  \label{fig:dsl_pipeline}
\end{figure*}

\subsection{Recipe Document Emission}
\label{sec:dsl_emission}

The DSL owns one narrow output contract: a model-free Recipe document containing signals, projections, decisions, and the routing strategy. Physical Models and their provider bindings remain control-plane resources, while Entrypoint assignments bind those Models to Recipe decisions. For complete-manifest compilation, the \texttt{--base} path replaces the document of exactly one Recipe in a complete v0.3 manifest; ambiguous multi-Recipe bases are rejected. Kubernetes and Helm artifacts are produced by their respective deployment controllers from the complete v0.3 resource model rather than by lossy DSL-specific emitters.
