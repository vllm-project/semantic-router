...
\begin{figure}[ht]
    ...
    % Frozen base model
    \node[box, fill=gray!15, minimum width=2.6cm, thick,
          minimum height=1.1cm] (base) at (0, 3)
      {\textbf{Frozen Base Model}\\$W$};

    % Adapter group 1 (PII) -- left
    \node[adapter, fill=blue!10] (a1) at (3.2, 3.9) {$A_{\text{pii}}$\;\scriptsize(rank $r$)};
    \node[adapter, fill=blue!10] (b1) at (3.2, 2.1) {$B_{\text{pii}}$\;\scriptsize(rank $r$)};

    % Dots
    \node[font=\large] at (5.0, 3) {\dots};

    % Adapter group N (intent) -- right
    \node[adapter, fill=yellow!20] (an) at (6.8, 3.9) {$A_{\text{intent}}$\;\scriptsize(rank $r$)};
    \node[adapter, fill=yellow!20] (bn) at (6.8, 2.1) {$B_{\text{intent}}$\;\scriptsize(rank $r$)};

    % Adapters bounding box
    \node[rectangle, draw, dashed, gray!60, rounded corners=3pt,
    ...
    % Outputs
    \node[outbox, fill=blue!5] (o1) at (1.2, -0.8) {\scriptsize Task 1: $W'\mathbf{x}$};
    \node[outbox, fill=yellow!10] (on) at (5.5, -0.8) {\scriptsize Task $N$: $W'\mathbf{x}$};
...
Huamin Chen, Yossi Ovadia, Xunzhuo, yehudit1987, abdallahsamabd, Yue Zhu, OneZero-Y, shown, Xunzhuo, asaadbalum, Jared, aeft, Wilson Wu, Tien Nguyen, Srinivas A, Qiping Pan, Nengxing Shen, Liav Weiss, GuanMu, ERIK, Alex Wang, Sanjeev Rampal, Zohaib Hassnain.
