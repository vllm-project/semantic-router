...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\linewidth]{figures/lora_architecture.svg}
    \caption{LoRA-based MoM Inference Architecture. The input query is processed by a single, shared frozen base model ($W$). Simultaneously, task-specific low-rank adapters ($A \times B$) calculate perturbations. The outputs are summed element-wise to produce final task-specific classifications, enabling multi-task support with minimal memory overhead.}
    \label{fig:lora_arch}
\end{figure}
