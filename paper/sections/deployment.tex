...
\noindent\textbf{Kubernetes: Sidecar gateway.}
Envoy runs as a sidecar container alongside the router in the same pod.
The ExtProc filter connects via localhost gRPC. In this topology, the Envoy instance is private to the semantic router and separate from cluster level Envoy gateways. Routes are internally configured and injected by the semantic router into Envoy.
Deployed via Helm charts with configurable replicas, resource limits, cache backends, and autoscaling.

\noindent\textbf{Kubernetes: Shared gateway.}
The router runs as an independent service behind an existing Istio or Envoy Gateway deployment, referenced via the gateway's ExtProc configuration. In this topology, there is no private Envoy within the semantic router pod. The cluster's Envoy based gateway is used, via Istio or another controller. Routes are explicitly configured at the cluster gateway level via APIs such as Kubernetes Gateway API.
This topology shares the gateway infrastructure across multiple services.
