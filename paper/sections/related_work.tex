...
\noindent\textbf{Contrastive selection.}
...
\noindent\textbf{RL-based routing.}
Router-R1~\cite{zhang2025routerr1} formulates multi-LLM routing as a sequential decision process, using rule-based reward RL for multi-round routing and aggregation.
...
\noindent\textbf{Legitimacy verification.}
PRISM~\cite{saidi2025prism} introduces a DNS-inspired constraint protocol (the 153-key) that forces each model to declare its domain boundaries and formally refuse out-of-scope queries.
While vLLM-SR answers ``which model is best suited?'', PRISM answers ``is this model legitimate to respond?''
We integrate PRISM as a configurable verification layer that can operate before selection (coarse filter), after selection (fine filter), or both (hybrid), complementing---rather than replacing---the selection algorithms.
...
Prompt injection defenses range from perplexity filtering and input preprocessing~\cite{jain2023baseline} to fine-tuned safety classifiers~\cite{inan2023llamaguard}.
...
Semantic caching for LLMs uses embedding similarity to match incoming queries against previously seen requests, avoiding redundant model invocations.
