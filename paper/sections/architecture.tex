...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}[
    layer/.style={rectangle, draw, thick, rounded corners=3pt,
                  minimum height=1.4cm, minimum width=4cm, align=center,
                  inner sep=6pt, font=\small},
    io/.style={rectangle, draw, rounded corners=2pt, fill=gray!12,
  ...
  % Configuration bar
  \node[rectangle, draw, thick, rounded corners=3pt, fill=yellow!8,
        minimum width=16.8cm, inner sep=5pt, font=\small]
    (cfg) at (6.4, 3.2)
  ...
  % Input
  \node[io] (req) at (-1.6, 0.8) {Request\\$r$};

  % Layer boxes -- spread out with 5.2cm spacing
  \node[layer, fill=blue!8]   (L1) at (2.4,  0.8) {\textbf{Layer 1}\\Signal Extraction};
  \node[layer, fill=green!8]  (L2) at (6.8,  0.8) {\textbf{Layer 2}\\Decision Engine};
  \node[layer, fill=orange!8] (L3) at (11.2, 0.8) {\textbf{Layer 3}\\Plugin Chain};

  % Output
  \node[io] (out) at (14.8, 0.8) {Provider\\Endpoint};
  ...
  % Detail boxes -- anchored to top, well below the layer boxes
  \node[detail, anchor=north] (d1) at (2.4, -0.5) {%
  ...
  % Feedback loop -- runs below the detail boxes
  \draw[darr, rounded corners=6pt]
    (out.south) -- ++(0,-4.0) -| (L1.south west);
