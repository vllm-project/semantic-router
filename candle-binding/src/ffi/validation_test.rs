...
fn test_validate_text_input_valid(
    ...
    // Clean up
    unsafe { free_validation_result(result) };
}
...
fn test_validate_confidence_threshold_various_values(
    ...
    let result = unsafe { validate_confidence_threshold(confidence, path_type) };
...
fn test_validate_confidence_threshold_out_of_range_low() {
    let result = unsafe { validate_confidence_threshold(-0.1, 0) };
...
fn test_validate_confidence_threshold_out_of_range_high() {
    let result = unsafe { validate_confidence_threshold(1.1, 0) };
...
fn test_validate_confidence_threshold_boundary_values() {
    let result_zero = unsafe { validate_confidence_threshold(0.0, 1) };
    let _ = result_zero.is_valid;
    unsafe { free_validation_result(result_zero) };

    let result_one = unsafe { validate_confidence_threshold(1.0, 1) };
    let _ = result_one.is_valid;
    unsafe { free_validation_result(result_one) };
}
...
fn test_validate_memory_parameters(
    ...
    let result = unsafe { validate_memory_parameters(size, alignment) };
...
fn test_validation_error_codes() {
    ...
    unsafe { free_validation_result(result_null) };

    let result_invalid_confidence = unsafe { validate_confidence_threshold(-1.0, 0) };
    assert_eq!(
        result_invalid_confidence.error_code,
        ERROR_INVALID_CONFIDENCE
    );
    unsafe { free_validation_result(result_invalid_confidence) };
}
