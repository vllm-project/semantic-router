//! Local ModernBERT Core Implementation with F16/BF16 dtype fixes
//!
//! This is a local copy of candle-transformers ModernBERT with fixes for:
//! - Dynamic dtype support (F16 for CPU, BF16 for GPU)
//! - Proper attention mask dtype conversion
//!
//! The upstream candle-transformers hardcodes DType::F32 for attention masks,
//! which causes dtype mismatch errors when loading models with F16/BF16.
//! See: https://github.com/huggingface/candle/blob/main/candle-transformers/src/models/modernbert.rs
...
/// Prepare 4D attention mask from 2D padding mask
/// 
/// FIX: Uses F32 for intermediate computation to avoid scalar-tensor dtype issues,
/// then converts to target dtype at the end.
fn prepare_4d_attention_mask(
    ...
    // Use F32 for intermediate computation (scalar 1.0 is f64, needs F32 tensor)
...
/// Get local attention mask for sliding window attention
...
/// ModernBERT backbone encoder
/// 
/// This is the core transformer encoder that processes input tokens.
/// FIX: Gets dtype from embeddings (not input U32) for proper F16/BF16 support.
#[derive(Clone)]
...
impl ModernBert {
    ...
    /// Forward pass through the ModernBERT encoder
    /// 
    /// FIX: Gets dtype from embeddings output (F16/BF16/F32) not input (U32)
    /// to properly create attention masks with matching dtype.
    pub fn forward(&self, xs: &Tensor, mask: &Tensor) -> Result<Tensor> {
        let seq_len = xs.shape().dims()[1];
        
        // Get embeddings first to determine dtype for attention mask
        // FIX: This ensures we use the model's dtype (F16/BF16/F32), not input dtype (U32)
...
/// ModernBERT for masked language modeling
#[derive(Clone)]
...
/// ModernBERT for sequence classification
/// 
/// FIX: Uses dynamic dtype from model output for mask operations
#[derive(Clone)]
