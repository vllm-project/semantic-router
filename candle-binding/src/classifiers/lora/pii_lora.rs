...
impl PIILoRAClassifier {
    ...
    pub fn new(model_path: &str, use_cpu: bool) -> Result<Self> {
        ...
        let backend = if Self::is_modernbert(model_path) {
            let classifier = TraditionalModernBertTokenClassifier::new(model_path, use_cpu).map_err(
                |e| {
            ...
            let classifier = HighPerformanceBertTokenClassifier::new(model_path, num_classes, use_cpu).map_err(
                |e| {
                    let unified_err = model_error!(
                        ModelErrorType::LoRA,
                        "PII token classifier creation",
