...
type candleImageResolverFunc func(context.Context, string, string) ([]netip.Addr, error)

func (resolve candleImageResolverFunc) LookupNetIP(
	ctx context.Context,
	network string,
	host string,
) ([]netip.Addr, error) {
...
func TestCandleRFC6052ExtractionSupportsEveryStandardPrefixLength(t *testing.T) {
	t.Parallel()

	ipv4 := netip.MustParseAddr("203.0.113.9")
	for _, prefix := range candleRFC6052TestPrefixes() {
		prefix := prefix
		t.Run(prefix.String(), func(t *testing.T) {
			t.Parallel()
			translated := synthesizeCandleRFC6052Address(prefix, ipv4)
			if prefix.Bits() != 96 {
	...
	malformed := synthesizeCandleRFC6052Address(prefix, ipv4).As16()
	malformed[8] = 1
	...
	if isPublicCandleImageDestination(netip.MustParseAddr("2606:4700:4700::1111%en0")) {
		t.Fatal("scoped IPv6 destinations must be rejected")
	}
}

func TestCandleRFC7050InfersEveryStandardPrefixLength(t *testing.T) {
	t.Parallel()

	for _, prefix := range append(
		[]netip.Prefix{rfc6052WellKnownTranslationPrefix},
		candleRFC6052TestPrefixes()...,
	) {
		prefix := prefix
		t.Run(prefix.String(), func(t *testing.T) {
			t.Parallel()
...
func TestCandleNetworkSpecificPref64AppliesPublicIPv4Policy(t *testing.T) {
	t.Parallel()

	for _, prefix := range candleRFC6052TestPrefixes() {
		for _, ipv4 := range []string{"10.0.0.1", "127.0.0.1", "169.254.169.254"} {
			translated := synthesizeCandleRFC6052Address(prefix, netip.MustParseAddr(ipv4))
			if isPublicCandleImageDestinationWithPrefixes(translated, []netip.Prefix{prefix}) {
				t.Fatalf("%s translation of non-public %s must be blocked", prefix, ipv4)
			}
		}
		translated := synthesizeCandleRFC6052Address(prefix, netip.MustParseAddr("1.1.1.1"))
		if !isPublicCandleImageDestinationWithPrefixes(translated, []netip.Prefix{prefix}) {
...
func TestCandleRFC7050DiscoveryNoDNS64AndFailures(t *testing.T) {
	t.Parallel()

	noDNS64 := candleImageResolverFunc(func(_ context.Context, network, host string) ([]netip.Addr, error) {
	...
	failing := candleImageResolverFunc(func(context.Context, string, string) ([]netip.Addr, error) {
	...
	invalidA := candleImageResolverFunc(func(_ context.Context, network, _ string) ([]netip.Addr, error) {
		if network == "ip6" {
			return nil, &net.DNSError{IsNotFound: true}
		}
...
func TestCandleRFC7050RejectsAmbiguousAndLocalUsePrefixes(t *testing.T) {
	t.Parallel()
	...
	if _, err := inferRFC7050Prefixes([]netip.Addr{
		synthesizeCandleRFC6052Address(localUse, rfc7050IPv4OnlyAddresses[0]),
	}); err == nil {
		t.Fatal("RFC 8215 local-use prefix must not be accepted as a public PREF64")
	}
}

func TestCandlePref64CacheCoalescesConcurrentLookupsAndExpires(t *testing.T) {
	prefix := netip.MustParsePrefix("2001:4860:1234:5678::/64")
...
func TestCandleImageDestinationsFailClosedWhenPref64CannotBeDiscovered(t *testing.T) {
	t.Parallel()

	var lookups atomic.Int32
	resolver := candleImageResolverFunc(func(context.Context, string, string) ([]netip.Addr, error) {
		lookups.Add(1)
...
func TestCandleImageDestinationsUseDiscoveredPref64(t *testing.T) {
	t.Parallel()
	...
	if candleImageDestinationsArePublic(context.Background(), []netip.Addr{private}, cache) {
	...
	if !candleImageDestinationsArePublic(context.Background(), []netip.Addr{public}, cache) {
...
func candleRFC6052TestPrefixes() []netip.Prefix {
	return []netip.Prefix{
...
func synthesizeCandleRFC6052Address(prefix netip.Prefix, ipv4 netip.Addr) netip.Addr {
