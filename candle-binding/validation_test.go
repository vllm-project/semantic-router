...
func TestValidateSemanticHelpers(t *testing.T) {
	...
	t.Run("validateTargetLayer", func(t *testing.T) {
		if err := validateTargetLayer(0); err != nil {
			t.Fatalf("expected 0 to be valid, got %v", err)
		}
		if err := validateTargetLayer(6); err != nil {
			t.Fatalf("expected 6 to be valid, got %v", err)
		}
	...
	t.Run("validateImageTensor", func(t *testing.T) {
		...
		if math.MaxInt > math.MaxInt32 {
			...
			if err := validateImageTensor(pixelData, 512, huge, 0); err == nil || !strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("expected width ABI overflow error, got %v", err)
			}
		}
		// Test overflow-safe shape multiplication
		if err := validateImageTensor(pixelData, math.MaxInt32, math.MaxInt32, 0); err == nil || !strings.Contains(err.Error(), "overflow integer range") {
	...
	t.Run("validateAudioTensor", func(t *testing.T) {
		...
		if math.MaxInt > math.MaxInt32 {
			...
			if err := validateAudioTensor(melData, 80, huge, 0); err == nil || !strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("expected timeFrames ABI overflow error, got %v", err)
			}
		}
		if math.MaxInt == math.MaxInt32 {
			// On 32-bit platforms, 1<<16 * 1<<16 overflows 32-bit integer
			if err := validateAudioTensor(melData, 1<<16, 1<<16, 0); err == nil || !strings.Contains(err.Error(), "overflow integer range") {
				t.Fatalf("expected audio shape overflow error on 32-bit, got %v", err)
			}
		}
	})
...
func TestMultiModalValidationRunsInBothModes(t *testing.T) {
	...
	t.Run("out-of-range ABI values rejected", func(t *testing.T) {
		if math.MaxInt > math.MaxInt32 {
			...
			if _, err := MultiModalEncodeImage(make([]float32, 3*512*512), 512, 512, huge); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("MultiModalEncodeImage: want ABI range error, got %v", err)
			}
			if _, err := MultiModalEncodeAudio(make([]float32, 80*100), 80, 100, huge); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("MultiModalEncodeAudio: want ABI range error, got %v", err)
			}
			if _, err := MultiModalEncodeImageFromBytes([]byte{1, 2, 3}, huge); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("MultiModalEncodeImageFromBytes: want ABI range error, got %v", err)
			}
			if _, err := MultiModalEncodeImageFromBase64("valid", huge); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("MultiModalEncodeImageFromBase64: want ABI range error, got %v", err)
			}
			if _, err := MultiModalEncodeImageFromURL("http://example.com/img.png", huge); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("MultiModalEncodeImageFromURL: want ABI range error, got %v", err)
			}
		}
		// Shape overflow check
		if _, err := MultiModalEncodeImage(make([]float32, 10), math.MaxInt32, math.MaxInt32, 0); err == nil ||
			!strings.Contains(err.Error(), "overflow integer range") {
			t.Fatalf("MultiModalEncodeImage: want overflow error, got %v", err)
		}
		if math.MaxInt == math.MaxInt32 {
			if _, err := MultiModalEncodeAudio(make([]float32, 10), 1<<16, 1<<16, 0); err == nil ||
				!strings.Contains(err.Error(), "overflow integer range") {
				t.Fatalf("MultiModalEncodeAudio: want overflow error, got %v", err)
			}
		}
	})
}
...
func TestSharedEmbeddingValidationRunsInBothModes(t *testing.T) {
	...
	t.Run("GetEmbedding APIs validation", func(t *testing.T) {
		...
		if math.MaxInt > math.MaxInt32 {
			...
			if _, err := CalculateSimilarityBatch("q", []string{"a"}, 1, "auto", huge); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("CalculateSimilarityBatch: want targetDim ABI range error, got %v", err)
			}
			if _, err := CalculateEmbeddingSimilarity("a", "b", "auto", huge); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("CalculateEmbeddingSimilarity: want targetDim ABI range error, got %v", err)
			}
			if _, err := GetEmbeddingWithDim("valid", 0.5, 0.5, huge); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("GetEmbeddingWithDim: want targetDim ABI range error, got %v", err)
			}
			if _, err := GetEmbeddingWithMetadata("valid", 0.5, 0.5, huge); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("GetEmbeddingWithMetadata: want targetDim ABI range error, got %v", err)
			}
			if _, err := GetEmbeddingWithModelType("valid", "qwen3", huge); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("GetEmbeddingWithModelType: want targetDim ABI range error, got %v", err)
			}
			if _, err := GetEmbedding2DMatryoshka("valid", "qwen3", huge, 0); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("GetEmbedding2DMatryoshka: want targetLayer ABI range error, got %v", err)
			}
			if _, err := GetEmbedding2DMatryoshka("valid", "qwen3", 0, huge); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("GetEmbedding2DMatryoshka: want targetDim ABI range error, got %v", err)
			}
			if _, err := GetEmbeddingBatched("valid", "qwen3", huge); err == nil ||
				!strings.Contains(err.Error(), "exceeds maximum 32-bit integer range") {
				t.Fatalf("GetEmbeddingBatched: want targetDim ABI range error, got %v", err)
			}
		}
	})
}
