...
The classifier workflow is the hard gate for required review-brief context:

- PRs at or above the size threshold must provide a valid review brief.
- Use `Review brief: N/A` when no brief is required.
- A referenced brief counts only when it resolves to readable, non-empty review
  context from the base branch or the PR.

The AI memory review runs only when the classifier finds a valid review brief.
During the initial rollout it is advisory: model failures, quota errors, or
missing AI output do not block merge after the deterministic classifier hard
gate has passed. The workflows still publish labels and comments first so
authors can see deterministic classifier failures directly on the PR before the
classifier check reports failure.
