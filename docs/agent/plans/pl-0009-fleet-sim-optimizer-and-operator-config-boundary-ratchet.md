...
- [x] `S003` Split fleet-sim optimizer ownership so analytical sizing, DES calibration, power or flexibility analysis, and report helpers stop growing in one `base.py`.
- [x] `S004` Narrow fleet-sim export ownership so `optimizer/__init__.py` and `fleet_sim/__init__.py` stop mirroring an overly broad optimizer surface.
- [x] `S005` Split operator config ownership so CRD schema families, webhook validation helpers, controller-side canonical translation, and sample-fixture expectations stop collapsing into the current hotspots.
- [x] `S006` Run the required harness and subsystem validation for the remaining work and close TD027 and TD028 when the narrowed seams are proven.
...
- 2026-04-06: TD028 is retired. Operator admission validation now fans out through family-specific helpers, controller-side canonical translation now fans out through adjacent family translators, and sample assertions now route through sample-specific helpers.
- 2026-04-06: TD027 is retired. Grid-flex and TPW analyses now live in sibling optimizer modules, the optimizer package owns the curated root-facing export list, and fleet-sim docs/tests now reference those narrower seams directly.
- Next loop target: plan complete; reopen only if new optimizer or operator boundary work falls outside the narrowed seams captured here.
...
- The operator side of this plan closed first, but `S006` stayed open until the fleet-sim optimizer side also converged on narrower sibling modules plus single-owner export policy.
