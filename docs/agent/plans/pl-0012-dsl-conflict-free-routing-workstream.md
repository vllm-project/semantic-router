...
- 2026-05-24: `TD036` was rechecked against current source and resolved by the narrowed contract instead of a metadata expansion. `DECISION_TREE` remains authoring sugar, runtime config remains flat `routing.decisions`, and docs/tests now explicitly cover the non-lossless tree-shape round-trip behavior.
