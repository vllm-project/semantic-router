...
- Current gap statement
  - The repo now has shared first-slice contract proof across router authoring compile, CLI merge, DSL/user-YAML emission, dashboard canonical save/load, and authoring-to-CRD emission, but operator reconciliation still does not consume the canonical contract directly.
...
- `2026-03-09`: `C006` advanced by adding the first operator-side `spec.authoringConfig` path. The webhook now validates canonical authoring payloads and rejects mixed-source combinations with `spec.vllmEndpoints` or overlapping legacy config fields; controller config generation can compile the shared first-slice authoring contract directly into runtime `config.yaml`; and the operator samples/tests now cover this common-path Kubernetes authoring flow.
- `2026-03-09`: `C003` advanced again on the router runtime side by adding a direct file-loading seam for the canonical authoring first slice. File-based router startup and config validation now accept shared first-slice authoring YAML, compile it into runtime `RouterConfig`, and apply the same runtime finalization/validation path before startup; legacy runtime YAML remains the fallback path, while richer authoring files still fail fast until the canonical contract grows beyond the first slice.
...
- `2026-03-09`: `C004` advanced again by aligning the embedded CLI `router-defaults.yaml` reference with that same `model_selection` runtime/dashboard shape, removing the stale `default_algorithm` / `llm_candidates_path` / `training_data_path` sample fields so new bootstrap workspaces no longer inherit a divergent top-level contract example.
- `2026-03-09`: `C004` advanced again by moving top-level leftover-key rejection into the parser and deleting the merger's generic top-level passthrough branch, so parser normalization plus explicit typed compat extraction is now the only supported dual-read path for top-level legacy keys.
- `2026-03-09`: `C004` advanced again by adding an explicit CLI projection for the canonical first slice plus `vllm-sr config authoring`, so the canonical contract is now a real CLI-emitted artifact instead of living only in the Go authoring seam and fixture tests.
- `2026-03-09`: `C004` advanced again by adding a dedicated CLI first-slice runtime compiler seam (`authoring_projection -> authoring_runtime_compile -> merger`), so listeners, keyword rules, provider-model runtime keys, and the base decision graph now compile through one canonical boundary instead of four scattered `translate_*` merge branches; decision plugins/algorithms and provider `external_models` are explicitly re-attached as temporary non-first-slice extensions.
- `2026-03-09`: `C004` is still intentionally left open after that seam reduction because, even with the embedded defaults aligned, the steady-state CLI write path still compiles through handwritten translation into the legacy flat router dict instead of writing the canonical authoring contract directly.
- `2026-03-09`: `C005` advanced by moving dashboard `config.yaml` validation onto the Python CLI parse/validate path when the saved payload uses the canonical providers-based shape, while legacy flat configs still fall back to the Go router parser. This removes one of the most important dashboard/backend validation mismatches without requiring an all-at-once UI rewrite.
- `2026-03-09`: `C005` advanced again by adding a dashboard Python-CLI bridge for canonical config load/write: `GET /api/router/config/all` now prefers the normalized CLI-owned nested DTO, and canonical dashboard saves now render back through the CLI serializer instead of writing raw YAML maps. Generic deep-merge remains only for legacy-shaped update payloads and other fallback paths.
...
- `2026-03-09`: `C006` advanced again by adding an explicit `authoring -> CRD` compiler seam in Go. The shared first-slice authoring fixture can now emit a `SemanticRouter` CRD envelope directly, so Kubernetes translation no longer has to start only from handwritten flat `RouterConfig` examples when proving the common contract path.
