...
The vLLM Semantic Router originally implemented routing logic tightly coupled with Envoy's External Processing (ExtProc) protocol. The core routing functionality (classification, caching, tool selection, replay recording) was embedded within the ExtProc implementation, making it difficult to:

1. Access router functionality without deploying Envoy proxy
2. Support alternative protocols (HTTP REST, nginx, gRPC)
3. Test routing logic independently
4. Extend the system with new protocol adapters
...
│             │                │                           │
│  ┌──────────▼───────┐   ┌────▼──────────┐               │
│  │  ExtProc Adapter │   │  HTTP Adapter  │               │
│  │  (Port 50051)    │   │  (Port 9000)   │               │
│  │  - gRPC          │   │  - REST        │               │
│  │  - Envoy Proto   │   │  - OpenAI API  │               │
│  └──────────┬───────┘   └────┬──────────┘               │
│             │                │                           │
│             └────────┬────────┘                           │
│                      │                                   │
...
1. **Nginx Adapter**
   - NJS module integration
   - Similar to ExtProc but for nginx
...
1. **Native gRPC Adapter**
   - Custom gRPC service definition
   - More efficient than ExtProc
...
- [Adapter Configuration Guide](../ADAPTER_CONFIGURATION.md)
...
- **Authentication:** Handled at adapter level (future work)
