...
| MD-27 | Exact-commit AMD image regression found that the Dashboard backend's selective Semantic Router package copy list omitted the new shared Unicode JSON validation package, so host tests passed while the canonical Dashboard container failed to compile | Copy the required package into the Dashboard builder and extend the Dockerfile surface contract to enumerate every intentionally imported Semantic Router package | Audit PR closes the concrete image/source drift; TEST-03 retains the permanent fail-fast local feature-image boundary. |
...
| TEST-03 Local feature-gate integrity | High / testing | XR-TEST-003, MD-27 | Audit PR and #2487 | Forced build/stop-failure propagation, repo-owned CLI resolution, successful post-fix feature smoke, and ownership-safe isolated teardown |
