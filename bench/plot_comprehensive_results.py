...
def load_and_clean_data(csv_path):
    ...
    try:
        ...
        df_latest = df.sort_values('Timestamp').groupby(['Dataset', 'Mode', 'Model']).tail(1)
...
def create_accuracy_plot(df, output_dir):
    ...
    for i, mode in enumerate(available_modes):
        ...
        plt.bar(x + i * width, accuracies, width, 
               label=display_name, color=colors.get(mode, f'C{i}'), alpha=0.8)
        
        # Add value labels on bars
        for j, acc in enumerate(accuracies):
            if acc > 0:
                plt.text(x[j] + i * width, acc + 0.01, f'{acc:.3f}', 
                        ha='center', va='bottom', fontsize=9, fontweight='bold')
    
    plt.xlabel('Dataset', fontsize=12, fontweight='bold')
    plt.ylabel('Accuracy', fontsize=12, fontweight='bold')
    plt.title('Accuracy Comparison: Router vs vLLM Direct\n(No Reasoning vs All Reasoning)', 
...
def create_token_usage_plot(df, output_dir):
    ...
    for i, mode in enumerate(available_modes):
        ...
        plt.bar(x + i * width, token_usage, width, 
               label=display_name, color=colors.get(mode, f'C{i}'), alpha=0.8)
        
        # Add value labels on bars
        for j, tokens in enumerate(token_usage):
            if tokens > 0:
                plt.text(x[j] + i * width, tokens + max(token_usage) * 0.01, 
                        f'{tokens:.0f}', ha='center', va='bottom', fontsize=9, fontweight='bold')
    
    plt.xlabel('Dataset', fontsize=12, fontweight='bold')
    plt.ylabel('Average Total Tokens', fontsize=12, fontweight='bold')
    plt.title('Token Usage Comparison: Router vs vLLM Direct\n(No Reasoning vs All Reasoning)', 
              fontsize=14, fontweight='bold', pad=20)
    plt.xticks(x + width, [d.upper() for d in datasets], rotation=45, ha='right')
    plt.legend(loc='upper left', frameon=True, fancybox=True, shadow=True)
    plt.grid(True, alpha=0.3, axis='y')
...
def create_efficiency_plot(df, output_dir):
    ...
    for mode in available_modes:
        ...
        plt.scatter(mode_data['Avg_Total_Tokens'], mode_data['Accuracy'], 
                   c=colors.get(mode, 'gray'), marker=markers.get(mode, 'o'),
                   s=100, alpha=0.7, label=display_name, edgecolors='black', linewidth=1)
        
        # Add dataset labels
        for _, row in mode_data.iterrows():
            plt.annotate(row['Dataset'].upper(), 
    ...
    plt.ylabel('Accuracy', fontsize=12, fontweight='bold')
    plt.title('Efficiency Analysis: Accuracy vs Token Usage\n(Higher accuracy with lower tokens is better)', 
              fontsize=14, fontweight='bold', pad=20)
...
def create_summary_table(df, output_dir):
    ...
    dataset_name = df['Dataset'].iloc[0] if unique_datasets == 1 else "Multiple Datasets"
    ...
    if unique_datasets == 1:
        ...
        print(f"{'Mode':<20} {'Accuracy':<10} {'Tokens':<10} {'Latency(ms)':<12} {'Samples':<8}")
        print("-" * 65)
        
        for mode in sorted(df['Mode'].unique()):
            mode_data = df[df['Mode'] == mode].iloc[0]
            print(f"{mode:<20} {mode_data['Accuracy']:<10.3f} {mode_data['Avg_Total_Tokens']:<10.1f} {mode_data['Avg_Latency_ms']:<12.1f} {mode_data['Sample_Count']:<8}")
        
        summary = df.groupby('Mode')[['Accuracy', 'Avg_Total_Tokens', 'Avg_Latency_ms']].first()
    else:
...
def main():
    ...
    parser.add_argument('--csv', type=str, default='research_results_master.csv',
                       help='Path to research results CSV file')
    parser.add_argument('--output-dir', type=str, default='research_plots',
