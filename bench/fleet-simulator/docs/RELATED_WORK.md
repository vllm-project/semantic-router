...
`inference-fleet-sim` sits at the intersection of several active research threads.
...
**Key differences from `inference-fleet-sim`:**

| | Mélange | inference-fleet-sim |
|---|---|---|
...
GPU SKU to rent. Mélange selects the type; `inference-fleet-sim` then tells you how
...
up/down through a 24-hour demand cycle. Use `inference-fleet-sim` first to size the
...
| | AIConfigurator | inference-fleet-sim |
|---|---|---|
...
**Relationship:** `inference-fleet-sim` embeds AIConfigurator's empirical constants
...
| | TokenScale | inference-fleet-sim |
|---|---|---|
...
**When to use together:** `inference-fleet-sim` gives the steady-state fleet size
...
Use the interactive wizard at [`tool-selector.html`](tool-selector.html) or follow the flowchart below.
