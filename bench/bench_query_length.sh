...
echo "Configuration: HOST=$HOST, PORT=$PORT, MODEL=$MODEL, RANGE=$START_LENGTH-$END_LENGTH, STEP=$STEP, WAIT=${SLEEP_TIME}s"
echo "==========================================="
...
for length in $(seq $START_LENGTH $STEP $END_LENGTH); do
    ...
    if [ "$http_code" != "200" ]; then
        echo "Warning: Request failed, HTTP status code: $http_code"
    fi
