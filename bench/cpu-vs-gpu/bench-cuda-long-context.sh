...
#   - A tiny built-in OpenAI stub upstream is started so the routed request
#     completes with a 200.
#   - Percentiles are computed in python (portable; host awk may lack asort).
#   - The run is gated: every benchmark request must return 200 and every
#     signal must record at least one sample per successful request, otherwise
#     the script aborts instead of writing a report.
#
# Prerequisites:
...
#     (default tag vllm-sr-cuda:local; see website/docs/installation/nvidia-cuda.md)
#   - envoyproxy/envoy:v1.33-latest
#   - Models under $MODELS_DIR (see README.md "Run the NVIDIA CUDA
#     benchmarks"): the three mmbert32k-*-merged classifiers plus
...
# mode sz count label -> sends `count` requests and sets SEND_OK / SEND_FAILED.
# When label != "warmup" it also records client wall-clock latency + HTTP code
# to an e2e CSV for that size. Only HTTP 200 counts as a usable sample: a
# transport failure ("000") or any non-2xx means the request never completed
# classification, so its wall-clock time is not a latency measurement.
send_requests() {
    ...
    for i in $(seq 1 "$count"); do
        ...
        if [ -n "$csv" ]; then
            echo "${i},${latency_ms},${http_code}" >> "$csv"
        fi
    done
}
...
# lack asort). Only HTTP 200 rows contribute; a failed request's wall clock is
...
# before after expected -> fails when any classifier recorded fewer than
# `expected` new samples, i.e. the traffic never reached signal extraction.
verify_signal_samples() {
...
run_phase() {
    ...
    for sz in "${SIZES[@]}"; do
        ...
        if [ "$SEND_OK" -eq 0 ]; then
            log "ERROR: ${mode} ${sz}tok warmup got no HTTP 200 (${SEND_FAILED} failed); aborting"
            stop_containers "$mode"
            return 1
        fi
    done
    for sz in "${SIZES[@]}"; do
        local before="$RESULTS_DIR/m-${mode}-before-${sz}-${TIMESTAMP}.txt"
        local after="$RESULTS_DIR/m-${mode}-after-${sz}-${TIMESTAMP}.txt"
        scrape_metrics "$before"
        send_requests "$mode" "$sz" "$REQUESTS_PER_SIZE" bench
        scrape_metrics "$after"
        if [ "$SEND_FAILED" -ne 0 ]; then
            log "ERROR: ${mode} ${sz}tok had ${SEND_FAILED}/${REQUESTS_PER_SIZE} non-200 responses; aborting"
            stop_containers "$mode"
            return 1
        fi
        if ! verify_signal_samples "$before" "$after" "$SEND_OK"; then
            log "ERROR: ${mode} ${sz}tok completed ${SEND_OK} requests but signal extraction did not record them; aborting"
            stop_containers "$mode"
            return 1
        fi
        log "  ${mode} ${sz}tok done (${SEND_OK} ok)"
    done
    stop_containers "$mode"
}

# Both phases must have produced usable data for every size before anything is
# published; a partially empty report reads like a slow run rather than a
# broken one.
verify_results() {
    local sz mode ef n
    for sz in "${SIZES[@]}"; do
        for mode in cpu gpu; do
            ef="$RESULTS_DIR/e2e-${mode}-${sz}-${TIMESTAMP}.csv"
            if [ ! -f "$ef" ]; then
                log "ERROR: missing e2e samples for ${mode} ${sz}tok"
                return 1
            fi
            read -r n _ _ _ _ _ <<< "$(compute_e2e_stats "$ef")"
            if [ "$n" = 0 ]; then
                log "ERROR: no successful responses for ${mode} ${sz}tok"
                return 1
            fi
            if ! verify_signal_samples \
                "$RESULTS_DIR/m-${mode}-before-${sz}-${TIMESTAMP}.txt" \
                "$RESULTS_DIR/m-${mode}-after-${sz}-${TIMESTAMP}.txt" 1 >/dev/null; then
                log "ERROR: no signal-extraction samples for ${mode} ${sz}tok"
                return 1
            fi
        done
    done
    ...
    local report="$RESULTS_DIR/report-cuda-${TIMESTAMP}.md"
    {
        ...
        for sz in "${SIZES[@]}"; do
            for mode in cpu gpu; do
                ...
                echo "| ~${sz} | ${mode^^} | $n | $avg | $p50 | $p95 | $mn | $mx |"
            done
        done
        echo ""
        for signal in domain jailbreak pii; do
            ...
            for sz in "${SIZES[@]}"; do
                for mode in cpu gpu; do
                    ...
                    echo "| ~${sz} | ${mode^^} | $n | $avg | $p50 | $p95 | $p99 |"
                done
            done
            echo ""
        done
        ...
        for sz in "${SIZES[@]}"; do
            for signal in domain jailbreak pii; do
                ...
                read -r _ cavg _ _ _ <<< "$(histogram_stats "$cb" "$ca" "$signal")"
                read -r _ gavg _ _ _ <<< "$(histogram_stats "$gb" "$ga" "$signal")"
                local speedup
