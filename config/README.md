...
- `global.model_catalog.system` is the stable lifecycle catalog for router-owned embedding, classifier, and detector models; `global.model_catalog.embeddings.semantic.model_refs` and `global.model_catalog.modules.*.model_ref` select those roles, while explicit `*_model_path`, `model_path`, or `model_id` fields remain direct overrides
