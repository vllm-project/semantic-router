...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `type` _string_ | Type is the plugin type. `response_cache` is canonical; `semantic-cache`, `semantic_cache`, and `response-cache` are deprecated aliases. |  | Enum: [fast_response hallucination header_mutation image_gen memory rag request_params response_jailbreak router_replay response_cache response-cache semantic_cache semantic-cache system_prompt tools] <br />Required: \{\} <br /> |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `name` _string_ | Name is the signal name that can be referenced in decision rules<br />e.g., "needs_fact_check" or "no_fact_check_needed" |  |  |
| `description` _string_ | Description provides human-readable explanation of when this signal is triggered |  |  |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `operator` _string_ | Operator defines the logical operator for combining conditions (AND/OR/NOT)<br />NOT uses NOR semantics: matches only when none of the conditions match. |  | Enum: [AND OR NOT] <br />Required: \{\} <br /> |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `type` _string_ | Type defines the type of signal (keyword/embedding/domain/fact_check/context/structure) |  | Enum: [keyword embedding domain fact_check context structure] <br />Required: \{\} <br /> |
| `name` _string_ | Name is the name of the signal to reference<br />For fact_check type, use "needs_fact_check" to match queries that need fact verification |  | MaxLength: 100 <br />MinLength: 1 <br />Required: \{\} <br /> |
...
| `contextRules` _[ContextRule](#contextrule) array_ | FactCheckRules defines fact-check rules for signal classification<br />Similar to KeywordRules and EmbeddingRules, but based on ML model classification<br />Each rule has a name that can be referenced in decision conditions |  |  |
| `structure` _[StructureSignal](#structuresignal) array_ | Structure defines request-shape routing signals. |  | MaxItems: 100 <br /> |
| `factCheckRules` _[FactCheckRule](#factcheckrule) array_ |  |  |  |
...
| --- | --- | --- | --- |
| `type` _string_ |  |  |  |
| `source` _[StructureSource](#structuresource)_ |  |  |  |
...
| `feature` _[StructureFeature](#structurefeature)_ |  |  |  |
| `predicate` _[NumericPredicate](#numericpredicate)_ |  |  |  |
...
| `sequences` _string array array_ |  |  |  |
