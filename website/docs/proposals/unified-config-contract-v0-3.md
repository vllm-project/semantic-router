...
- `routing.decisions[].algorithm.budget` optionally declares a per-request token/cost/wall-time ceiling for Looper execution (`confidence`, `ratings`, `remom`, `fusion`, `workflows`), carried alongside routing evidence (decision confidence and matched signal identifiers, never prompt content) into the Looper request. It is a request-scoped resource policy, distinct from the separate, non-configurable hard upstream-call cap; config validation rejects `algorithm.budget` on algorithm types that do not execute through Looper.
