...
- `provider_prompt_cache` and `context_compression` are post-decision plugins: the first injects provider-native cache markers, while the second compresses large tool outputs before provider dispatch without changing signal evaluation input.
