...
- each `providers.models[].backend_refs[]` item carries its own transport and auth fields such as `endpoint`, `base_url`, `protocol`, `auth_header`, `auth_prefix`, `api_key`, `api_key_env`, and `forward_authorization_header`
- `providers.models[].backend_refs[].forward_authorization_header` (default `false`) is an opt-in that forwards the caller's inbound `Authorization` header verbatim to that backend instead of injecting a static service key. When enabled, `api_key`/`api_key_env` are ignored for the backend and requests without an inbound `Authorization` header are rejected with `401`. This preserves per-caller identity (for example, per-user virtual keys) through the router and looper re-dispatch to gateways such as LiteLLM.
