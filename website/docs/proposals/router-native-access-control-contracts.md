...
Execution/pricing semantics live in [Model runtime](./router-native-access-control-model-runtime); applied counters and reconciliation live in [quota runtime](./router-native-access-control-quota-runtime).
...
| `routing_recipes`, `routing_recipe_revisions`, `routing_recipe_decisions` | Draft/published Recipe documents and stable decision UIDs. |
| `routing_entrypoints`, `routing_entrypoint_rules`, `routing_entrypoint_assignments` | Callable aliases, trusted matchers, Recipe references, and ordered Model assignments. |
...
| Subject | `(namespace_id, id)` primary key and immutable kind `user|team|api_key`; lifecycle exists only on the concrete resource. |
| User | Subject FK of kind User, normalized unique email, display name, status, timestamps. |
| Team | Subject FK of kind Team, unique name, `draft|active|disabled`, timestamps. Activation materializes an AccessPolicy binding and RateLimit allocation binding. |
...
| Model/backend | Namespace, immutable Model UID, mutable unique name/aliases, capability metadata, status/current immutable revision; the revision embeds execution and four-rate pricing values, while ordered endpoints optionally reference ProviderCredential UIDs when their provider authentication schema requires one. |
| Recipe/revision | Namespace, immutable Recipe UID, mutable name, lifecycle/revision; immutable validated document revision with signals, projections, decisions, algorithms, plugins, and stable decision UIDs. |
| Entrypoint/rule | Namespace, immutable Entrypoint/rule UIDs, mutable names/aliases, lifecycle/revision, trusted matchers, exact Recipe revision, and complete decision-to-Model UID assignments. |
...
| Rate binding | Composite policy and typed-subject FKs, `allocation|hard_cap`, quota partition, status, revision. |
...
| Role binding | Principal and role FKs, discriminated `cluster|namespace|team|user|resource` scope with namespace, typed resource kind/ID, separate delegation-ceiling permission set, status, revision. |
...
| mTLS identity mapping | Global immutable ID, exact normalized `spiffe_id|san_uri|san_dns|subject_dn_sha256` matcher, ManagementPrincipal FK, workload-assurance class and assured-at time, status, revision; uniqueness prevents ambiguous matches. |
| Service account/credential | ManagementPrincipal subtype with reserved issuer, immutable `cluster|namespace` owner scope, public credential ID, HMAC/pepper, workload-assurance class and assured-at time, lifecycle, expiry, and no inference authority. Namespace-owned principals cannot bind elsewhere. |
...
| Provider credential/version | Namespace, immutable provider ID and normalized scheme/host/port/base-path origin, encrypted secret/nonce/KEK version, lifecycle, revision; no inference-key HMAC or reveal. |
...
| Unknown usage fence | Stable fence ID, admission ID, namespace, reason, evidence, `open|reconciling|resolved` lifecycle, reconciliation strategy/actor/revision, and one or more affected binding FKs. |
...
The projector compiles each calendar rule's tzdb version and UTC boundary schedule at
least 18 months ahead. Valkey `TIME` selects the interval from that schedule; Functions
never interpret an IANA zone. The projector refreshes before a 30-day safety horizon,
and access readiness fails closed if the active schedule cannot cover server time.
...
Retention may compact old settlement rows only after the usage and reconciliation
streams have trimmed their items, no consumer pending entry can replay them, and the
maximum recovery/replay horizon has expired. The number of hash partitions and
retention bound are operator-visible capacity settings.
...
- Unique namespace-scoped Model, Recipe, and Entrypoint names/aliases plus immutable
  ...
  revision and a Model in the same namespace.
...
hold renewable readiness leases, fetch and validate the staged digest, and
acknowledge the publication. Activation atomically changes
...
YAML and built-in catalogs are import manifests in managed mode. Import calls the
same Management resources and does not mount a second live configuration source.
...
and is deleted at TTL. After that TTL, recovery uses audited reveal when enabled; a
...
The credential's provider and normalized origin are immutable. A fixed public
provider takes its origin from the catalog; private providers bind the validated
origin at credential creation. Model, discovery, and probe paths must match that
provider/origin exactly; connection overrides may change only schema-approved path
fields. Changing origin creates a new credential. Secret-bearing calls never follow
