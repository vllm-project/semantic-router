...
This configuration is routing evidence, separate from the execution ledger in
[sr-bench 1.0](sr-bench). sr-bench freezes its own target, dataset and price
identities. Inspect a complete live report before explicitly publishing an
operator evaluation record; a catalog value is not a fresh measurement.
