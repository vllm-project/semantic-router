...
| `x-vsr-learning-actions: session_aware=switch` | Learning allowed a model change after stay-vs-switch scoring. | `switch/run` in compact route columns, with `reason switch allowed` and `model switched` in detail text. |
| `x-vsr-learning-actions: session_aware=hard_lock` with `x-vsr-learning-reasons: session_aware=hard_lock=tool_loop` | A tool loop is active, so the model is pinned for tool-call stability. | `hard lock/run` in compact route columns, with `reason tool loop` and `tool-loop pinned` in detail text. |
