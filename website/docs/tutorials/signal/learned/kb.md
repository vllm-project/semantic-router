...
- Lets projections consume continuous KB metrics without turning signals into a scripting surface.
...
- `routing.projections` can consume KB metrics such as `best_score`, `best_matched_score`, or configured group margins
...
- one startup-loaded KB result should feed several routes
...
Keep KB names stable because KB signals bind to those names directly.
...
- `label + best`: match only when the label is the KB's best label
- `label + threshold`: match when the label score clears its effective threshold
- `group + best`: match only when the group is the KB's best group
- `group + threshold`: match when any member label clears its threshold

## Projection Metrics

KB signals are boolean routing inputs. Numeric outputs stay in `routing.projections`.
...
Named KB metrics are declared under `global.model_catalog.kbs[].metrics[]`. Built-in metrics `best_score` and `best_matched_score` are always available.
