...
Configure in `examples/runtime/semantic-cache/milvus.yaml`:

```yaml
# examples/runtime/semantic-cache/milvus.yaml
connection:
  host: "localhost"
  port: 19530
  auth:
...
- Set `backend_config_path: "examples/runtime/semantic-cache/milvus.yaml"` in `config/config.yaml`
