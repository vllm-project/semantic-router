...
RouterDC (Dual-Contrastive) uses semantic embeddings to match user queries with model descriptions. It selects the model whose capabilities best align with the query's intent.

Based on the [RouterDC paper](https://arxiv.org/abs/2409.19886) (NeurIPS 2024).
...
### Dual-Contrastive Training

The embedding model is trained with contrastive loss:

```text
L = -log( exp(sim(q, m⁺) / τ) / Σ exp(sim(q, m) / τ) )
```

Where:

- `m⁺` = Correct model for query `q`
