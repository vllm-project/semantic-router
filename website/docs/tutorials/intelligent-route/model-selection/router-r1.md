...
Router-R1 uses an LLM as the router itself, performing multi-round "think" and "route" actions to make intelligent routing decisions.

Based on the [LLM-as-Router concept](https://arxiv.org/abs/2404.14618) - Using language models for intelligent query routing.
...
The Router LLM uses a structured output format:
...
| Action | Description |
|--------|-------------|
| `<think>...</think>` | Reasoning step (can repeat) |
...
Router-R1 selects based on LLM reasoning:

```text
m* = argmax_m P(m | query, model_descriptions)

where P is the Router LLM's output distribution
```

### Iterative Refinement

With max_iterations = k:

```text
for i in 1..k:
    action = RouterLLM(query, context, history)
