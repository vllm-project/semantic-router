...
Requests carrying Dynamo `nvext` or Dynamo routing headers skip shadow dispatch, even when the shadow target is also Dynamo. This includes routing headers added by the primary provider profile or decision. The primary request continues normally; the router records a dropped shadow with reason `dynamo_request_state`. Ordinary requests without Dynamo-specific state remain eligible for shadow dispatch to Dynamo backends. Top-level `cache_salt` is a regular request parameter and does not by itself trigger this Dynamo-specific skip.
