...
Requests carrying Dynamo `nvext`, top-level `cache_salt`, or Dynamo routing headers skip shadow dispatch, even when the shadow target is also Dynamo. This includes routing headers added by the primary provider profile or decision. The primary request continues normally; the router records a dropped shadow with reason `dynamo_request_state`. Ordinary requests without Dynamo-specific state remain eligible for shadow dispatch to Dynamo backends.
