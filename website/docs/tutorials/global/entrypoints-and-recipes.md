...
- Entrypoint names appear in `/v1/models` next to the auto aliases, using the recipe description as the listing description. Each item also declares stable `routing` metadata: `resolution` (`virtual` or `passthrough`), `selectable`, and optional `default_route`. Clients use these fields instead of inferring behavior from model IDs, owners, descriptions, or internal execution modes.
