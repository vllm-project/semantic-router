...
`vllm-sr decision serve` starts a standalone HTTP service for one Decision model. Your
...
```mermaid
%%{init: {"flowchart": {"rankSpacing": 18, "nodeSpacing": 12, "curve": "linear"}}}%%
flowchart TB
    API["HTTP API<br/>validate · admit"] --> Batch["Decision scheduler<br/>state × question · bounded batch"]
    Batch --> Model["Resident model on CPU or ROCm<br/>candidate probabilities"]
    Model --> Answers["Typed answers<br/>Noul · Choice · Score"]
```

## Follow one request

1. **Send the state and questions.** Use `POST /v1/systemone` for one state or
   `POST /v1/systemone/batches` for several states that share the same questions.
...
Each `decision serve` instance has **one model, one revision, and one port**. To serve Kai
