...
| --- | --- |
| `--backend auto` (default) | On Linux, use ROCm when available; otherwise use CPU, including on a CUDA-only host. Only sub-1B models have a CPU executor in this release. Detection does not guarantee that a particular device or model is supported. Apple Silicon is not supported yet. |
...
make a ROCm kernel support a larger physical batch. Eos on ROCm currently
accepts at most eight rows per model forward; the CLI rejects a larger value
before preparing model files. See
...
Sol and Nox automatically reuse the model backbone for repeated, short
eight-row batches. Sol checks a new eligible shape on first use, so that
request can take longer. Nox prepares its common short shape before the
service becomes ready. Longer inputs and other batch sizes use ordinary
