...
| --- | --- |
| `--backend auto` (default) | On Linux, detect one visible GPU type, or select CPU when none is visible. Detection does not guarantee that the selected model and image can run there. If both ROCm and CUDA devices are visible, choose a backend explicitly. Apple Silicon is not supported yet. |
