...
| Reasoning summary requests (`reasoning.summary`) | Not forwarded; reported as `dropped` | Supported; reported as `dropped` when the Router sends reasoning controls in `chat_template_kwargs` | Not forwarded; reported as `dropped` |
