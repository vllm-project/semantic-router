...
- `decision.algorithm.budget` declares an optional per-request token/cost/wall-time ceiling for Looper execution (`confidence`, `ratings`, `remom`, `fusion`, `workflows` only; config validation rejects it on other algorithm types). Every field defaults to unlimited; exhausting any one stops further escalation deterministically and returns the best response obtained so far. It never raises or overrides the separate, non-configurable hard upstream-call cap.
