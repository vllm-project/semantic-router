...
- `global` owns router-wide runtime overrides.
  ...
  - `global.router.learning.adaptations` enables cross-request Router Learning after the base decision algorithm. `session_aware` protects agentic continuity; `bandit` provides conservative day-0 feedback/cost-aware scoring; `elo` provides day-0 pairwise rating states; `personalization` provides day-0 user preference states. `decision.algorithm.type=session_aware|elo|rl_driven|gmtrouter` is removed; decisions can use normal base algorithms or omit `algorithm`. Per-decision `adaptations.<name>` is strictly validated and should be used only for `mode: apply|observe|bypass` plus sparse overrides.
