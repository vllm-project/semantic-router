...
| `history-reset` — route plugin | `history_reset` is a route-local context plugin that removes complete eligible prior turns from the provider-bound request after an accepted topic change. | [`config/fragments/plugin/history-reset/`](https://github.com/vllm-project/semantic-router/tree/main/config/fragments/plugin/history-reset/) | [Guide](../tutorials/plugin/history-reset) |
