...
- `global` owns router-wide runtime overrides.
  ...
  - `decision.algorithm.type=session_aware` enables agentic stay-vs-switch routing with router-owned session memory, tool-loop hard locks, idle-time re-selection, prefix-cache cost, confidence-gated remaining-turn priors, and replayed policy traces.
