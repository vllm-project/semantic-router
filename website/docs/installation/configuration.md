...
- `global` owns router-wide runtime overrides.
  ...
  - `decision.algorithm.type=session_aware` enables agentic stay-vs-switch routing with router-owned session memory, tool-loop hard locks, provider-only context protection, idle-time re-selection, prefix-cache cost, and replayed policy traces.
