...
| `complexity` — learned signal | `complexity` estimates whether a request is `easy`, `medium`, or `hard` by comparing it with configured example sets (`method: embedding`, default) or with a trained 3-class classifier bound under `global.model_catalog.modules.complexity.classifier` (`method: model`). | [`config/fragments/signal/complexity/`](https://github.com/vllm-project/semantic-router/tree/main/config/fragments/signal/complexity/) | [Guide](../tutorials/signal/learned/complexity) |
