...
| `decision-diagnostics` — route plugin | `decision_diagnostics` is an opt-in route-local plugin that emits a bounded, content-free summary of the selected routing decision as ExtProc dynamic metadata. | [`config/fragments/plugin/decision-diagnostics/`](https://github.com/vllm-project/semantic-router/tree/main/config/fragments/plugin/decision-diagnostics/) | [Guide](../tutorials/plugin/decision-diagnostics) |
