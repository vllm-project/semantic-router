...
| `context-dedup` — route plugin | `context_dedup` is a route-local context plugin that removes the later copy of prior turns a request carries twice in a row, so repeated history stops costing input tokens without changing what the conversation means. | [`config/fragments/plugin/context-dedup/`](https://github.com/vllm-project/semantic-router/tree/main/config/fragments/plugin/context-dedup/) | [Guide](../tutorials/plugin/context-dedup) |
