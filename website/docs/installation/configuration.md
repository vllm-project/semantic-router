...
| `prompt-cache` — route plugin | `prompt_cache` is a route-local plugin that inserts Anthropic prompt-cache breakpoints on a matched route. | [`config/fragments/plugin/prompt-cache/`](https://github.com/vllm-project/semantic-router/tree/main/config/fragments/plugin/prompt-cache/) | [Guide](../tutorials/plugin/prompt-cache) |
...
- **Deployment scoping**: In standalone Envoy deployments generated by Semantic Router templates, `stream_idle_timeout` is rendered directly into the Envoy route `idleTimeout`, and `connect_timeout` is rendered directly into the upstream cluster `connect_timeout`. In external Gateway deployments like Envoy Gateway or Envoy AI Gateway, `request_timeout` is dynamically enforced per-request via `x-envoy-upstream-rq-timeout-ms`. However, Envoy does not consume dynamic per-request HTTP headers for stream idle or connect timeouts. Per-model stream idle timeouts on that path should be configured via Gateway route policies like `EnvoyPatchPolicy` or `ClientTrafficPolicy`, while backend connect timeouts should be managed at the cluster or transport layer via policies like `BackendTrafficPolicy`.
