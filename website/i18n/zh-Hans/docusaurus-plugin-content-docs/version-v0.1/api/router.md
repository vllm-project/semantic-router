...
> 注意：除了 ExtProc 路径外，该项目还在 8080 端口启动了一个轻量级的 HTTP Router Apiserver，用于健康检查、模型信息、分类工具和 Router 配置管理。OpenAI 兼容的 `/v1/models` 端点由该 HTTP API (8080) 提供，并可以通过路由规则在 Envoy (8801) 上有选择地公开。
...
- 8080 (HTTP, Router Apiserver)
  ...
  - `GET /health` → Router Apiserver 健康状况
  - `GET /info/models` → 已加载的分类器模型 + 系统信息
  - `GET /info/classifier` → 分类器配置详情
  - `POST /api/v1/classify/intent|pii|security|batch` → 直接分类工具
  - `GET|PATCH|PUT /config/router`、`GET /config/router/versions`、`POST /config/router/rollback` → Router 配置 API
...
- **[Router Apiserver API](apiserver.md)**：HTTP 健康、分类与 Router 配置端点
