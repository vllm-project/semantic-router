...
1. **基于关键字的路由** - 确定性的、快速的、用于精确术语匹配的 Boolean 逻辑
...
- **策略驱动融合**：允许用户使用 Boolean 表达式、阈值和加权规则组合信号
...
- 复杂匹配缺乏 Boolean 逻辑（例如，“Kubernetes AND 安全” vs “Kubernetes OR Docker”）
...
- 扫描传入查询中是否存在这些关键字
- 支持 Boolean 运算符（AND/OR）以组合多个关键字
- 可以是大小写敏感或大小写不敏感的
- 返回匹配的规则及其关联的候选模型
...
- **Boolean 表达式**：使用 AND、OR、NOT 组合多个信号条件
- **灵活比较**：支持数字阈值的 `==`, `!=`, `>`, `<`, `>=`, `<=`

**表达式能力：**

- 引用关键字匹配：`keyword.kubernetes-infrastructure.matched`
- 检查相似度分数：`similarity.multi-step-reasoning.score > 0.75`
...
- **运算符：** Boolean 逻辑（OR = 任何关键字，AND = 所有关键字）
- **大小写敏感性：** 是否进行大小写敏感匹配
- **候选模型：** 匹配时路由到的模型列表
