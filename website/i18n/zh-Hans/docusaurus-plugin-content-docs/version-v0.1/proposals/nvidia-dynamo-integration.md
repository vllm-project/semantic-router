...
- **Semantic Router** 的智能请求分类（14 个领域类别）、领域感知的 system_prompt、融合路由（BERT 分类 + 关键词匹配 + 相似度搜索）、安全过滤、基于 Milvus 的 semantic-cache
...
- **增强 LLM 质量**：利用领域感知的 system_prompt 改善 Chain-of-Thought (CoT) 推理、token 效率和 MoE 专家匹配
...
| **领域感知 system_prompt** | 自动注入类别特定的 system_prompt 用于 prompt 工程 | 准确性：LLM CoT 质量、token 效率、MoE 专家匹配 |
...
- **智能 prompt 工程**：自动注入领域特定的 system_prompt，以优化 LLM 行为和输出质量
...
| **Prompt 工程** | ✅ 领域感知 system_prompt | ❌ 无 prompt 优化 | ✅ 优化的 CoT 和 MoE 匹配 |
