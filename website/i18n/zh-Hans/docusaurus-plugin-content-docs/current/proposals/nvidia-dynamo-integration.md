...
- **Semantic Router** 的智能请求分类（14 个 domain 类别）、domain 感知的 system_prompt、融合路由（BERT 分类 + keyword 匹配 + 相似度搜索）、安全过滤、基于 Milvus 的 semantic-cache
- **Dynamo** 的 disaggregated serving、KV-aware 路由和多层内存管理
...
- **增强 LLM 质量**：利用 domain 感知的 system_prompt 改善 Chain-of-Thought reasoning、token 效率和 MoE 专家匹配
- **自适应路由智能**：通过融合路由实现：根据查询复杂度，从快速路径 (keyword) 到深度分析 (BERT)，在不牺牲准确性的情况下最大化效率
- **多信号决策**：结合 BERT 分类、keyword 匹配和相似度搜索，实现稳健且准确的路由
...
- **缓存感知**：利用现有的 KV cache 来降低 prefill 成本
- **负载均衡**：在工作节点之间分配解码工作负载
- **性能导向**：通过智能调度最小化 TTFT 和 ITL

### 2.2 Semantic Router 能力（系统智能层）

vLLM Semantic Router 提供了在请求理解层运行的**系统级智能**，通过在 **14 个 domain 类别**中进行智能决策，实现**准确性**与**效率**之间的最佳平衡：

| 能力 | 描述 | 智能焦点 |
|------------|-------------|---------------------|
| **意图分类** | 基于 BERT 的分类（14 个类别：数学、代码、商务、法律等） | 准确性：精确的 domain 理解 |
| **模型选择** | 为每个类别路由到表现最佳的模型 | 准确性：特定任务的质量优化 |
| **Domain 感知 system_prompt** | 自动注入类别特定的 system_prompt 用于 prompt 工程 | 准确性：LLM CoT 质量、token 效率、MoE 专家匹配 |
| **融合路由** | 多信号路由 (keyword + 相似度 + BERT) | 效率：基于查询复杂度的自适应延迟 |
...
| **reasoning 控制** | 为复杂查询自动启用 reasoning 模式 | 准确性：质量感知的模式选择 |

**系统智能特征：**

- **整体智能**：跨 14 个 domain 类别理解查询意图、复杂度和安全影响
- **准确性-效率平衡**：根据查询复杂度动态选择路由策略 (keyword/相似度/BERT)，在最小化延迟的同时最大化准确性
- **质量优化**：根据特定任务的准确性要求选择模型和 prompt
- **智能 prompt 工程**：自动注入 domain 特定的 system_prompt，以优化 LLM 行为和输出质量
- **主动安全**：在到达 inference 层之前拦截恶意或违反隐私的请求
- **成本智能**：对于简单查询避免使用昂贵的模型，同时确保复杂任务的质量
- **自适应路由**：多信号融合路由根据查询特征进行调整，以实现最佳的准确性-效率权衡

#### 2.2.1 具有 system_prompt 的 14 个领域分类

Semantic Router 将查询分为 **14 个专业类别**：数学、计算机科学、物理、化学、生物、工程、经济、商业、法律、心理学、哲学、历史、健康和其他。每个类别都有一个根据查询分类自动注入的优化 system_prompt。

**system_prompt 收益：**

1. **改进 Chain-of-Thought (CoT)**：domain 特定的 prompt 引导 LLM 使用适当的 reasoning 模式
...
2. **token 效率**：优化的 prompt 在保持质量的同时减少了不必要的冗余
   ...
   - 对于需要特定方法论的复杂 domain（数学、物理）使用详细的 prompt

3. **MoE 专家匹配**：精心编写的 system_prompt 改善了 Mixture-of-Experts 模型中的专家选择
   - domain 特定的术语激活相关专家
...
- **路由逻辑**：
  1. 首先检查 keyword 规则（最快）
  2. 如果没有 keyword 匹配，检查相似度规则
...
- **准确性-效率平衡**：根据查询复杂度动态选择路由策略——确定性模式的快速路径 (keyword) 可实现最小延迟，而复杂查询的深度分析 (BERT) 可确保最大准确性
...
| **Prompt 工程** | ✅ Domain 感知 system_prompt | ❌ 无 prompt 优化 | ✅ 优化的 CoT 和 MoE 匹配 |
| **融合路由** | ✅ BERT + keyword + 相似度融合 | ❌ 仅 KV-aware | ✅ 多信号智能路由 |
...
- **类别**：14 个 domain（数学、计算机科学、物理、化学、生物、工程、经济、商业、法律、心理学、哲学、历史、健康、其他）
- **输出**：类别标签 + 置信度评分
- **阈值**：0.6（可配置）
- **训练数据**：包含 domain 特定示例的 MMLU-Pro 数据集
...
- 触发 domain 特定的 system_prompt 注入
- 控制 reasoning 模式激活
...
3. **执行层 (vLLM/SGLang/TRT-LLM)**
   - 模型特定的工作节点池
   - disaggregated prefill/decode 工作节点
   - 后端无关的执行
...
│                                                                              │
│ 步骤 4：融合路由（多信号分类）                                               │
│   - 信号 1：keyword 匹配（快速路径）                                         │
│   - 信号 2：相似度搜索（语义概念）                                           │
│   - 信号 3：BERT 分类（深度理解）                                            │
│   - 基于熵的 reasoning 决策                                                 │
│   - 类别：数学、代码、reasoning、创意等                                      │
│   - 延迟：自适应（keyword：极低，相似度：低，BERT：中等）                     │
│                                                                              │
│ 步骤 5：模型选择                                                             │
│   - 查询类别 → 模型得分映射                                                  │
│   - 为该类别选择表现最佳的模型                                               │
...
│     - system_prompt 已由 VSR 注入到 messages 中                      │
│     - 工作节点对 VSR 的参与并无感知                                  │
│                                                                       │
│ 6.2 Inference 执行                                                   │
│     - 模型：DeepSeek-V3                                              │
│     - Messages：[system_prompt + 用户查询]                           │
...
2. **system_prompt 注入**
   - Semantic Router 将 system_prompt 注入到 messages 数组中
...
4. **可观测性（可选头）**
   ...
   - `x-vsr-injected-system-prompt`：是否注入了 system_prompt（如 "true" 或 "false"）
...
- ✅ system_prompt 正确注入到 messages 中
- ✅ Dynamo 透明运行，无需修改
- ✅ 延迟开销 < 10ms
- ✅ 不对现有部署产生破坏性更改
...
| **融合路由** | 结合 BERT 分类、keyword 匹配和相似度搜索的多信号路由 |
...
### 10.2 system_prompt 示例

**关键类别的 Domain 感知 system_prompt：**

集成利用了 **14 个专业的 system_prompt**，这些 prompt 根据查询分类自动注入。以下是代表性示例：
