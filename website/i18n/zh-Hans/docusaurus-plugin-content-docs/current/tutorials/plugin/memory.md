...
Candle `bert` 模型没有内容描述符，因此 Router 改用编码器版本为其记忆分区。每当 Candle BERT 的向量发生变化，这个版本就会随之更新，例如填充 token 不再计入平均值时。跨越这类变化升级后，BERT 记忆会使用新的集合或索引以及新的 Redis 热缓存。升级前存储的条目仍保留在旧集合中，但不会再被召回，记忆会随新的对话重新积累。由其他运行时提供的 BERT 保留已有存储。
