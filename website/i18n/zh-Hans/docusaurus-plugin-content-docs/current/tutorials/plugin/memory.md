...
此前没有身份标记的集合会保留，但不会自动接管：相同向量维度不代表两个模型的嵌入兼容。管理 API 没有记忆导入或批量导出端点，因此新模型的集合从空开始，并随新流量重新积累记忆。启动或迁移不会删除旧集合。这项自动身份绑定目前覆盖本地 `mmbert`；除下文所述的 Candle `bert` 外，其他嵌入提供方保留已有行为。

Candle `bert` 模型没有内容描述符，因此 Router 改用编码器版本为其记忆分区。每当 Candle BERT 的向量发生变化，这个版本就会随之更新，例如填充 token 不再计入平均值时。跨越这类变化升级后，BERT 记忆会使用新的集合或索引以及新的 Redis 热缓存。与更换模型时一样，新集合从空开始，并随新流量重新积累记忆。升级前存储的条目仍保留在旧集合中，但不会再被召回。由其他运行时提供的 BERT 保留已有存储。
