...
`header_mutation` 是一个路由局部插件，用于添加、更新或删除下游请求头。
...
有些路由需要不同于路由器其他部分的下游请求头。`header_mutation` 明确声明这种转换，而不是将其隐藏在代理或应用代码中。

## 何时使用

- 某个路由应在请求头中标记租户或套餐元数据
- 下游服务需要路由专用的请求头
- 调试或来源请求头只应添加到选定流量
...
请求头值是静态配置，而不是模板。不得使用不受信任的请求元数据来构造身份或授权请求头。完整示例见：
