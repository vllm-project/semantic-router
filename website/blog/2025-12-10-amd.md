...
AMD is a leading provider of high-performance computing and GPU solutions for AI at scale, powering data centers, cloud platforms, and cutting-edge model workloads worldwide. It is also a long-term technology partner for the **vLLM** community: from accelerating the **vLLM engine** on AMD GPUs to now exploring the next layer of the stack with **vLLM Semantic Router (VSR)**.

As the ecosystem moves from single-model deployment toward **Mixture-of-Models (MoM)** architectures, the core challenge is no longer “how big is your model”, but **how intelligently you orchestrate many models, tools, and workflows together**.

vLLM Semantic Router focuses on exactly this problem.

VSR acts as the **control plane for multi-model AI**—using semantics, domain context, cost, latency, and risk signals to decide *which* model (or chain of models) should handle each request. Together with AMD, we are working to make this routing layer:
...
* **Production-ready on AMD GPUs** – from inference to CI/CD to online playgrounds.
...
## Long-Term (1): AMD GPU–Powered CI/CD and End-to-End Testbed for VSR OSS
...
## Long-Term (2): An AMD-Backed Mixture-of-Models Playground
